...
> 当前状态：API P0 主链、Web 多浏览器/诊断主链、Android 设备租约主链、iOS/Appium 本地执行边界和性能增强代码已实现；真实设备、性能节点与外部服务验收仍在进行。
...
- [x] 支持数据集的顺序、随机、固定次数和受控组合策略，并对组合上限和输入摘要做脱敏处理。
...
- API 行为测试覆盖 SSE、multipart、Cookie、XML/XPath、JSON Schema 和脚本沙箱的成功/失败/超时路径。
...
- [~] 增加设备/系统版本/分辨率兼容性矩阵和隔离子运行汇总；真实设备池并行调度仍待验收。
...
- [x] 增加目标服务指标接入边界，区分 Worker 本机指标、平台依赖指标和目标服务指标；真实 Prometheus 联调仍待验收。
...
2. P0/P1：Web Trace、网络/Console 日志、多浏览器和元素资产管理。
