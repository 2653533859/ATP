...
| N1 | 接口测试 | 已完成受控 GraphQL、WebSocket、流式 gRPC；下一步完成完整报告闭环 | `[~]` | 协议创建/执行/断言/提取/报告/清理均有脱敏证据 |
...
1. **N1 完整报告闭环**：GraphQL、WebSocket、Server/Client/Bidi Streaming 已在 q19 受控目标完成创建、审批、执行、断言/提取和清理；下一步补报告导出、详情展示、步骤证据和临时数据清理回读。本地已有报告页面不能代替完整报告真实证据。
...
| 2 | API 测试工作台 | 环境变量、认证复用、OpenAPI/Postman、HTTP/GraphQL/WebSocket/gRPC、断言/提取/依赖 | q19 已通过 HTTP、会话复用、gRPC TLS Unary、GraphQL、WebSocket、三种流式 gRPC 以及 OpenAPI/Postman 导入预览/落库/回读/清理；协议用例已在保存和派发两层拒绝无效/空步骤；完整报告仍待真实目标闭环 | 请求、断言、变量传递、报告和权限边界完整 | `[~]` |
...
| API 完整报告尚未形成真实证据 | API 工作台只能维持“协议已验收、报告待验收” | 在已通过协议目标上完成报告导出、详情、步骤证据和清理回读 |
...
| N1 | 接口测试 | HTTP、环境变量、会话复用、OpenAPI/Postman 导入、gRPC TLS Unary、GraphQL、WebSocket 和流式 gRPC 形成闭环 | 请求、断言、提取、依赖、导入预览/落库/回读和清理有证据；完整报告仍待验收 | `[~]` q19 协议目标已通过；完整报告真实证据待验收 |
...
1. **N1 完整报告补齐**：协议目标已在 q19 受控环境完成真实执行；下一步在这些已通过的运行上补报告导出、详情展示、步骤证据和清理回读。本地报告页面已经存在不等于完整报告门禁关闭。
