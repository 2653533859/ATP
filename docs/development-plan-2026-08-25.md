...
- [x] 测试数据集影响范围中的套件、计划入口现在保留当前 `project_id`，跳转后可继续使用同一项目上下文，不会回到默认项目或混入其他可见项目。
- [x] 新增套件/计划影响入口回归；数据集定向 `9 passed`，前端全量 `69 files / 299 tests passed`，`vue-tsc` 和生产构建通过；独立审查未发现问题。
