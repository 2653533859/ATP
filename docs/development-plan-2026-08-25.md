...
| 2 | API 测试工作台 | 环境变量、认证复用、OpenAPI/Postman、HTTP/GraphQL/WebSocket/gRPC、断言/提取/依赖 | 使用可控真实 HTTP 目标完成一条 API 用例并复核报告 | 请求、断言、变量传递、报告和权限边界完整 | `[E]` |
...
1. **API 真实协议目标**：在 q19 的受控目标上创建临时 API 用例，验证环境变量、认证复用、请求执行、断言、运行终态和报告；成功后清理临时项目、用例和产物。
...
| API 真实协议目标尚未形成证据 | API 工作台只能标记本地实现 | 使用受控 HTTP/API 目标完成临时用例并清理 |
