...
> 当前状态统一说明（截至 2026-09-01）：2.4.1～2.4.21 是 2.4.22 之前的过程快照，若其中的待验收描述与本节总表冲突，以 2.4.0 总表和 2.4.22 受控真实验收为准；本轮本地联调记录见 2.4.0.6，前端主题与交互收口见 2.4.23，Hermes 智能化第一阶段见 2.4.24，Hermes 多轮上下文见 2.4.25，Hermes 只读工具执行层见 2.4.26，Hermes 结构化草稿审阅与人工确认见 2.4.27，Hermes 评测与治理见 2.4.28。
...
6. 当前状态：Hermes H1～H5 本地实现、测试、审查修复和文档同步已完成；真实模型阈值、token usage/成本、角色矩阵、审计和目标部署仍需独立验收。P4 环境资源准备立即启动并与 P0～P3、外部供应商复核并行，P9 发布收口在所有必要门禁完成后绑定最终 SHA。
...
- H2～H5 已有本地实现和自动化证据；更深的真实模型阈值、token usage/成本、角色矩阵、审计与目标部署证据仍需独立环境验收，不因本地通过而扩大写入或执行范围。

**Status**：`[E]` Hermes H1～H5 本地实现、测试、审查修复和文档同步完成；真实模型、角色矩阵、调用审计、成本与目标部署仍按 U-P5-P8-EXT 复核，不改变 P4/P9 发布结论。
...
- 客户端历史是不可信上下文，只用于连续对话参考，不能替代当前项目来源；H3 已提供显式只读工具 API，自动工具编排仍未开放。
...
- `[E]` H3 是本地工具执行层，工具通过显式 API 调用；当前尚未接入 LLM 根据自然语言自动选择或串联工具。
...
**Status**：`[E]` Hermes H1～H5 本地实现、测试、审查修复和文档同步完成；H5 的真实模型效果、成本和发布门禁仍保持外部验收边界。
