...
- 目标主机仍需部署当前提交，保留迁移与容器日志，并连续观察 backend、Worker、Redis、PostgreSQL 和 MinIO；预检通过不能替代真实重启恢复证据。
