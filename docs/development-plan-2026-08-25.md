...
## 2.1.0 当前开发计划与跟踪快照（2026-08-25）

本节是当前唯一有效的执行游标，覆盖下方早期 2.0 计划中的历史顺序描述。N2 Karing 单设备闭环已经完成；当前先处理 N4 的真实性能环境门禁，环境条件不足时不把本地代码或 q19 Compose 观察结果写成生产通过。每个可交付模块继续执行：**实现/调整 → 定向测试 → 受影响全量门禁 → 独立代码审查 → 修复 → 文档与记忆同步 → 提交推送**。
...
- **N5**：先验证模型列表拉取和健康检查，再验证多模态与思考参数开关、草稿生成、来源/审计、限额和敏感值脱敏；只保存摘要，不保存 API Key 或原始提示中的密钥。
