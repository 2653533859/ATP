...
> 2026-09-08 更新：本文保留为历史交付与环境证据记录。新增开发游标、架构收口和单节点发布范围以 [`development-plan-2026-09-08.md`](development-plan-2026-09-08.md) 为准；多节点高可用与跨主机 MinIO 灾备不再阻塞当前单节点版本。
...
## 2.4.36 单节点 K3s Helm 联调准备（完成，2026-09-04）

本项只准备受控的 Helm 联调入口，避免把 P4 的双副本跨节点验收 overlay 误用在当前单节点 K3s。

### 交付内容与验收出口

- [x] 新增 [`values-performance-single-node.example.yaml`](../deploy/helm/atp/values-performance-single-node.example.yaml)：固定 1 个稳定身份为 `atp-single-node` 的性能 Worker 和专用队列，普通 Worker 排除性能队列；关闭跨节点反亲和、HPA、Ingress 与 ServiceMonitor。
- [x] overlay 仅引用外部 `atp-single-node-secrets`，三个组件固定 `imagePullPolicy: Never`，要求先导入同一不可变 SHA 的 backend、worker、frontend 镜像，拒绝隐式外部拉取；不含 Secret、镜像端点、认证或 kubeconfig 内容。
- [x] 仓库部署预检、部署契约定向 `24 passed`、本地 Helm lint 与 template 通过；目标 Linux 已按 SHA-256 校验安装 Helm `v4.2.4`，使用临时 Chart 副本复核远端 Helm lint/template 通过，临时目录已清理。
...
## 2.4.40 Windows 本地前端连接 Linux Backend（完成，2026-09-04）

本项收口“代码和页面在 Windows、本地前端通过真实 Linux Backend 联调”的开发链路，不迁移前端项目，也不把本地代理验证写成生产发布验收。

### 实现与审查

- [x] Vite 的 `/api` 和 `/ws` 代理不再硬编码 Windows `127.0.0.1:8000`，改由 `VITE_BACKEND_ORIGIN` 统一驱动；未配置时继续回退到本机 Backend，保持既有本地开发行为。
- [x] 新增代理解析和 HTTP/WebSocket 目标回归测试；独立审查未发现需要修复的正确性问题，`git diff --check` 通过。
- [x] 启动文档补充远程 Linux Backend 的 PowerShell 配置方式，并保留凭据不入库边界。

### 联调出口

- [x] 使用 `VITE_BACKEND_ORIGIN=http://192.168.3.196:8000` 临时启动 Windows Vite；本地前端根页面 HTTP 200。
...
## 2.4.45 单节点 k6 短任务、取消恢复与清理（完成，2026-09-05）

- 使用用户提供的 ATP 账号登录真实 API，凭据仅在专用终端进程环境中使用，未写入仓库。节点数据库 ID `2` 对应 `atp-single-node`，任务显式指定该节点，队列为 `performance.atp-single-node`；运行镜像仍是 `1bccfef4`、Helm revision 6。
- 复用已有独立验收容器 `/healthz`，仅在临时 k6 脚本中把现有 allowlist 测试域名映射到该容器 IP；未修改全局 DNS、系统白名单、旧 Compose 目录或业务服务。每次 1 VU、请求间隔 1 秒、单请求超时 3 秒。
- Run `4`：10 秒、10 次迭代、错误率 0、success。Run `5`：配置 60 秒，观察到 running 后等待 3 秒发送 stop，最终 cancelled。Run `6`：随后运行 5 秒、5 次迭代、错误率 0，证明取消后可继续执行。三次 JSON 报告中的运行 ID/状态均核对通过。
- 初次临时脚本遗漏读取 `ATP_K6_OPTIONS`，Run `2/3` 仅执行一次后成功结束，stop 返回 409；该次不计入通过。修正临时脚本后才得到以上正式短任务/取消/恢复结果。
- 重跑创建临时项目时发现默认编码按名称截断，两个不同名称生成同一编码，触发唯一索引及 HTTP 500。当前代码为自动编码追加 UUID 随机后缀，手工编码和旧项目不变；相关 API 回归 `37 passed`、受影响文件独立 `28 passed`，Ruff/格式检查通过。审查覆盖截断碰撞、中文/同名项目、编码长度与手工编码保留。此修复尚未部署 Linux，不以本次旧镜像验收替代修复后的真实复验。
- 临时项目 `69/71` 及其 5 条运行记录已删除并通过 API 404 核验；2 个上传脚本和 4 个结果对象共 6 个 MinIO 对象按精确名称清理，剩余为 0。Worker 中残留 k6 进程为 0，5 个核心 Pod Ready/0 重启、Backend 健康。原始临时产物已删除，仅保留脱敏摘要证据。
...
- 基于原 Worker `1bccfef4` 构建仅覆盖 `performance_process.py` 的补丁镜像 `process-group-042b3267fd71`，导入 containerd 后 Helm revision 7 部署成功。提交钩子随后发现 Windows mypy 缺少 POSIX 专属符号类型，改为在 POSIX 分支内解析 `killpg/SIGKILL`；93 项回归与 Linux 真实子进程检查重新通过，再同步最终补丁 `process-group-b7fcd25390d5` 至 revision 8，部署文件 SHA-256 与最终代码一致、5 个 Pod Ready/0 重启、Backend 健康。JMeter 三段任务实测发生在 revision 7，revision 8 仅复核源码和启动状态，不混写测试版本。此为单文件补丁，不是当前 main 全量部署，2.4.45 项目编码修复没有随之部署。
