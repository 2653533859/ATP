...
| 测试资产 | 测试用例、测试计划、缺陷管理、测试报告、用例评审 | 管理可复用资产，串联运行、失败证据、缺陷和评审 | 运行结果无法追溯到用例、计划和缺陷 |
...
| N2 | APP 自动化 | Windows Agent/ADB、设备池、APK 包名、低代码、录屏、专项任务和报告 | Karing 真实 APK 在单设备完成执行、事件、日志、媒体、报告和清理 | `[~]` 通用 APK 本地链路完成，Karing 真机门禁待解除 |
...
| N6 | 测试资产 | 用例、计划、缺陷、报告、评审之间的关联、追踪和审计 | 失败运行可追到证据/缺陷/评审，项目权限隔离有效 | `[E]` 本地完成，真实项目和外部平台待复核 |
...
2. **N2 Karing 单设备闭环**：取得 Karing APK 或设备确认出的真实 `package_name` 后，按 APK → 低代码 → 录屏/异常回放 → 专项任务 → 事件/日志/报告 → 下载与清理执行。
