...
2. **Redis Pub/Sub 监听**：在 `run_test_suite` 启动一个 Redis subscriber 监听所有子用例的 `atp:run:{run_id}` channel，收集完成事件后更新 SuiteRun（需要将 SuiteRun 状态变更也写入 Redis channel `atp:suite:{suite_run_id}`）
...
- 项目已有 Flower 监控（Celery beat schedule 中无 Flower，但有 flower 部署）
