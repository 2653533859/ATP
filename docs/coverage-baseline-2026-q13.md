...
| After Q14-00 evidence tooling | 1323 passed | 82.19% | 70% | Added publishable Q12 external-evidence templates, scaffold/validator helpers, and an automated Prometheus/API/ADB collector behind `make collect-q12-evidence` with a fake-source collector regression; the remaining SLO/device capture is no longer manual, but still requires live external sources before Q14-00 can close. |
