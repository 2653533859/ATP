...
- [~] P2.4 前端 E2E（MVP 落地 2026-05-21：Playwright + chromium + mock API 模式；`frontend/e2e/` 5 个 spec（login 3 例 / dashboard / case-list / case-run / run-detail）；`playwright.config.ts` 自动启 vite dev；nightly cron 03:43 UTC + workflow_dispatch；CI 缓存 ms-playwright 浏览器目录。下一迭代：渐进引入 data-testid 提升选择器稳定性 + 视觉回归 baseline + 真后端 E2E 子集）
