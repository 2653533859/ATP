...
> 当前计划跟踪版本为 2.4.0，最新同步检查点为 2.4.37：参考导航按工作台、测试能力、测试资产、智能中枢、系统五组跟踪，按 P0 工作台、P1 接口、P2 APP、P3 UI、P4 性能、P5 AI、P6 测试资产、P7 智能中枢、P8 系统和 P9 发布收口推进。P5～P8 已在 2.4.22 取得 q19 受控真实证据；2.4.23～2.4.31 已完成前端主题与 Hermes H1～H8 的本地交付，2.4.32～2.4.33 已完成 Linux Compose 启动韧性及预检门禁本地交付，2.4.34～2.4.37 已完成目标 Kubernetes 客户端、单节点 K3s、Helm 与外部 Secret 联调准备；这些本地交付不改变 P4 未完成真实环境复核前的“存在未关闭门禁”结论；Windows 与 Android Worker 的本地边界不影响 Web/API 使用。

> 2026-09-04 P4 目标环境预检、客户端、单节点 K3s、Helm 与外部 Secret 准备：初始预检发现目标 Linux 没有 `kubectl` 和 Kubernetes context，随后按官方二进制路径安装并 SHA-256 校验 `kubectl v1.37.0`；再按单节点范围固定安装 K3s `v1.36.3+k3s1`，root 私有 `0600` kubeconfig 的 context 为 `atp-single-node`。节点、CoreDNS、local-path-provisioner、Metrics Server 和 API `/readyz` 均已就绪；公共镜像仓库超时通过复用 Docker 既有、无凭据且可达的镜像源恢复，未记录镜像端点或认证。Helm `v4.2.4` 也已按 SHA-256 校验安装，并以临时 Chart 副本通过远端 lint/template；没有创建应用 release 或 Pod。已从目标 Linux 当前 ATP 运行容器受控创建 `atp-single-node` namespace 内的 `atp-single-node-secrets`，三个 Host 改为节点内部地址，Secret 完整性通过且未输出值；临时 Pod 到 PostgreSQL `5432`、Redis `6379`、MinIO `9000` 的 TCP 连通均通过，探针已清理。目标集群仍没有 Prometheus Operator/ServiceMonitor CRD，K3s containerd 也没有 ATP 镜像。单节点环境只用于 Kubernetes 开发/联调，不能证明发布级 Prometheus、独立 MinIO、跨主机恢复或原 P4 多节点验收。脱敏记录见 [`evidence/k3s-single-node-secret-preflight-2026-09-04.json`](evidence/k3s-single-node-secret-preflight-2026-09-04.json)；P4 保持 `[-]`，发布结论继续为“存在未关闭门禁”。
