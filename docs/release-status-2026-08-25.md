...
> 2026-09-04 P4 目标环境只读预检与客户端准备：初始预检发现目标 Linux 没有 `kubectl`，随后按 Kubernetes 官方二进制路径安装并 SHA-256 校验 `kubectl v1.37.0`；当前仍未配置 Kubernetes 上下文，因而无法取得 Ready 节点。Docker 中仅统计到 Prometheus、MinIO 和性能相关容器，单机容器不能证明发布级指标采样，也不能满足不同主机或不同 IP 的 MinIO source/target 要求。预检和安装验证未记录凭据、kubeconfig、环境变量、Compose 值或容器名称，脱敏记录见 [`evidence/p4-environment-preflight-2026-09-04.json`](evidence/p4-environment-preflight-2026-09-04.json) 与 [`evidence/p4-kubectl-install-2026-09-04.json`](evidence/p4-kubectl-install-2026-09-04.json)。P4 保持 `[-]`，发布结论继续为“存在未关闭门禁”。
