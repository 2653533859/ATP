...
> 2026-09-01 Hermes H2 多轮上下文本地交付：Hermes 查询增加项目绑定会话 ID、最多 12 条历史、服务端脱敏与字符预算裁剪；新增来源类型和更新时间范围筛选，SQL 在 limit 前执行时间过滤；前端展示会话状态、历史使用/裁剪统计和新会话操作，并丢弃切换项目后的旧请求响应。后端 H2 定向 `12 passed`、后端非集成全量 `2400 passed`、前端 Hermes 定向 `11 passed`、前端全量 `69 files / 325 tests passed`，TypeScript 检查和提交前门禁通过；H2 不持久化会话，真实跨副本会话、模型、角色矩阵和调用审计仍待复核，不改变 P4/P9 发布阻塞结论。
