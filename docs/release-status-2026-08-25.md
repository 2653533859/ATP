...
> 当前计划跟踪版本为 2.4.0，最新同步检查点为 2.4.39（进行中）：参考导航按工作台、测试能力、测试资产、智能中枢、系统五组跟踪，按 P0 工作台、P1 接口、P2 APP、P3 UI、P4 性能、P5 AI、P6 测试资产、P7 智能中枢、P8 系统和 P9 发布收口推进。P5～P8 已在 2.4.22 取得 q19 受控真实证据；2.4.23～2.4.31 已完成前端主题与 Hermes H1～H8 的本地交付，2.4.32～2.4.33 已完成 Linux Compose 启动韧性及预检门禁本地交付，2.4.34～2.4.38 已完成目标 Kubernetes 客户端、单节点 K3s、Helm、外部 Secret、当前 SHA 镜像导入和服务端 dry-run 联调准备，2.4.39 已定位并修复单节点首装中的迁移 Hook/ConfigMap 顺序缺陷，实际安装复核进行中；这些本地交付不改变 P4 未完成真实环境复核前的“存在未关闭门禁”结论；Windows 与 Android Worker 的本地边界不影响 Web/API 使用。

> 2026-09-04 单节点 K3s 首次 Helm 安装诊断：迁移 `pre-install` Hook Pod 先因依赖尚未创建的普通 ConfigMap 处于 Pending，修复后又暴露 q19 PostgreSQL 端口仅绑定宿主机回环、普通 Pod 误连另一套 `5432` 服务导致数据库鉴权失败。Chart 已让迁移 Job 内联非敏感配置，并为单节点 overlay 增加显式 `hostNetwork` 开关；联调 Secret 的 Host/Port 已切到 q19 回环端点，未输出凭据或修改旧 Compose 文件。修复后的目标安装与稳定性证据待提交推送后重新执行。
