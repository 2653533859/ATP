...
> 2026-09-04 单节点 K3s 首次 Helm 安装诊断：迁移 `pre-install` Hook Pod 因依赖尚未创建的普通 ConfigMap 处于 Pending，目标事件明确为 `configmap ... not found`；镜像已存在且 PostgreSQL/Redis/MinIO TCP 预检此前已通过。Chart 已改为让迁移 Job 内联非敏感 ConfigMap 配置、只从已存在 Secret 读取敏感配置，并补充回归测试；修复后的目标安装与稳定性证据待提交推送后重新执行。
