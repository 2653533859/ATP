...
> 2026-09-20 C3.4.2 首个 7 日窗口完成但初始校准未通过：2026-09-13～19 的 Backend/Worker 每日均为 288/288，只有 9 月 14 日存在 34 次有界合成请求和 3 次运行，可用性 100%、5m/1h P95 95ms、运行成功率 100%；其他 6 日无业务样本，共 24 项 data gap，无业务 breach，告警和发布门禁继续 deferred。审查修复 Prometheus 午夜 range 样本被错误归入次日、导致“运行量 0、成功率 100%”的跨日污染，定向 `20 passed`、非集成后端全量 `2618 passed / 1 skipped`。主机重启后 Backend/Prometheus 均为 200、revision 46 deployed、6/6 Pod Ready；Pod 单次 `Unknown/255` 与 WSL 主机重启一致，Prometheus 容器自身 RestartCount 为 0。详见 [`slo-history-2026-09-13-2026-09-19.md`](slo-history-2026-09-13-2026-09-19.md) 与 [`evidence/c3-slo-history-day7-2026-09-20.json`](evidence/c3-slo-history-day7-2026-09-20.json)。该窗口只证明 scrape 连续性，不满足代表性 7/14 天、独立 MinIO、P4 或 P9 门禁。
...
> 2026-09-10 C1.2 单节点性能生命周期完成：真实 k6 验收发现普通上传脚本未应用平台 options，提交 `76115f34` 改用原生临时 config 后部署至 Helm revision 40。修复后短压完成 974 次迭代、错误率 0、2/2 Threshold 通过、4 项基线指标无回归，JSON/CSV/raw summary 和资源样本均核对；运行中取消无进程、临时目录、队列或对象残留，5 条验收运行和 3 个对象已清理，原测试配置及节点 allowlist 已恢复。30 秒 4 次收尾采样保持 6/6 Pod Ready、零重启、0 告警。该证据关闭 C1.2 单节点联调范围，不替代发布级 Prometheus/Operator、长期 SLO、独立 MinIO 或多节点证据，因此 P4 继续阻塞。详见 [`evidence/c1-performance-lifecycle-2026-09-10.json`](evidence/c1-performance-lifecycle-2026-09-10.json)。

> 2026-09-10 C1.1 单节点有界可观测性入口完成：提交 `67ef34f4` 新增 Kubernetes API 采样器，在当前 `atp-single-node` 连续 40 秒取得 5 个样本；每次均为 1 节点 Ready、6/6 Pod Ready、零重启/Pressure，Backend、普通 Worker、Performance Worker 三个 Prometheus 指标端点持续覆盖且 0 告警。该采样器为没有 Operator 的单节点环境提供可重复、失败可见的联调证据，但不提供 Prometheus TSDB、ServiceMonitor target、告警执行或长期 SLO 历史，因此 P4 继续保持阻塞。脱敏证据见 [`evidence/c1-k3s-bounded-observability-2026-09-10.json`](evidence/c1-k3s-bounded-observability-2026-09-10.json)。

> 2026-09-10 B4 Android 单设备闭环已完成：当前 K3s/Windows Worker 链路通过连续低代码、逐步截图、device-info、logcat、录像和报告验证；活动设备断连会明确失败并可恢复扫描。Worker 中断实测发现运行回收后设备租约残留，提交 `ff14383c` 已修复为按 `case-run` 精确释放并写入恢复摘要，部署至 Helm revision 35 后 run 109 正确收敛且设备立即由 run 110 复用。50 秒六次采样保持 6/6 Pod Ready、零重启和三队列归零。该结论关闭单设备 B4，不代表可选多设备矩阵、发布级 Prometheus、生产性能或 P9 已完成。脱敏证据见 [`evidence/b4-android-fault-recovery-2026-09-10.json`](evidence/b4-android-fault-recovery-2026-09-10.json)。

> 2026-09-04 单节点 K3s 首次 Helm 安装诊断与修复：迁移 `pre-install` Hook Pod 先因依赖尚未创建的普通 ConfigMap 处于 Pending，修复后又暴露 q19 PostgreSQL 端口仅绑定宿主机回环、普通 Pod 误连另一套 `5432` 服务导致数据库鉴权失败。Chart 已让迁移 Job 内联非敏感配置，并为单节点 overlay 增加显式 `hostNetwork` 开关；联调 Secret 的 Host/Port 已切到 q19 回环端点，未输出凭据或修改旧 Compose 文件。修复后 Release `atp-single-node` 为 `deployed`，迁移成功，5 个核心 Pod Ready，Backend `/health` 返回 200，约 30 秒后无新增重启。脱敏证据见 [`evidence/k3s-single-node-helm-install-2026-09-04.json`](evidence/k3s-single-node-helm-install-2026-09-04.json)；P4/P9 仍未关闭。

> 2026-09-04 Windows 本地前端 → Linux Backend 联调：Vite 的 `/api`、`/ws` 代理已改由 `VITE_BACKEND_ORIGIN` 驱动。使用 `http://192.168.3.196:8000` 临时启动本地开发服务时，前端根页面返回 HTTP 200，通过本地代理访问受保护依赖接口返回预期 HTTP 401，确认请求到达 Linux Backend；前端全量 `70 files / 336 tests`、类型检查和生产构建通过。凭据和响应正文未写入证据；该结果只证明本地代理链路，不代表登录后权限、真实模型/Worker 或发布门禁通过。脱敏证据见 [`evidence/local-frontend-linux-backend-proxy-2026-09-04.json`](evidence/local-frontend-linux-backend-proxy-2026-09-04.json)。

> 2026-09-04 P4 单节点目标环境只读复验：`192.168.3.196` 的 context 为 `atp-single-node`，仅有 1 个 Ready 节点；性能 Worker 为 `1/1` 且资源 requests/limits 存在，但没有 `ServiceMonitor` CRD 或 Prometheus 工作负载，主机上的 MinIO 也不能证明独立 source/target。P4 保持 `[-]`，本次未修改集群、监控或 MinIO 数据；脱敏证据见 [`evidence/p4-single-node-blocker-2026-09-04.json`](evidence/p4-single-node-blocker-2026-09-04.json)。

> 2026-09-05 Flower OOM 与 hostNetwork 升级修复：目标 Linux Flower 旧 Pod 曾以 `OOMKilled/137` 退出，原内存 limit 为 `256Mi`；第一次资源升级因默认 RollingUpdate 与宿主机 5555 端口冲突产生 Pending Pod，随后回滚到 revision 3。Chart 提升 Flower limit 至 `512Mi`，hostNetwork 场景设置 `maxSurge=0/maxUnavailable=100%`，revision 5 升级成功；新 Flower Pod 及其它 4 个核心 Pod Ready，升级后 30 秒均为 0 重启，Backend `/health` 为 200，迁移 head 正确。该修复仅覆盖单节点运行稳定性，不关闭 P4/P9。脱敏证据见 [`evidence/k3s-single-node-flower-oom-fix-2026-09-05.json`](evidence/k3s-single-node-flower-oom-fix-2026-09-05.json)。

> 2026-09-04 P4 目标环境预检、客户端、单节点 K3s、Helm、外部 Secret 与当前 SHA 镜像联调：初始预检发现目标 Linux 没有 `kubectl` 和 Kubernetes context，随后按官方二进制路径安装并 SHA-256 校验 `kubectl v1.37.0`；再按单节点范围固定安装 K3s `v1.36.3+k3s1`，root 私有 `0600` kubeconfig 的 context 为 `atp-single-node`。节点、CoreDNS、local-path-provisioner、Metrics Server 和 API `/readyz` 均已就绪；公共镜像仓库超时通过复用 Docker 既有、无凭据且可达的镜像源恢复，未记录镜像端点或认证。Helm `v4.2.4` 也已按 SHA-256 校验安装，并以当前提交 Chart 通过 server-side dry-run；当前提交 `1bccfef4` 的 backend、worker、frontend 镜像已构建并导入 K3s containerd，dry-run 后没有应用 release、Pod 或迁移 Job。已从目标 Linux 当前 ATP 运行容器受控创建 `atp-single-node` namespace 内的 `atp-single-node-secrets`，三个 Host 改为节点内部地址，Secret 完整性通过且未输出值；临时 Pod 到 PostgreSQL `5432`、Redis `6379`、MinIO `9000` 的 TCP 连通均通过，探针已清理。目标集群仍没有 Prometheus Operator/ServiceMonitor CRD。单节点环境只用于 Kubernetes 开发/联调，不能证明应用启动、发布级 Prometheus、独立 MinIO、跨主机恢复或原 P4 多节点验收。脱敏记录见 [`evidence/k3s-single-node-image-dry-run-2026-09-04.json`](evidence/k3s-single-node-image-dry-run-2026-09-04.json)；P4 保持 `[-]`，发布结论继续为“存在未关闭门禁”。

> 2026-09-04 当前 SHA 镜像与 Helm server-side dry-run：构建输入为当前 Git archive；Backend/Worker 为避免目标基础网络与浏览器 CDN 阻塞，使用目标机上 `requirements.txt` SHA-256 与当前提交一致的依赖/浏览器层做源码覆盖构建，Frontend 按当前 Dockerfile 构建。仓库 Dockerfile 未修改，这是一项单节点开发/联调证据，不替代发布级可复现构建来源审计。三条精确镜像引用已导入 K3s containerd，server-side dry-run 通过；dry-run 不执行迁移或启动应用。完整脱敏记录见 [`evidence/k3s-single-node-image-dry-run-2026-09-04.json`](evidence/k3s-single-node-image-dry-run-2026-09-04.json)。

> 2026-09-04 Linux Compose 启动预检门禁本地交付：`validate-deployment-readiness` 现在覆盖默认及外部基础设施 Compose、backend 迁移门、健康依赖、外部 backend 有界重启和入口脚本语法；外部 backend 一旦跳过镜像默认迁移入口即被拒绝。具备 Docker/Compose 的发布机将对两套配置执行 `compose config --quiet`；本机无该运行时会明确显示 `SKIP`。部署契约 `30 passed`，后端非集成全量 `2451 passed`。目标主机的只读预检发现 ATP 目录仍为旧 SHA、detached HEAD，且有未提交 Compose 变更及备份文件；必须先由负责人保留/确认这些现场变更，禁止强制覆盖。预检只验证仓库契约，不能替代当前 SHA 的 Linux 构建、部署、连续稳定性观察或 P4/P9 的真实环境门禁。
...
> 2026-09-01 Hermes H4 结构化草稿审阅与人工确认本地交付：Hermes 计划建议支持名称、目标、测试点、模块、用例和失败任务回归范围编辑，并展示生成基线/当前 diff、证据来源和影响数量；未确认时不能进入计划页，确认后通过有界路由状态预填现有计划页，计划仍需用户选择套件并点击保存；持久会话路径仅在草稿二次确认后创建禁用手工计划，不自动执行，首次保存可按需创建空会话。Hermes/计划页定向 `20 passed`，后端非集成全量 `2429 passed`，前端全量 `69 files / 327 tests passed`，TypeScript、生产构建和 `git diff --check` 通过。真实模型、目标角色、审计和 P4/P9 门禁仍待独立复核，不改变发布阻塞结论。

> 2026-09-01 Hermes H5 评测与治理本地交付：新增版本化 `hermes-core-v1` 五题只读评测集和元数据接口；治理汇总按有效 `[S#]` 引用统计覆盖率，输出拒答/无结果率、平均/P95 延迟、prompt 版本、人工反馈、活动量和成本不可用状态；Hermes 页面增加紧凑治理状态卡片，并在项目切换时丢弃过期治理响应。H5 定向后端 `18 passed`、前端 Hermes `12 passed`，后端非集成全量 `2432 passed`、前端全量 `69 files / 327 tests passed`，TypeScript、mypy、生产构建、Python 编译、Ruff、格式/差异检查、密钥扫描及提交钩子均通过。真实模型阈值、token usage/成本、角色矩阵、审计和目标部署仍待独立复核，不改变 P4/P9 发布阻塞结论。

> 2026-09-04 Hermes H8 挂起意图取消与安全恢复本地交付：当 H7 的受控 `pending_orchestration` 存在时，精确取消词会在同一用户、项目、会话和 `conversation_id` 下返回 `cancelled` 并清除状态；不会调用工具、写入业务数据或扩大意图。跨会话取消不影响原状态，无 pending 的取消词也不会误路由为失败任务读取；取消后可继续新的编排或项目证据检索。取消控制被排除在 H5 回答质量、前端评价入口和后端反馈之外。Hermes 编排/服务定向 `27 passed`、前端 Hermes `19 passed`，后端非集成全量 `2441 passed`、前端全量 `69 files / 334 tests passed`，TypeScript、mypy、生产构建、Python 编译、Ruff、格式/差异检查、密钥扫描及提交钩子均通过。真实模型工具选择效果、角色矩阵、成本 usage、完整审计和目标部署仍待独立复核，不改变 P4/P9 发布阻塞结论。

> 2026-09-03 Hermes H7 多轮参数补全本地交付：`needs_input` 会把最小 allow-list 只读意图写入当前用户/项目会话；只有相同会话和 `conversation_id` 的下一轮输入能补齐原来的运行、需求/用例或知识工具，单独数字不在会话外执行。页面恢复会校验并复用已保存的 `conversation_id`，所以刷新后仍可续接，项目切换时旧会话也不会迟到覆盖新项目。明确新意图会清除旧状态，新的追问会替换旧状态；控制性追问被排除在 H5 回答质量和人工反馈之外，前端刷新后也不显示评价入口。Hermes 编排/服务定向 `25 passed`、后端受影响文件独立 `18 passed`/`7 passed`、前端 Hermes `17 passed`，后端非集成全量 `2439 passed`、前端全量 `69 files / 332 tests passed`，TypeScript、mypy、生产构建、Python 编译、Ruff、格式/差异检查、密钥扫描及提交钩子均通过。真实模型工具选择效果、角色矩阵、成本 usage、完整审计和目标部署仍待独立复核，不改变 P4/P9 发布阻塞结论。

> 2026-09-03 Hermes H6 受控自然语言只读编排本地交付：新增 `/hermes/orchestrate`，从五个固定 H3 只读工具中按自然语言最多选择两步；缺少运行/需求/用例/知识显式目标时返回 `needs_input`，未知问题返回 `no_match` 并回退 H1 项目证据检索。实际工具调用复用 viewer 权限、参数白名单、5 秒超时、脱敏、稳定证据、审计和项目/用户会话；Hermes 页面展示并恢复自动读取链路。H6 定向后端 `22 passed`、前端 Hermes `14 passed`，后端非集成全量 `2436 passed`、前端全量 `69 files / 329 tests passed`，TypeScript、mypy、生产构建、Python 编译、Ruff、格式/差异检查、密钥扫描及提交钩子均通过。真实模型工具选择效果、角色矩阵、成本 usage、完整审计和目标部署仍待独立复核，不改变 P4/P9 发布阻塞结论。

> 2026-09-01 前端主题与交互收口已完成本地实现、独立审查、修复和回归：统一 Apple 玻璃感主题，增加实时 Web 录制 Worker 状态卡片，快捷搜索保留 `keyword`/`project_id` 深链，补齐键盘语义按钮、中英文文案和 390px 移动端无溢出适配。前端全量 `69 files / 321 tests passed`，`type-check`、生产构建、浏览器冒烟和 `git diff --check` 通过；该结果只关闭本地交付，不替代真实角色、目标部署或 P4 性能环境证据。

> 2026-09-01 Hermes H1 智能化本地交付：项目级检索结果在存在启用 AI 配置时进入统一 LLM 客户端，提示词仅使用脱敏项目证据并要求 `[S1]` 引用；未配置、超限、模型异常或无有效引用自动回退规则检索，模型混合文本中的 JSON 敏感字段也经共享脱敏器处理。响应新增 `llm_grounded` 模式，前端显示生成模式标签并修复欢迎语 i18n key。后端 Hermes 定向 `8 passed`、LLM 脱敏定向 `4 passed`、非集成全量 `2396 passed`，前端全量 `69 files / 321 tests passed`，Ruff、Python 编译、前端生产构建和浏览器自由提问冒烟通过；真实模型、角色矩阵、调用审计和目标部署仍待复核，不改变 P4/P9 发布阻塞结论。

> 2026-09-01 Hermes H2 多轮上下文本地交付：Hermes 查询增加项目/用户隔离会话、最多 12 条历史、服务端脱敏与字符预算裁剪；新增来源类型和更新时间范围筛选，SQL 在 limit 前执行时间过滤；前端展示会话状态、历史使用/裁剪统计和新会话操作，并在新会话时清空旧 Session ID，丢弃切换项目后的旧请求响应。后端 H2 定向 `12 passed`、后端非集成全量 `2400 passed`、前端 Hermes 定向 `11 passed`、前端全量 `69 files / 325 tests passed`，TypeScript 检查和提交前门禁通过；会话虽已持久化，跨副本部署、模型、角色矩阵和调用审计仍待复核，不改变 P4/P9 发布阻塞结论。

> 2026-09-01 Hermes H3 只读工具执行层本地交付：新增固定工具目录和显式执行 API，覆盖失败任务、运行详情、质量趋势、需求—用例关联和知识详情；所有工具要求项目 viewer 权限，参数按专属白名单校验，最长执行 5 秒，结果统一脱敏并返回稳定 `HERMES-*` 证据路径，执行摘要写入 `hermes_read_tool` 审计且不记录原始参数或正文。H3 API/服务定向 `11 passed`、H1～H3 Hermes 组合回归 `27 passed`、后端非集成全量 `2411 passed`、前端全量 `69 files / 325 tests passed`；当时自然语言自动工具编排、真实模型、角色矩阵、跨副本会话和目标部署仍待复核，H6 当前状态见上方 2.4.29 记录，不改变 P4/P9 发布阻塞结论。
...
> 2026-08-25 P1 API 工作台项目环境列表竞态保护本地交付：项目序列保护环境列表的成功、失败和 loading 收尾，旧项目响应不会覆盖最新项目；定向 `6 passed`、前端全量 `69 files / 308 tests passed`，`vue-tsc`、生产 build 和 diff-check 通过。真实角色和执行数据仍待环境复核，不改变发布阻塞结论。

> 2026-08-25 P1 API 工作台项目刷新竞态保护本地交付：项目列表刷新增加序列保护，旧刷新响应/错误不会覆盖最新项目；清空项目时用例请求也推进序列并停止 loading；定向 `8 passed`、前端全量 `69 files / 310 tests passed`，`vue-tsc`、生产 build 和 diff-check 通过。真实角色和执行数据仍待环境复核，不改变发布阻塞结论。

> 2026-08-25 P1 UI 自动化工作台运行记录竞态保护本地交付：最近运行记录和随后的用例详情加载均绑定用例序列，旧模块响应不会覆盖最新模块；定向 `5 passed`、前端全量 `69 files / 311 tests passed`，`vue-tsc`、生产 build 和 diff-check 通过。真实角色和浏览器执行数据仍待环境复核，不改变发布阻塞结论。

> 2026-08-25 P2 APP 自动化工作台项目数据竞态保护本地交付：项目列表刷新和项目数据整组加载增加代际保护，设备/Worker、APK、Android 用例、专项任务/运行及 Android 运行记录的旧响应不会覆盖当前项目；定向 `4 passed`、前端全量 `69 files / 312 tests passed`，`vue-tsc`、生产 build 和 diff-check 通过。真实角色和 Android/Worker 执行数据仍待环境复核，不改变发布阻塞结论。

> 2026-08-25 P8 N8 治理角色矩阵 token 契约本地交付：普通 viewer 角色矩阵支持短期 `ATP_VIEWER_TOKEN`，令牌优先并兼容用户名/密码，认证值不写入证据；N8 定向 `7 passed`、后端非集成全量 `2387 passed`，Ruff/格式、发布契约 `19 passed` 和 diff-check 通过。真实管理员/viewer 与目标部署治理证据仍待环境复核，不改变发布阻塞结论。

> 2026-08-25 P7 N7 智能中枢角色矩阵 token 契约本地交付：普通 viewer 角色矩阵支持短期 `ATP_VIEWER_TOKEN`，令牌优先并兼容用户名/密码，认证值不写入证据；N7 定向 `12 passed`、后端非集成全量 `2389 passed`，Ruff/格式和 diff-check 通过。真实 viewer/admin、模型与目标部署治理证据仍待环境复核，不改变发布阻塞结论。
