...
Hermes 模块开发说明、H1～H5 路线和本地联调入口见 [`hermes-development-plan-2026-09-01.md`](hermes-development-plan-2026-09-01.md)。

该脚本验证“临时项目 → 模块 → 需求/知识/用例 → 需求解析与检索 → Hermes 来源引用 → 清理”的链路。blank 项目没有模块时，脚本会显式创建一个临时模块。Hermes 查询只读取当前项目和已发布的全局知识，不会自动修改测试资产；来源响应包含项目上下文、匹配词、脱敏摘要和可回看的路径。项目绑定启用的 AI 配置且检索到来源时，查询会额外尝试返回 `mode=llm_grounded` 的证据约束总结；未配置、超限、调用失败或无有效引用时返回 `project_retrieval`，不影响来源验收。H2 还支持显式传入会话 ID、有限历史、来源类型和更新时间范围；H3 增加五个项目 viewer 权限保护的显式只读工具，H4 增加前端结构化草稿审阅与人工确认后交接到现有计划页，同时远端会话路径支持二次确认后创建禁用手工 draft；这些能力都不改变项目权限边界或自动执行计划；工具结果脱敏、限时并带证据路径。
