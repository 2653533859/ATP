...
页面与 `.env.example`、后端 `backend/app/core/config.py` 对齐；配置数量由契约测试动态校验：

### 基础设施

`POSTGRES_HOST`、`POSTGRES_PORT`、`POSTGRES_CONNECT_TIMEOUT_SECONDS`、`POSTGRES_DB`、`POSTGRES_USER`、`POSTGRES_PASSWORD`、`POSTGRES_MIGRATION_USER`、`POSTGRES_MIGRATION_PASSWORD`、`REDIS_HOST`、`REDIS_PORT`、`REDIS_USERNAME`、`REDIS_PASSWORD`、`REDIS_CONNECT_TIMEOUT_SECONDS`、`MINIO_HOST`、`MINIO_PORT`、`MINIO_ROOT_USER`、`MINIO_ROOT_PASSWORD`、`MINIO_ACCESS_KEY`、`MINIO_SECRET_KEY`、`MINIO_BUCKET`、`MINIO_CONNECT_TIMEOUT_SECONDS`、`MINIO_READ_TIMEOUT_SECONDS`
...
生产环境可将 `POSTGRES_MIGRATION_USER/POSTGRES_MIGRATION_PASSWORD` 成对配置为仅供 Alembic 使用的 DDL 身份；留空时兼容复用运行账号。Redis 使用 ACL 时填写 `REDIS_USERNAME`，留空则使用 `default` 用户。MinIO 客户端优先使用成对的 `MINIO_ACCESS_KEY/MINIO_SECRET_KEY`，用于 bucket 级应用身份；留空时回退到 legacy `MINIO_ROOT_*` 字段。页面会将不完整的可选凭据对标记为未就绪，浏览器草稿不会保存迁移密码、MinIO Secret 或其他敏感字段。
...
五类长任务的数据库租约由 `EXECUTION_RUN_LEASE_TTL_SECONDS`（默认 90 秒）、`EXECUTION_RUN_LEASE_HEARTBEAT_SECONDS`（默认 30 秒）和 `EXECUTION_RUN_LEASE_RECONCILE_INTERVAL_SECONDS`（默认 60 秒）控制。心跳线程实际续期间隔会自动限制在 TTL 的三分之一以内，配置修改后需同时重启执行 Worker 和 Maintenance Worker/Beat。

`CELERY_CONCURRENCY`、`CELERY_QUEUES`、`WORKER_METRICS_PORT`、`FILE_RETENTION_DAYS`、`STALE_PENDING_CLEANUP_ENABLED`、`STALE_PENDING_TIMEOUT_MINUTES`、`STALE_PENDING_CLEANUP_INTERVAL_SECONDS`、`RUN_CLEANUP_ENABLED`、`RUN_RETENTION_DAYS`、`RUN_CLEANUP_BATCH_SIZE`、`NOTIFICATION_DELIVERY_CLEANUP_ENABLED`、`NOTIFICATION_DELIVERY_RETENTION_DAYS`、`AUDIT_LOG_CLEANUP_ENABLED`、`AUDIT_LOG_RETENTION_DAYS`、`ADB_SCAN_ENABLED`、`ADB_SCAN_INTERVAL`、`ADB_SCAN_MODE`、`ANDROID_WORKER_ID`、`ANDROID_WORKER_QUEUE`、`ANDROID_WORKER_REGISTRY_PREFIX`、`ANDROID_WORKER_HEARTBEAT_SECONDS`、`ANDROID_WORKER_TTL_SECONDS`、`ADB_RECONNECT_ENABLED`、`ADB_RECONNECT_MAX_ATTEMPTS`、`ADB_RECONNECT_BACKOFF_MS`、`ADB_HEARTBEAT_ENABLED`、`ADB_HEARTBEAT_INTERVAL_SEC`、`ADB_HEARTBEAT_FAILURE_THRESHOLD`、`CASE_SNAPSHOT_MAX_PER_CASE`、`MOCK_STANDALONE_PORT`、`WEB_RECORDER_MODE`、`WEB_RECORDER_WORKER_QUEUE_PREFIX`、`WEB_RECORDER_WORKER_ID`、`WEB_RECORDER_WORKER_MAX_SESSIONS`、`WEB_RECORDER_WORKER_HEARTBEAT_SECONDS`、`WEB_RECORDER_WORKER_TTL_SECONDS`、`WEB_RECORDER_COMMAND_TIMEOUT_SECONDS`、`WEB_RECORDER_REPLY_TTL_SECONDS`、`WEB_RECORDER_SESSION_TTL_SECONDS`、`WEB_RECORDER_DISPLAY`，以及性能指标采样、性能节点和性能执行器配置。
...
管理员页面和 Windows 全量冒烟会在登录后调用 `/api/v1/health/dependencies`，逐项检查 PostgreSQL、Redis、MinIO；该诊断接口只允许管理员访问，因为它会实际发起基础设施连接。任一基础设施不可用时，冒烟会在后续文件传输、Worker 和报告检查前明确失败，并将结果写入脱敏报告。公开存活检查仍使用 `/health`。
