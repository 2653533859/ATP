...
| Q13-03 | Frontend workbench behavioral coverage | Four slices (CaseList, RunDetail, SuiteList, DashboardView) extracting testable helpers per the proven Q12-02 pattern; frontend statements >= 8% with gates ratcheted per the 0.25pt headroom policy | In progress (CaseList + RunDetail slices done: statements 4.38%->5.10%, `58 passed`; SuiteList/DashboardView next) |
...
Continue Q13-03: SuiteList (537 uncovered) and DashboardView (513) slices remain, extracting testable helpers per the CaseList/RunDetail pattern toward frontend statements >= 8%. Q13-05 (AI healing apply-loop) is unblocked since ai_healing is covered.
