...
| Q13-01 | Backend execution-chain coverage | Unit seams for `worker/tasks.py` dispatch/finalization and per-executor request-build/assert/extract logic; backend TOTAL >= 60% with the CI gate raised 52% -> 56% | Planned |
...
Start Q13-01: introduce the executor unit-seam conventions (fake transport/driver fixtures per executor family), cover `worker/tasks.py` dispatch and finalization branches first (largest single gap, 362 missed statements), and record the before/after totals in a new `docs/coverage-baseline-2026-q13.md` following the Q12 baseline format.
