...
| Q13-03 | Frontend workbench behavioral coverage | Four slices (CaseList, RunDetail, SuiteList, DashboardView) extracting testable helpers per the proven Q12-02 pattern; frontend statements >= 8% with gates ratcheted per the 0.25pt headroom policy | In progress (CaseList + RunDetail + SuiteList done: statements 4.38%->5.54%, `62 passed`; DashboardView last) |
...
Continue Q13-03: DashboardView (513 uncovered) is the last workbench slice, extracting testable helpers per the CaseList/RunDetail/SuiteList pattern toward frontend statements >= 8%. Q13-05 (AI healing apply-loop) is unblocked since ai_healing is covered.
