...
- 单用例运行可导出 HTML、JUnit XML 和 PDF；HTML 按运行更新时间、模板和封面签名缓存在 `reports/run-{run_id}/`，相同内容再次下载直接命中缓存；
- HTML 缓存是可再生对象，可按 `FILE_RETENTION_DAYS` 独立淘汰；只要运行仍存在，后续下载会重新生成；
- `result_summary` 中属于当前 `run_id` 的录像、Trace、视觉差异和移动端产物会被登记为数据库引用，通用存储清理不得提前删除；引用其他运行路径的异常值不会获得保护，也不会在本运行清理时跨运行删除；
- `RUN_RETENTION_DAYS` 到期后，终态运行（包括 `cancelled`）按批提交数据库删除，再清理其截图、录像、Trace 和全部 HTML 缓存；数据库提交失败时保留 MinIO 对象，MinIO 删除失败时由后续存储治理发现孤儿对象。
