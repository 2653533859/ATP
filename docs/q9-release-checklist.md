...
- Dataset storage: run migration `20260812_0055`, then execute `scripts/minio-dataset-acceptance.py` against the release MinIO and a dedicated backup bucket. Require every check, including read-only policy, to pass. Upload database-mode and MinIO-mode datasets through the API, create a version snapshot and rollback, then run the administrator reconciliation endpoint in dry-run mode. Only after a verified object backup may staging repeat it with `{ "purge": true }`; retain the JSON report, response and audit-event evidence.
...
- Performance notification: configure an enabled project channel, use `scripts/notification-channel-smoke.py` with `ATP_TOKEN` or `ATP_USERNAME`/`ATP_PASSWORD`, and additionally run `scripts/notification-channel-acceptance.py` separately for each enabled SMTP/WeCom/DingTalk channel. Execute a performance run that produces metrics or a terminal failure, and verify that the delivered message includes RPS/P95/P99/error rate, threshold status and event reasons. Archive both scripts' redacted JSON reports together with recipient/provider-side delivery/message evidence. SMTP/WeCom/DingTalk delivery is external evidence; unit tests and a successful API response do not replace it.
- Windows Android Worker: on a Windows host with an authorized device, run `scripts/windows-android-acceptance.ps1` and `scripts/windows-local-smoke.ps1 -AndroidCaseId <approved-case-id> -RequireAndroidLowcode -RequireAndroidEvidence -SkipReports`. A missing device must remain `BLOCKED`, not be recorded as a pass.
- Linux/Kubernetes performance stack: run `scripts/performance-environment-smoke.py` with `--require-kubernetes`, a real Deployment, API, node, target, Prometheus, TLS and metrics arguments; retain JSON evidence for the worker queue, allowlist, cancellation and target metrics. Do not treat an invocation that omits the Deployment check as a Kubernetes pass.
- System governance: run `scripts/n8-system-governance-acceptance.py` against the target with `--require-role-matrix --allow-mutations --rollback --require-rollback`; retain only its redacted report plus audit evidence, and do not treat a local or Compose run as target-deployment proof.
