...
从“工作台 -> 我的待办”查看待评审用例、失败任务、逾期计划和设备异常。待办列表使用服务端分页，项目和 `todo_page` 会保留在 URL，切换项目会回到第一页；页面显示“还有更多待办”时可翻页查看后续事项。总数来自当前用户可见范围，分页上限为 1,050 条，超过上限时应通过项目筛选缩小范围。

从“工作台 -> 任务中心”统一筛选 Case、Suite、Plan、Android 和 Performance 任务。任务中心使用服务端分页，项目、状态、类型和页码会保留在 URL，筛选条件变化会回到第一页；重试、终止和批量操作仍按当前用户角色、任务状态和任务类型限制，操作完成后可跳转到原领域详情页。页面显示“还有更多任务”时可翻页或继续缩小筛选范围。
...
发布前请先查看 [`docs/release-status-2026-08-25.md`](release-status-2026-08-25.md)，确认能力证据、环境边界和待验收门禁。该索引明确区分本地回归、q19/Windows 证据与真实生产环境证据：ADB `offline`、回环 SMTP 的 `local_link_only`、无外部平台凭据或未启动的生产性能环境都只能记录为“待环境验收”，不能作为发布通过条件。
