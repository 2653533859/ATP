...
| 测试设计 | 项目管理、用例管理、测试套件、测试计划，负责从项目建模到场景编排 |
| 测试资产 | Mock 服务、测试数据集、Web 元素与 POM、API 契约资产，负责可复用测试资产 |
| 执行中心 | 执行记录、性能压测、存储管理、执行记录清理、看板告警 |
| Android 专项 | 设备管理、APK 管理、专项任务、报告中心 |
| 平台配置 | 环境管理、启动配置、全局变量、AI 模型与自愈能力 |
| 外部集成 | 通知配置、缺陷跟踪，负责连接外部协作和消息系统 |
| 安全管理 | 用户管理、审计日志，负责账号权限和操作追溯 |

侧栏按上述七组折叠展示，不再把所有入口集中在“系统管理”下面。进入某个页面时，会自动展开对应分组，并将顶部标题切换为具体页面名称；原有 URL、角色权限和书签均保持不变。
...
性能 Run 的通知沿用“外部集成 -> 通知配置”中的项目级通道。启用通知后，正常完成、阈值失败、基线回归、节点/资源异常，以及执行器未启用、节点不可用、容量不足和启动前取消等提前终止结果，都会进入同一通知链路；正文会展示 RPS、P95/P99、错误率、阈值状态和触发原因。只有 `scope=all` 的项目通道会匹配性能事件，套件/计划专用通道不会匹配。可用 `status_filters` 只接收失败告警。通知发送失败只记录 Worker 日志，不改变已经落库的 Run 结果。
