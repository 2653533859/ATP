...
当前目标 Linux 已按上述边界创建 `atp-single-node` namespace 和 `atp-single-node-secrets`。该 Secret 只复用主机当前运行服务的配置值，并将 PostgreSQL、Redis、MinIO Host 改为 K3s 节点内部地址；Secret 值未写入仓库。临时 Pod 到三项服务的 TCP 连通性已通过并清理；K3s containerd 尚未导入当前 SHA 的 ATP 镜像，因此暂不执行 Helm 安装。
