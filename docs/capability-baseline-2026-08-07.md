...
> 2026-08-25 API 证据：q19 受控目标已完成 HTTP 最小执行、显式会话复用和 gRPC TLS Unary；gRPC 使用公开 PEM 根证书和 SNI，状态断言/JSONPath 提取/清理通过，根证书不进入执行快照。脱敏证据见 [`api-grpc-tls-2026-08-25.json`](evidence/api-grpc-tls-2026-08-25.json)。OpenAPI/Postman、GraphQL/WebSocket/流式 gRPC、生产目标和完整报告仍待验收。
