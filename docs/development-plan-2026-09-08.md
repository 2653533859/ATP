...
- [~] `C3.4` 收集发布级 SLO、告警、服务重启和持续稳定性证据，明确 Redis 与 MinIO 的剩余灾备决定。
  ...
  - [~] `C3.4.2` 已将 SLO 采集从 Android 真机演练中解耦，增加 5 分钟连续性、非有限值、完整 UTC 日和数据缺口门禁，并固化安全的认证读/可选本机 API 运行 canary。2026-09-13～19 的首个 7 日 scrape 窗口因 6 日无业务样本而未通过；采集器已修复午夜 range 样本跨日污染。2026-09-20 已通过 20 次有界认证读取和 Run 115/116 启动新候选窗口，须在 UTC 日结束后确认首日，并继续每日产生受控流量直至 7/14 天。候选异机 `172.31.27.133` 的 VPN/路由和服务仍不可用；待恢复后配置 MinIO 备份端点并完成恢复演练。
