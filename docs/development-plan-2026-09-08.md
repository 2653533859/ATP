...
- [x] `H9.1` 模型生成候选工具计划；仅在确定性规则未命中且不是取消/挂起补全时调用。服务端重新执行固定工具白名单、严格参数 Schema、项目权限和最多两步限制；非法响应整体拒绝并回退，前端展示规划来源、校验结果和每步理由。
- [x] `H9.2` 记录 token usage、模型调用次数、延迟、成本估算和回退原因，并纳入治理汇总；兼容 OpenAI、Claude、Ollama 常见 usage 结构，价格只读取项目模型显式配置，缺失 usage 或价格时返回稳定原因码而不伪造成本。
- [x] `H9.3` 将固定评测集升级为 `hermes-core-v2` 十题，声明查询/编排入口、期望工具、来源类型、答案要点和拒答预期；只对精确题目响应评分，统计工具选择准确率、引用相关性、答案完整度和拒答正确率。
- [x] `H9.4` 从失败任务生成回归范围和套件映射建议：Case 运行反查包含该用例的活动套件，Suite 运行直接映射自身，Plan 运行展开原计划套件；建议按命中失败数排序、去重并限制 8 项，可逐项取消。确认交接只预填现有计划页并默认禁用；会话草稿二次确认时后端重新校验套件属于当前项目且仍为活动状态，只创建禁用手工 draft，不自动执行。
- [x] `H9.5` 为持久会话增加数据库乐观版本；消息、挂起意图、取消、草稿与治理指标的陈旧更新统一回滚并返回 `409`，前端显式提示刷新且不降级为其他请求或重复确认。治理聚合窗口按更新时间和 ID 稳定排序。本地双 Session 以及目标 K3s 同节点双 Backend 进程均验证无静默覆盖；同节点证据不替代多节点故障域或 P4/P9 验收。
...
- [~] `C3.4` 收集发布级 SLO、告警、服务重启和持续稳定性证据，明确 Redis 与 MinIO 的剩余灾备决定。
  ...
  - [~] `C3.4.3` 完成发布前 Chart/资源/迁移 Hook 差异预检，旧 Chart 漂移与未审查的 6 个 Deployment 变更均被拒绝；Flower RSS、进程启动时间和事件量纳入发布 Prometheus/Grafana，目标运行中的规则与样本已核验。9 月 24～25 日完整 UTC 报告确认前者 Backend 一次 `up=0`，后者虽然 `288/288` 但业务运行量低。首次 Worker 结果序列从 1 才出现导致日增量漏算，仓库零值初始化修复已完成而 revision 56 尚未部署。继续积累代表性流量与长期 Flower 趋势，暂不关闭 C3.4。
...
- 2026-09-25：Hermes 目标环境项目 Viewer/管理员角色与审计按事前冻结计划 **9/9** 通过。Viewer 在授权临时项目 A 获得带有效来源的真实模型回答和只读工具访问；A 的两项越权写入与 B 的五项跨项目访问均 403 并按项目、用户写入 `access_denied`；跨用户反馈 404，自己的重复反馈计数幂等。成员撤销后读取再次 403，A/B 项目、需求、会话清理而审计留存；项目 77 未变。Helm revision 55 四次 6/6 Pod Ready、健康 200、零重启。全局 Engineer/项目 Editor 完整矩阵、供应商侧审计、代表性业务内容和 C3.4/P4/P9 仍待独立验收。见 [`evidence/hermes-role-audit-2026-09-25.md`](evidence/hermes-role-audit-2026-09-25.md)。
- 2026-09-24：按事前冻结的五条项目 77 用例元数据、八题事实判据在独立临时项目验收 Hermes。首轮人工 3/8；`a49d0734` 修复用例类型/优先级/级别证据缺失和泛指用例漏检后，相同题集人工 8/8、缺失执行证据题 2/2、会话审计完整。正式单节点 Helm revision 55 四次采样 6/6 Pod Ready、健康 200、零重启；定向 71 项及单文件 38/33 项通过，临时资产清理，项目 77 保持未绑定模型。只关闭真实用例**元数据**样本，后续需业务确认的需求、知识、步骤及运行证据扩样；供应商审计、代表性长期 SLO 和 P4/P9 仍待验收。见 [`evidence/hermes-real-case-metadata-2026-09-24.md`](evidence/hermes-real-case-metadata-2026-09-24.md)。
- 2026-09-24：C3.4.2 以当前发布 Prometheus 和相同 SHA 的正式采集器生成 9 月 23 日完整 UTC 日报告：Backend/Worker 均 `288/288`，20 次请求、2 次运行，可用性 100%、P95 95ms、运行成功率 100%，无 breach 或 gap；采集器仍将告警/发布门禁标记 `deferred`。9 月 24 日定时 canary 最近一次服务退出码 0，脱敏报告为 `passed`，12 次认证读取与 Run 123/124 均通过，四个 Prometheus target 全部 up；当天尚未结束，不计为完整日。两日均仅有受控合成流量，不能替代代表性 7/14 日校准。见 [`slo-history-2026-09-23-2026-09-23.md`](slo-history-2026-09-23-2026-09-23.md)。

- 2026-09-24：Q06 续验中 `hermes-v5` 虽消除固定流程顺序，完整冻结题集却退化为人工 8/12、安全 2/3，故未采纳该轮质量结论。`de0d485c` 的 `hermes-v6` 将默认回答限定为所问结论、直接证据和必要未知项，相同 12 题最终人工 12/12、安全 3/3、审计链完整；定向 69 项、两个文件独立运行及静态质量钩子通过。正式单节点 Helm revision 54、6/6 Pod 就绪、健康 200、零重启，项目 96～99 及关联资产清理。项目 77 只有 17 条用例、无需求/知识且未绑定模型，实际业务代表性质量、供应商审计及 C3.4/P4/P9 仍需独立验收。见 [`evidence/hermes-quality-sample-2026-09-24.md`](evidence/hermes-quality-sample-2026-09-24.md)。
- 2026-09-24：在正式单节点按运行前冻结的 12 题合成样本扩大 Hermes 回答内容复核。首轮 4 题漏检；`98bd86ca` 修复中文短业务词及会话追问检索，第二轮 12/12 有来源但人工 8/12。`47994193` 约束无依据流程、数值和测试预期并升至 `hermes-v4`，最终人工 11/12、Q07/Q08/Q11 安全隔离 3/3，Q06 仍把未定义阶段排列为固定流程。当前模型配置 1 运行后核对为 `claude-opus-4-6-thinking`；Helm revision 52、6/6 Pod 就绪、临时项目及关联资产清理。定向 67 项和两个文件独立 30/37 项通过，本轮未重跑后端全量。合成题达标不关闭实际业务代表性质量、供应商审计或 C3.4/P4/P9。见 [`evidence/hermes-quality-sample-2026-09-24.md`](evidence/hermes-quality-sample-2026-09-24.md)。
...
- 2026-09-21：C3.4.2 新候选窗口改为无人值守每日 canary。UTC 日结束后采集 2026-09-20，确认 Backend/Worker 均只有 `276/288` 五分钟检查点；虽然该日有 30 次请求、P95 95ms、2 次运行且成功率 100%，主机重启造成的 12 点缺口仍阻断所有分项，所以 9 月 20 日不计入连续窗口。发布机通过正式 API 创建专用全局 `tester` 用户（项目 77 `editor`），随机密码仅保存在 root `0600` 文件并由 systemd `LoadCredential` 映射；仓库增加 service/timer，脚本支持 `ATP_PASSWORD_FILE`/`ATP_TOKEN_FILE` 且拒绝普通宽权限文件。审查实测发现 systemd 受保护凭据文件的模式不能单独作为拒绝依据，现结合 `CREDENTIALS_DIRECTORY` 边界识别；同时修正旧契约测试，不再硬编码直接环境变量实现。手动验证产生 12 次认证读取与 Run 117/118，均 passed；最近 1 小时 HTTP 增量约 21.09、P95 95ms、唯一运行结果为 `case/passed` 且增量约 2.01，Backend/Prometheus 均 200、6/6 Pod Ready、无新增重启。Timer 已启用，下一次为 2026-09-22 08:30 后随机 0～5 分钟；定向 `35 passed / 1 skipped`、后端非集成全量 `2622 passed / 2 skipped`。新连续窗口从 2026-09-21 起重算。证据见 [`slo-history-2026-09-20-2026-09-20.md`](slo-history-2026-09-20-2026-09-20.md) 与 [`evidence/c3-slo-daily-canary-2026-09-21.json`](evidence/c3-slo-daily-canary-2026-09-21.json)。当前日尚未结束，不替代完整 7/14 日、独立 MinIO、P4 或 P9 验收。

- 2026-09-20：启动 C3.4.2 新候选校准窗口。复核目标主机启动时间仍为 12:06:49，本次没有第二次重启；Backend/Prometheus 均为 200、revision 46 deployed、6/6 Pod Ready，重启计数仍为此前主机重启造成的 1。使用既有 `parado` 账号通过隐藏输入运行安全 canary，密码只进入临时进程环境并在结束时清除：先完成 8 次认证读取预检，再由正式入口产生 12 次认证读取；项目 77 的本机单步 GET `case_id=42` 生成 Run 115/116，均 passed，并分别等待 Prometheus 抓取。最近 1 小时 HTTP 外推增量约 21.62、P95 95ms、运行结果唯一为 `case/passed` 且外推增量约 1.49；两份临时报告位于 Git 忽略目录，均标记 `credentials_in_report=false`。脱敏证据见 [`evidence/c3-slo-new-window-day1-canary-2026-09-20.json`](evidence/c3-slo-new-window-day1-canary-2026-09-20.json)。当前 UTC 日尚未结束，这只是新窗口的流量种子，不算完整日或代表性 7/14 天通过。

- 2026-09-20：推进 C3.4.2 首个 7 日窗口。目标主机重启后恢复访问，Backend 与 Prometheus readiness 均为 200，Helm revision 46 保持 deployed，6/6 Pod Ready；各 Pod 的一次 `Unknown/255` 重启与本次 WSL 主机启动一致，发布 Prometheus 容器重启计数为 0。采集 2026-09-13～19 后确认 Backend/Worker 每日均为 `288/288`，但整个窗口只有 9 月 14 日的 34 次请求和 3 次运行可评估，可用性 100%、5m/1h P95 95ms、运行成功率 100%；其余 6 日没有业务样本，产生 24 项 data gap，无业务 breach，初始校准不通过，告警与发布门禁继续 deferred。审查原始报告时发现午夜的 1h/5m range 样本被按时间戳归入次日，造成 9 月 15 日“运行量 0、成功率 100%”的假象；现按右闭区间把午夜边界归回其实际覆盖日，并增加回归，定向 `20 passed`、非集成后端全量 `2618 passed / 1 skipped`。证据见 [`slo-history-2026-09-13-2026-09-19.md`](slo-history-2026-09-13-2026-09-19.md) 与 [`evidence/c3-slo-history-day7-2026-09-20.json`](evidence/c3-slo-history-day7-2026-09-20.json)。该窗口只证明 scrape 连续性，不满足代表性流量、独立 MinIO、P4 或 P9 门禁。
...
- 2026-09-14：完成 H9.5 多副本状态一致性本地切片。`hermes_sessions` 增加 `state_version` 与 Alembic 迁移，SQLAlchemy 更新使用乐观版本检查；会话消息、H7 挂起意图、H8 取消、H4 草稿确认、H9 规划/评测指标及人工反馈的陈旧写入统一回滚并返回稳定 `409`，防止跨副本静默覆盖，草稿确认冲突时同事务创建的计划也会回滚。会话列表和治理最近 500 会话窗口增加稳定次级排序。前端对查询、编排、评价和草稿冲突给出刷新提示，编排冲突不再回退 H1 查询，草稿冲突不重试或误标已确认。代码审查发现并修复编排 `409` 被原回退链吞掉的问题。定向后端 `52 passed`、前端 `2 files / 26 tests passed`；后端非集成全量 `2602 passed / 1 skipped`、前端全量 `76 files / 367 tests passed`，TypeScript、mypy、Ruff、格式和生产构建通过。H9 本地开发阶段收口；当前结果来自两个独立数据库 Session 的并发模拟，不替代目标环境真实多副本部署、角色、模型或 P4/P9 发布验收。

- 2026-09-14：完成 H9.4 失败任务回归范围与套件映射建议。Hermes 同步读取当前项目套件和计划，按失败 Case/Suite/Plan 的来源关系生成最多 8 个活动套件建议，保留命中任务、用例和深链理由，支持逐项编辑并进入草稿 diff；失败用例在用例草稿中优先展示。确认交接到计划页时只预填建议套件并强制默认禁用，仍需用户点击保存；持久会话的二次确认路径由后端重新查询并校验所选套件属于当前项目且未归档，之后只创建 `manual + draft + disabled` 计划，并记录有界失败任务/建议套件元数据，不触发执行。代码审查补齐归档套件过滤。定向后端 `25 passed`、前端 `3 files / 33 tests passed`；后端非集成全量首轮因 Windows 系统临时目录权限产生 55 个 setup error，切换到项目内 `--basetemp` 后 `2599 passed / 1 skipped`；前端全量 `76 files / 363 tests passed`，TypeScript、mypy、Ruff、格式、生产构建和差异检查通过。开发游标进入 H9.5 多副本一致性验证；本地结果不替代目标环境角色或发布验收。

- 2026-09-14：完成 H9.3 可复现质量评测。固定集由 `hermes-core-v1` 五题升级为 `hermes-core-v2` 十题，覆盖单/双只读工具选择、运行/需求/知识目标、项目证据引用、无依据拒答和提示注入拒答；每题公开执行入口、期望工具/来源、引用要求、答案要点和拒答预期。服务端只识别与当前版本题目精确匹配的响应，查询类题目绕过工具模型规划后进入 H1 检索，普通问答、旧版本、未知题号和不适用评分字段不进入分母。评分采用确定性规则：工具集合精确匹配；LLM 的 `[S#]` 必须实际指向期望类型且检索分数大于零的来源；答案覆盖题目要点；`no_results` 与拒答预期一致。结果持久化在 Session 指标中，累计运行次数并按会话/题号保留最新评分，避免 40 条消息裁剪导致历史消失或同会话重复题改变权重。治理 API 和前端新增四项准确率、各项样本数、运行次数及题目覆盖；完成评测后页面自动刷新。自审修复查询评测被 H6 关键词路由截获、消息保留导致指标丢失、重复题加权和相关来源与实际引用编号未绑定的问题。定向后端 `36 passed`、前端 `26 passed`，受影响后端文件独立 `13 passed` 与 `23 passed`；后端非集成全量 `2597 passed / 1 skipped`、前端全量 `76 files / 363 tests passed`，TypeScript、mypy 和生产构建通过。开发游标进入 H9.4 失败任务回归范围与套件映射建议；真实模型阈值和项目评测数据准备仍需目标环境验收。

- 2026-09-14：完成 H9.2 模型规划 usage 与成本治理。每次模型候选规划记录调用次数和耗时，兼容 OpenAI `prompt_tokens/completion_tokens`、Claude `input_tokens/output_tokens` 与 Ollama `prompt_eval_count/eval_count`；严格拒绝负数、非整数和非有限 usage。成本只按项目 AI 配置 `default_params.usage_pricing.hermes_tool_planning` 中显式的每百万输入/输出 Token 单价与币种估算，不内置易漂移的供应商价格；无模型调用、供应商无 usage、未配置价格和部分未计价分别返回稳定原因码。会话聚合规划尝试、调用、Token、平均延迟、回退原因和分币种成本，治理面板展示统计及前三项回退原因。自审修复未命中规划写指标时覆盖原 `conversation_id`、导致 H7 挂起意图无法恢复的问题，并限制异常历史数据下回退总数不超过规划尝试数。定向后端 `32 passed`、前端 `26 passed`，受影响后端文件独立 `21 passed` 与 `11 passed`；后端非集成全量 `2593 passed / 1 skipped`、前端全量 `76 files / 363 tests passed`，TypeScript、生产构建、Ruff、格式和 mypy 通过。开发游标进入 H9.3 质量评测扩展；真实供应商 usage/价格准确性仍需目标环境验收。

- 2026-09-14：完成 H9.1 模型辅助只读规划。确定性 H6～H8 保持优先，只有规则未命中的普通问题才调用项目绑定模型；取消和挂起参数补全不增加模型调用。新增版本化规划提示词和工具目录，只接受最多两个固定只读工具，服务端重新执行 Pydantic 参数 Schema 并整体拒绝未知工具、额外字段、无效编号、重复工具和超步数计划。API 返回并持久化规划来源、校验状态、模型名、提示词版本和通用回退原因，Hermes 会话展示规划来源、校验通过状态及每步理由。自审补强空计划外层额外字段校验；模型不可用、限额、异常或策略拒绝时继续走原确定性/项目检索链路。定向后端 `31 passed`、前端 `24 passed`，后端非集成全量 `2592 passed / 1 skipped`、前端全量 `76 files / 363 tests passed`，TypeScript、mypy 和生产构建通过；H9 开发游标进入 H9.2 usage、延迟、成本和回退治理。
