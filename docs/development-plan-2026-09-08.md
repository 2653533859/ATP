...
- [x] `H9.5` 为持久会话增加数据库乐观版本；消息、挂起意图、取消、草稿与治理指标的陈旧更新统一回滚并返回 `409`，前端显式提示刷新且不降级为其他请求或重复确认。治理聚合窗口按更新时间和 ID 稳定排序。本地以两个独立数据库 Session 模拟副本并验证无静默覆盖；目标环境真实多副本部署冒烟保留为环境验收。
