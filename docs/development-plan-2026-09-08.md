...
- [ ] `B1.3` 使用受控管理员、工程师、Viewer 账号和五域真实任务执行 Linux Backend 验收，并在 Windows 本地前端验证项目切换、刷新、深链、侧栏折叠和窄屏。
- [ ] `B1.4` 验证五类任务轮询、重试、停止、批量操作、过期确认和执行后状态收敛，绑定同一提交 SHA 保存证据。

执行说明见 [`docs/b1-workbench-role-matrix.md`](b1-workbench-role-matrix.md)。B1 当前仍为 `[E]`：本地契约和验收入口完成，真实三角色与真实任务证据尚未执行。

B1.3 前置自动化已覆盖任务中心从 URL 恢复项目、状态、任务类型与有界页码，Viewer 操作标记不渲染重试/停止，以及只执行服务端明确授权的操作。真实浏览器检查仍须在部署本轮代码后使用三套独立账号执行。
