...
> 状态：H1～H9.5 本地开发及目标同节点双 Backend 一致性验收已完成；K3s 临时项目的真实模型规划、单条合成需求回答、有界 Viewer 权限与两类受控回退已验收；完整质量、超时/限额、多节点故障域与发布门禁仍待独立验收
...
- [x] K3s 临时配置验证无引用响应及连接失败时查询退回项目检索摘要，连接失败时规划返回 `model_call_failed`；超时和限额仍待验证。见[受控回退记录](evidence/hermes-k3s-answer-fallback-cjk-2026-09-24.md)。
