...
> 版本：H9.5 加固与目标环境有界验收 / 2026-09-25
> 状态：H1～H9.5 本地开发及目标同节点双 Backend 一致性验收已完成；正式单节点 revision 55 上合成回答 12/12、五条真实用例元数据 8/8、项目 Viewer/管理员九项 9/9、全局 Engineer + 项目 Editor 十项 10/10。完整业务内容、供应商侧审计、多节点故障域与发布门禁仍待独立验收；Flower 一次 OOM 重启须纳入 C3.4 排查
...
- [x] 目标 K3s 使用已有启用的 AI 模型完成一条合成需求的真实问答，并核对来源引用；见[回答验收记录](evidence/hermes-k3s-answer-fallback-cjk-2026-09-24.md)。
- [x] 在临时 K3s Backend 连接的独立夹具库核对固定编号资产，以隔离 Viewer 运行 `2026-09-23.1` 十题和独立模型规划挑战：修复前后均为 10/10 与规划通过，实际模型生成题为 2 道；见[验收记录](evidence/hermes-k3s-fixed-ten-prompt-v3-2026-09-24.md)。
- [ ] 预先定义代表性真实回答的质量阈值、扩大样本并复核失败题；固定十题结构评分和两条人工抽查不足以关闭此项。
- [x] 正式 K3s 以运行前冻结的 12 题合成样本完成有界扩样：`hermes-v4` 人工 11/12、Q07/Q08/Q11 安全隔离 3/3、会话审计链完整；Q06 仍有无来源的固定流程顺序，实际业务代表性样本仍按上一条待验。见[扩样记录](evidence/hermes-quality-sample-2026-09-24.md)。
- [x] `hermes-v6` 在同一冻结 12 题上人工 12/12、安全隔离 3/3；`hermes-v5` 虽修正 Q06，却在别题退化为 8/12，因此保留失败轮证据。当前结论仅关闭合成题有界质量，实际业务代表性样本仍待独立验收。见[续验记录](evidence/hermes-quality-sample-2026-09-24.md)。
- [ ] 真实模型异常、超时、限额及无有效 `[S#]` 引用场景仍能回退规则结果。
- [x] K3s 临时配置验证无引用响应及连接失败时查询退回项目检索摘要，连接失败时规划返回 `model_call_failed`。见[受控回退记录](evidence/hermes-k3s-answer-fallback-cjk-2026-09-24.md)。
- [x] 临时 K3s Backend 的本机模型桩验证查询 60 秒超时、每日查询限额和规划限额均安全回退；见[异常报告](evidence/hermes-k3s-timeout-quota-2026-09-24.json)。真实供应商故障仍待单独验证。
- [ ] 管理员和 viewer 完成跨项目读写隔离验证。
- [ ] 供应商请求、回答、审计日志中没有 API Key、Token、Cookie 或敏感正文。
- [x] K3s 临时项目通过正式 API 绑定已启用模型，真实规划选出两步有证据的只读工具；Viewer 读、写拒绝和跨项目拒绝及审计经过有界验证，临时项目已清理。见[验收记录](evidence/hermes-k3s-real-model-acceptance-2026-09-24.md)。
...
### 2026-09-25 / Engineer 与项目 Editor 目标环境验收

- 冻结十项中全局 Engineer 与 A 项目 Editor 完成真实模型回答、禁用手工计划草稿、Owner/跨项目拒绝、跨用户确认、降权、撤销和停用链路，**10/10 通过**。初次准备因需求 `source` 超长停止，项目已清理；复跑复用同一临时账号，项目、需求、计划与会话清理，账号经正式 API 停用并保留为审计身份。Helm revision 55 四次 6/6 Pod Ready、健康 200；Flower 在本轮前发生一次 `OOMKilled`，总重启 1，不声明稳定性门禁通过。见[脱敏证据](evidence/hermes-engineer-editor-2026-09-25.md)。

### 2026-09-25 / 目标环境项目角色与审计边界

- 事前冻结九项，在两个临时项目中以管理员和现有验收账号的项目 Viewer 身份复核真实模型来源、只读工具、越权写入、跨项目读取、跨用户反馈、幂等反馈与成员撤销，**9/9 通过**。A 的只读工具审计 1 条、拒绝审计 3 条，B 的拒绝审计 5 条；临时项目、需求及会话清理，审计留存。正式单节点 Helm revision 55 四次 6/6 Pod Ready、健康 200、零重启，项目 77 保持未绑定模型。未覆盖全局 Engineer/项目 Editor、供应商账单与完整业务内容，见[脱敏证据](evidence/hermes-role-audit-2026-09-25.md)。

### 2026-09-24 / 真实项目用例元数据验收

- 从项目 77 五条真实用例经 API 复核指纹，仅复制七个元数据字段到临时项目。运行前冻结的八题首轮人工 3/8，暴露用例类型、优先级、级别未进入来源摘录及泛指用例问题漏检。`a49d0734` 修复后原题复测人工 8/8、无执行证据题 2/2、审计完整；定向 71 项和单文件 38/33 项通过。正式单节点 Helm revision 55、四次 6/6 Pod Ready/健康 200/零重启，临时项目清理，项目 77 不绑定模型。该验收只覆盖真实用例元数据，业务需求、知识、步骤与运行证据扩样另行冻结，供应商侧审计及 P4/P9 继续待办。见[脱敏证据](evidence/hermes-real-case-metadata-2026-09-24.md)。

### 2026-09-24 / Q06 修复与短答约束复测

- `77cb2d18` 修复 Q06 固定流程顺序，但 `hermes-v5` 完整复测发现另外四题超出来源，人工 8/12、安全 2/3。`de0d485c` 将回答范围收紧到所问结论、直接证据和必要未知项；相同冻结 12 题人工 12/12、安全 3/3、会话审计链完整。正式 K3s Helm revision 54、6/6 Pod 就绪、临时资产清理。定向 `69 passed`；实际业务项目 77 只有 17 条用例、无需求/知识且未绑定模型，仍需另建真实业务样本判据。见[脱敏证据](evidence/hermes-quality-sample-2026-09-24.md)。

### 2026-09-24 / 回答质量合成扩样与检索修复

- 运行前冻结两临时项目 12 题与独立事实判据。首轮 4 题 `no_results`；`98bd86ca` 修复中文短业务词检索和同会话指代追问，次轮 12/12 有来源但人工仅 8/12。`47994193` 升至 `hermes-v4` 并约束无来源流程/数值，最终人工 11/12、Q07/Q08/Q11 安全隔离 3/3；Q06 仍把未定义流程写成固定顺序。正式单节点 Helm revision 52、六个 Pod 就绪、临时资产已清理。配置 1 在运行后核对为 `claude-opus-4-6-thinking`；本轮是合成样本，不关闭实际业务代表性回答、供应商审计或发布门禁。见[脱敏证据](evidence/hermes-quality-sample-2026-09-24.md)。

### 2026-09-24 / K3s 十题、内容范围修复与受控异常

- 临时 K3s Backend 复用独立数据库夹具，隔离 Viewer 固定十题 10/10、模型规划挑战通过；人工审查发现结构评分未识别“检索片段未显示”被写成“项目缺少用例”的内容错误。提交 `fcf2c507` 收紧提示词并升至 `hermes-v3`，再次十题 10/10，两个生成回答均明确证据范围并先建议查询现有记录。定向 `63 passed`、独立 `27 passed`、非集成全量 `2665 passed / 2 skipped`。本机桩的 60 秒超时、查询及规划限额回退通过，临时项目、配置、配额键和命名空间均清理。
- 正式 K3s Backend 升至 Helm revision 49；一条临时合成需求真实模型冒烟获得 `hermes-v3`、正确来源与有效引用，4 次采样六个 Pod 就绪、Backend 健康。固定十题使用的是临时 Pod 和独立夹具库，不等于正式业务库项目十题通过。见[脱敏证据](evidence/hermes-k3s-fixed-ten-prompt-v3-2026-09-24.md)。

### 2026-09-24 / K3s 回答、回退与中文检索

- 一条合成登录需求的固定短问句获得真实模型 `llm_grounded` 回答和有效 `[S1]`；人工核对了锁定阈值、时长和通知方式缺口。受控桩返回无引用正文、模型本机连接失败时，查询均回退检索摘要；连接失败的候选规划返回 `model_call_failed`。这些结果不等于固定十题或供应商异常验收。
- 长中文问句最初漏掉同名需求并返回 `no_results`。提交 `9f161362` 增加有界四字片段兜底检索，后端全量 `2664 passed / 2 skipped`；单节点 Helm revision 48 部署后，原长问句获得一条正确项目来源和有效 `[S1]`，无关问题仍为 `no_results`。临时项目与模型配置均清理。见[脱敏证据](evidence/hermes-k3s-answer-fallback-cjk-2026-09-24.md)。

### 2026-09-24 / K3s 真实模型规划与权限

- 在临时项目 84 绑定现有已启用配置，先做一次空项目规划，再准备最小自回环失败运行并再次规划；第二次模型调用选出 `failed_tasks` 和 `quality_trend`，两步工具均返回 `ok` 和证据，Token 用量已记录，成本没有可用估算。
- 临时 Viewer 的只读访问成功，写入与跨项目读取均返回 403，拒绝审计持久化；成员及项目、用例、运行已通过正式 API 清理，原项目 77 的模型绑定保持为空。完整固定十题与模型回答质量仍以独立环境证据为界，K3s 不补造固定编号。见[脱敏记录](evidence/hermes-k3s-real-model-acceptance-2026-09-24.md)。
