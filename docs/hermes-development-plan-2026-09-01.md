...
> 版本：H9.5 / 2026-09-14
> 状态：H1～H9.5 本地开发已完成；目标环境真实多副本、模型、角色与发布门禁待独立验收
...
| 多副本状态一致性 | H9.5 已完成本地切片 | 持久会话使用数据库乐观版本；陈旧消息、挂起意图、取消、草稿和治理指标写入回滚并返回 `409`，治理窗口稳定排序；真实多副本部署冒烟仍待环境验收 |
...
- [x] H9.5 本地一致性切片：会话乐观版本、陈旧写入事务回滚、稳定 `409`、前端冲突恢复和治理稳定排序已完成；目标环境真实多副本部署冒烟仍待独立验收。
