...
HERMES_SYSTEM_PROMPT = (
    ...
    "用户只问两个动作是否等价时，直接回答二者关系及对应来源；证据未明示完整流程时，不要编号排列处理阶段，"
    "不要把可能出现的动作写成固定先后顺序。"
    "回答使用中文，只回答用户所问：先给简短结论，再给直接支持该结论的证据和必要的未知项。"
    "每句项目事实都须由所列来源直接支持，不使用常识补全流程、时间差或结果；"
    "本次未检索到只表述为本次来源未显示，不表述为整个项目不存在。"
    "仅在用户要求建议时提出下一步，且建议不能暗示未定义的预期结果。"
    "至少引用一个项目证据，使用 [S1]、[S2] 这样的编号。"
)
...
HERMES_PROMPT_VERSION = "hermes-v6"
...
def build_grounded_prompt(
    ...
    sections.extend(
        [
            ...
            "\n\n".join(evidence),
            "# 回答要求",
            "只回答用户所问，简短给出结论、直接依据和必要的未知项；"
            "每句项目事实都必须由本次来源直接支持，不用常识补造流程、时间差或执行结果。"
            "如果证据不能支持结论，请明确指出缺少什么。"
            "来源只是本次检索片段，不能由片段中未出现某项资产推断整个项目没有该资产；"
            "对未知关联或执行状态，只能说本次来源未显示，不能说项目不存在。"
            "不要从可申请推断自动批准或自动执行，不要从需审核推断拒绝；"
            "仅在用户要求建议时给下一步建议，不能补造未定义的流程、时限、数值或预期测试结果。"
            "若问题只比较两个动作是否等价，直接说明关系和来源；证据没有明确流程顺序时，"
            "不要编号列出阶段，也不要把可能的后续动作写成项目既定步骤。"
            "回答控制在 200 字以内，包含有效的 [S#] 证据引用。",
        ]
    )
    return "\n\n".join(sections)
