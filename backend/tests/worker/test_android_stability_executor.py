...
def test_start_app_uses_launcher_or_explicit_activity(monkeypatch):
    ...
    monkeypatch.setattr(android_stability_executor, "launch_android_app", fake_launch)
    ...
    assert android_stability_executor._start_app("emu-5554", "com.example.app", ".WelcomeActivity") is True
    assert calls[-1] == ("emu-5554", "com.example.app", ".WelcomeActivity")

    monkeypatch.setattr(
        android_stability_executor,
        "launch_android_app",
        lambda *a, **kw: (_ for _ in ()).throw(android_stability_executor.AndroidPreflightError("failed")),
    )
