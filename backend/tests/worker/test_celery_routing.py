...
def test_celery_task_routes_split_operational_queues():
    ...
    assert routes["check_mobile_special_schedules"]["queue"] == "maintenance"
    assert routes["cleanup_stale_mobile_special_runs"]["queue"] == "maintenance"
    assert routes["reclaim_expired_device_leases"]["queue"] == "maintenance"
    assert routes["dispatch_android_device_scan"]["queue"] == "maintenance"
    assert routes["scan_adb_devices"]["queue"] == "android"
    assert routes["heartbeat_android_worker"]["queue"] == "android"
    assert routes["run_android_device_operation"]["queue"] == "android"
