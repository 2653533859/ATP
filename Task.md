...
- [x] N6 数据集影响范围项目上下文：数据集影响范围的套件/计划入口保留当前 `project_id`，避免跳转后丢失项目上下文；数据集定向 `9 passed`、前端全量 `69 files / 299 tests passed`，`vue-tsc`、生产构建和独立审查通过。真实项目角色、跨项目可见性和可清理数据仍待 N6 复核。
