...
- [~] C1 单节点 K3s 性能与可观测性：C1.1 已新增可重复运行的有界采样器，统一采集节点/Pod 资源、Ready/重启/Pressure、Backend/Worker/Performance Worker Prometheus 指标并对缺失项失败收口；提交 `67ef34f4` 在当前 K3s 连续 40 秒取得 5 个采样点，均为 1 节点 Ready、6/6 Pod Ready、三指标端点覆盖且 0 告警。下一步 C1.2 执行短压、取消、Threshold、基线、报告和对象清理；发布级 Prometheus/P4 仍为独立门禁，多节点高可用及跨主机 MinIO 灾备保持 `[OUT]`。
