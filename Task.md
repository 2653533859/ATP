...
> 当前有效顺序（2026-08-25 2.3.0）：参考导航按“工作台、测试能力、测试资产、智能中枢、系统”五组落地；先复核 N0 导航壳与 N1 任务中心，再按 N2-API、N2-APP、N3-UI、N4 性能、N5 AI、N6 测试资产、N7 智能中枢和 N8 系统治理/ N9 发布收口推进。N4 因缺少 Kubernetes、发布级 Prometheus 指标覆盖和独立 MinIO 保持阻塞；N2～N3、N5～N8 本地实现保持 `[E]`，不得把页面可打开或单节点 Compose 写成真实通过。详细模块边界、依赖和验收出口以开发计划 2.3.0 为准，下方 2.2.0 及历史记录不覆盖当前执行版。

## 2026-08-25 当前开发计划 2.3.0 跟踪摘要

- [x] 导航边界登记：五组分工、旧 URL 兼容、项目上下文和系统管理收敛规则已记录。
- [ ] N0/N1 工作台与任务中心：复核深链、待办、轮询、重试、终止、批量操作、失败事件和越权边界。
- [ ] N2-API、N2-APP、N3-UI 测试能力工作台：按配置→执行→过程→报告/证据顺序复核，真实协议/Worker/浏览器证据独立记录。
- [ ] N4 性能测试工作台：等待 Kubernetes、发布级 Prometheus、独立 MinIO source/target；保持 `[-]`，不以 mock、单节点 Compose 或跳过项替代。
- [ ] N5～N7 AI、测试资产和智能中枢：准备真实模型、临时项目、角色矩阵、需求/知识数据，验证来源、审计、项目隔离和可清理性。
...
- [x] N7 Hermes 跨任务失败诊断本地闭环：case 复用原诊断链，suite/plan/android/performance 通过统一工作台入口读取执行摘要、Android 异常事件/错误事件和性能指标生成规则诊断；后端定向 `13 passed`、后端非集成全量 `2345 passed`，前端全量 `69 files / 293 tests passed`，新增服务 mypy、Ruff、格式、`vue-tsc` 和生产构建通过。真实模型、需求/知识数据和角色矩阵仍待验收。
...
- [x] N6 项目资产与角色矩阵验收工具：新增临时项目全链路脚本，覆盖用例评审、套件/计划、可选真实执行/报告、缺陷关联、普通 viewer 只读/写入拒绝和删除后 404 清理；凭据仅环境变量、HTTP 错误脱敏、`--allow-mutations`/`--execute` 显式授权；脚本定向 `6 passed`、质量门禁一致性 `10 passed`，Makefile/CI/pre-commit/运行手册已同步。真实部署验收仍待受控账号与可清理运行数据。
- [~] N8/N9 系统治理与发布收口：汇总脱敏证据、配置差异、回滚边界、操作手册和最终提交 SHA。
- [ ] 每个模块必须完成“实现/调整 → 测试 → 独立审查 → 修复 → 文档与记忆同步 → 提交推送”后才能移动游标。

详细拆分、依赖和状态口径见 [`docs/development-plan-2026-08-25.md`](docs/development-plan-2026-08-25.md) 的 2.3.0；2.2.0 以下内容仅作历史记录。
