...
开发计划和单模块交付规则已记录在 [`docs/product-navigation-roadmap-2026-08-24.md`](docs/product-navigation-roadmap-2026-08-24.md)，本节只维护阶段状态；导航已按参考方案固定为五组，N0 导航壳、N1 工作台与任务中心、N2.1/N2.2/N2.3/N2.4/N2.5、N3.1 API、N3.2 APP、N3.3 UI、N3.4 性能、N4.1 AI 智能测试工作台、N4.2 Hermes 助手、N4.3 需求与用例追踪、N4.4 知识中枢、N5.1 远程工具箱和 N5.2.1～N5.2.6 配置中心本地实现、审查和回归已完成，均待真实环境验收。
...
- [~] N6 质量收口：回归、E2E、迁移、文档、外部环境证据和发布检查；N6.1 配置中心浏览器回归已完成本地实现、审查和门禁，继续推进文档/能力矩阵与真实环境证据。

当前推荐下一步：继续 N6.2 发布文档、能力矩阵和真实环境证据收口。N5.2 已统一聚合启动配置、环境、全局变量、AI、存储、通知和性能配置，N6.1 已补齐配置中心管理员回退链与普通角色路由拦截的浏览器回归；N5.1 及此前模块的真实环境验收作为并行环境证据补齐。每个子模块完成后先审查和修复，再同步文档并提交推送。
...
- **验证与边界**：文档链接和状态审查通过，`git diff --check` 通过。真实数据库、生产密钥、Android/Windows Worker、性能节点、通知和外部缺陷平台仍需按环境生成带日期证据，下一入口为 N6.2 真实环境与发布 readiness 验收。
