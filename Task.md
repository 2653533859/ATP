...
- [~] C3 迁移、备份恢复、SLO、证据索引和最终 SHA 发布收口：C3.1～C3.3 与 C3.4.1 已完成。C3.4.2 已增加完整 UTC 日 SLO 采集门禁，并新增 `make slo-traffic-canary` 安全流量入口：默认只做有界认证读；只有显式给出项目、API 用例和 `--confirm-case-run` 才运行，且服务端元数据必须满足 active/approved/可自动执行/单步 GET/本机 HTTP 回环。真实入口以 4 次读请求及 Run 113/114 验证通过，每次运行后等待 Prometheus 抓取周期；定向与契约 `23 passed`、非集成后端全量 `2615 passed / 1 skipped`。2026-09-13 首个完整 UTC 日 Backend/Worker 均为 288/288，但无业务样本；后续 canary 已确认 HTTP、P95 与 `case/passed` 指标链可增长，仍须等待有代表性流量的 7/14 天完整历史。候选异机 `172.31.27.133` 的 VPN/路由与服务仍不可用，未部署 MinIO。之后进入 C3.5 最终 SHA/证据索引。证据见 [`docs/evidence/c3-slo-traffic-canary-tool-2026-09-15.json`](docs/evidence/c3-slo-traffic-canary-tool-2026-09-15.json)。
