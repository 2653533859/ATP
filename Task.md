...
## 2026-08-24 产品导航与能力扩展路线（规划）

开发计划已单独记录在 [`docs/product-navigation-roadmap-2026-08-24.md`](docs/product-navigation-roadmap-2026-08-24.md)，本节只维护阶段状态；N0 基础导航已实现，项目选择器和动态徽标数据待 N1 工作台聚合。
...
- [~] N0 导航壳与信息架构：五层分组、路由注册、项目上下文基础、权限、占位入口和旧 URL 兼容已完成；项目选择器、面包屑和动态徽标数据待 N1 聚合。
- [ ] N1 工作台与任务中心：待办聚合、统一任务队列、重试、终止、批量执行、失败重跑和定时任务。
- [ ] N2 测试资产闭环：内部缺陷、报告中心和独立用例评审入口。
...
当前推荐下一步：进入 N1 工作台与任务中心，优先接入待办聚合、项目选择器和任务状态徽标，不在导航重构阶段重复开发已有业务能力。
