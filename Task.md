...
> 当前有效顺序（2026-08-25 2.1.0）：导航已统一为“工作台、测试能力、测试资产、智能中枢、系统”五组；N2 Karing 单设备闭环已通过，当前执行游标转为 N4 真实性能环境。N4 因缺少 Kubernetes/Prometheus/独立 MinIO 保持阻塞，随后推进 N5-N8 外部依赖复核和 N9 发布收口。详细阶段、验收出口和状态口径以开发计划 2.1.0 为准，下方更早的历史记录不覆盖当前执行版。
...
- [ ] N9 发布收口：绑定最终提交 SHA，汇总测试、审查、环境证据、操作手册和剩余阻塞，当前为 `[~]`。
