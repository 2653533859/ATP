...
- [~] C3 迁移、备份恢复、SLO、证据索引和最终 SHA 发布收口：C3.1～C3.3 与 C3.4.1 已完成。C3.4.2 已取得 2026-09-13～19 连续 7 个完整 UTC 日的 Backend/Worker `288/288` 抓取，但只有 9 月 14 日存在 34 次有界合成请求和 3 次运行，其余 6 日没有业务样本；初始校准因此未通过。采集器已修复午夜 range 样本跨日污染，定向 `20 passed`、非集成后端全量 `2618 passed / 1 skipped`。2026-09-20 已启动新候选窗口：`parado` 账号先完成 8 次只读预检，再由正式入口产生 12 次认证读取，Run 115/116 均 passed；Prometheus 观察到 HTTP、95ms P95 和 `case/passed` 增量，凭据未进入报告。须在 UTC 日结束后确认首日完整证据，并继续每日产生受控流量直至 7/14 天；候选异机 `172.31.27.133` 尚不可用于独立 MinIO，之后才能进入 C3.5。证据见 [`docs/evidence/c3-slo-new-window-day1-canary-2026-09-20.json`](docs/evidence/c3-slo-new-window-day1-canary-2026-09-20.json)。
