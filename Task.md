...
- [~] C3 迁移、备份恢复、SLO、证据索引和最终 SHA 发布收口：C3.1～C3.3 与 C3.4.1 已完成。C3.4.2 已增加可独立运行的 SLO-only 采集、每天 288 个五分钟连续性检查点、`NaN`/无穷值过滤及缺口门禁；采集器会拒绝未结束的当前/未来 UTC 日，并分别判定 Backend 与 Worker 抓取连续性。2026-09-13 首个完整 UTC 日两者均为 288/288，但请求量为 0，SLO 仍不可评估；随后以受控账号执行 37 次认证只读请求和两次自回环 API canary，Run 111/112 均 passed，Prometheus 跨抓取周期确认 HTTP、P95 与 `case/passed` 运行结果序列增长，证明三条 SLO 数据链可用，但当前 UTC 日未结束，不能计入完整日门禁。候选异机 `172.31.27.133` 从 Windows 仅经 Karing TUN 建连后于 SSH 握手前重置，MinIO 健康检查无有效响应；发布主机有默认网关路由但 ICMP 与 22/9000/5432/6379 均超时，未部署 MinIO。继续等待有代表性流量的 7/14 天历史与可用的不同故障域 MinIO，之后进入 C3.5 最终 SHA/证据索引。证据见 [`docs/evidence/c3-slo-traffic-canary-2026-09-15.json`](docs/evidence/c3-slo-traffic-canary-2026-09-15.json)。
