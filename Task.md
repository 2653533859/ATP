...
开发计划和单模块交付规则已记录在 [`docs/product-navigation-roadmap-2026-08-24.md`](docs/product-navigation-roadmap-2026-08-24.md)，本节只维护阶段状态；导航已按参考方案固定为五组，N0 导航壳、N1 工作台与任务中心、N2.1/N2.2/N2.3/N2.4 本地实现和审查已完成，均待真实环境验收。
...
- [~] N2 测试资产闭环：按 N2.1 内部缺陷 → N2.2 失败证据 → N2.3 报告中心 → N2.4 用例评审 → N2.5 外部缺陷兼容推进，N2.1/N2.2/N2.3/N2.4 已完成本地实现，当前开发入口为 N2.5。
...
当前推荐下一步：实现 N2.5 外部缺陷兼容，复用现有 Jira/禅道/GitHub/GitLab 链路，补齐内部缺陷与外部 Issue 的映射和同步边界；N1/N2.1/N2.2/N2.3/N2.4 真实环境验收作为并行环境证据补齐，不阻塞本地模块开发。每个模块完成后先审查和修复，再同步文档并提交推送。
