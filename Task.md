...
- [~] C3 迁移、备份恢复、SLO、证据索引和最终 SHA 发布收口：C3.1 已完成最小权限凭据代码通道，运行期 PostgreSQL 与 Alembic 迁移账号可分离，Redis 支持 ACL 用户名，MinIO 支持 bucket 级应用凭据；Helm 迁移凭据仅注入 hook Job，不下发到长期运行 Pod。目标 Linux 尚未创建和切换受限角色，C3 仍保持进行中。
