...
- [E] 计划检查点 2.4.46：JMeter 单节点短任务、取消/恢复及 JSON 报告通过；Linux 取消路径改为独立 session/进程组终止，93 项性能服务回归通过、1 项 Linux 专属用例在 Windows 跳过，另有 Linux 真实子进程验证通过。单文件 Worker 补丁已部署至 Helm revision 7，复验无存活 Java 负载进程，但仍有 1 个僵尸记录，下一项优先补齐 PID 1 回收。2 个临时项目、6 条运行和 6 个 MinIO 对象已清理；不算完整进程清理或 P4/P9 通过，详见开发计划 2.4.46。
