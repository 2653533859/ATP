...
- [-] P4 性能测试工作台：等待 Kubernetes、发布级 Prometheus、独立 MinIO source/target；不以 mock、单节点 Compose 或跳过项替代。
...
- [~] Linux 后端稳定性：发现后端容器启动时曾因 Compose 内 `postgres` 服务名临时 DNS 解析失败而重启；当前重试后已恢复健康，仍需补齐依赖就绪检查/启动重试并完成持续稳定性复核。
