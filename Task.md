...
> 当前有效顺序：先推进 N1 的 GraphQL/WebSocket/流式 gRPC 与完整报告真实证据；Android Karing 因设备未确认真实包名保持阻塞，但不阻塞 N1；随后推进性能真实环境、通知、外部缺陷平台和发布收口。下方更早的历史记录不覆盖当前执行版。
...
- `[E]` N1 接口测试：HTTP、会话复用、gRPC TLS Unary、OpenAPI/Postman 导入预览/落库已通过；GraphQL、WebSocket、流式 gRPC 和完整报告仍待环境验收。
...
当前执行游标为 **N1 其他协议/完整报告 → N4～N9 外部收口**；N2 Karing 真机门禁并行保持阻塞，N0/N3 Windows 复核已由本轮完整 smoke 关闭。每个游标项都要完成实现、测试、代码审查、修复、文档/记忆同步和提交推送后再移动。

## 2026-08-25 当前开发计划登记

- [~] **N1 接口测试**：下一模块是可控 GraphQL、WebSocket、流式 gRPC 和完整报告真实验收；已有 HTTP、会话复用、gRPC TLS Unary、OpenAPI/Postman 导入预览/落库证据保持有效。
