...
> 当前有效顺序：N1 受控协议与完整报告闭环已完成，下一步推进 N4 性能真实环境；Android Karing 因设备未确认真实包名保持阻塞，但不阻塞 N4；随后推进通知、外部缺陷平台和发布收口。下方更早的历史记录不覆盖当前执行版。
...
- `[~]` N4 性能测试：本地与 q19 短压链路已完成，性能验收脚本已支持 Kubernetes 多节点/副本/Worker 资源预检，并新增跨端点 MinIO 复制/恢复 smoke；真实多节点、Prometheus/MinIO 生命周期和跨主机恢复证据仍待目标环境。
- `[E]` N5 AI 智能测试、N6 测试资产、N7 智能中枢、N8 系统：本地实现已完成，真实模型、项目数据、目标部署和外部平台边界仍需单独验收。
- `[~]` N9 发布收口：依赖 N2、N4 和真实外部服务；未完成项必须保留阻塞原因与复验命令。

当前执行游标为 **N4～N9 外部收口**；N2 Karing 真机门禁并行保持阻塞，N0/N1/N3 Windows 复核已由本轮证据关闭。每个游标项都要完成实现、测试、代码审查、修复、文档/记忆同步和提交推送后再移动。
