...
> 当前有效顺序（2026-08-25 2.0）：导航已统一为“工作台、测试能力、测试资产、智能中枢、系统”五组；N4 真实性能环境仍等待 Kubernetes/Prometheus/独立 MinIO，N2 的 Karing 包名、启动入口、Windows Worker、设备扫描、低代码和录屏回传已通过，下一步进入异常回放、专项任务及事件/报告闭环，随后推进 N5-N8 外部依赖复核和 N9 发布收口。详细阶段、验收出口和状态口径以开发计划 2.0 为准，下方更早的历史记录不覆盖当前执行版。
...
当前执行游标为 **N4 真实环境验收（外部目标等待） → N2 Karing 单设备解阻 → N5-N8 外部依赖复核 → N9 发布收口**；N2 已完成入口兼容本地补强，但真机门禁仍保持阻塞，N0/N1/N3 Windows 复核已由本轮证据关闭。每个游标项都要完成实现、测试、代码审查、修复、文档/记忆同步和提交推送后再移动。
...
- [-] **N2 APP 自动化**：等待 Karing APK 或设备中确认出的真实包名；解除后按单设备低代码、录屏/异常回放、专项任务、事件/日志/报告和清理顺序执行。
...
后续执行顺序固定为：N4 真实性能环境 → N2 Karing 单设备解阻 → N5-N8 外部依赖复核 → N9 发布收口。每个模块均执行“实现/调整 → 定向与全量测试 → 代码审查 → 修复 → 文档/记忆同步 → Conventional Commit 推送”；`[E]` 不等于真实环境通过，`offline`、mock 和跳过项不得关闭验收门禁。
