...
开发计划和单模块交付规则已记录在 [`docs/product-navigation-roadmap-2026-08-24.md`](docs/product-navigation-roadmap-2026-08-24.md)，本节只维护阶段状态；导航已按参考方案固定为五组，N0 导航壳、N1 工作台与任务中心、N2.1/N2.2/N2.3/N2.4/N2.5、N3.1 API、N3.2 APP、N3.3 UI、N3.4 性能、N4.1 AI 智能测试工作台、N4.2 Hermes 助手、N4.3 需求与用例追踪、N4.4 知识中枢和 N5.1 远程工具箱本地实现、审查和回归已完成，均待真实环境验收。
...
- [~] N0 导航壳与信息架构：五层分组、路由注册、项目上下文基础、权限、占位入口和旧 URL 兼容已完成；动态徽标与面包屑继续由工作台能力承接。
...
- [~] N5.2 配置中心：N5.2.1 只读聚合已完成本地实现；继续按“版本快照/审计 → 差异/影响提示 → 单资源回滚 → 前端统一入口”交付。
- [ ] N6 质量收口：回归、E2E、迁移、文档、外部环境证据和发布检查。

当前推荐下一步：实施 N5.2.2 版本快照与变更审计。N5.2 统一聚合启动配置、环境、全局变量、AI、存储、通知和性能配置，补齐配置差异、变更审计、影响提示和可控回滚；N5.1 及此前模块的真实环境验收作为并行环境证据补齐。每个子模块完成后先审查和修复，再同步文档并提交推送。

### 2026-08-24 N5.2 配置中心开发计划（文档跟踪）

目标：把当前分散的运行配置收敛到一个“配置中心”入口，提供可追踪、可比较、可回退的配置治理能力；不重复实现各配置页面已有的编辑逻辑，不通过浏览器直接重启服务。

- [ ] **N5.2.1 配置只读聚合与权限边界**：统一展示启动配置、环境、全局变量、AI 模型、存储策略、通知和性能节点；支持项目筛选、配置域筛选、当前状态和原页面深链接。摘要只返回脱敏字段，启动配置标为运行时只读；管理员/工程师可进入，具体编辑/查看权限沿用现有接口边界。
- [ ] **N5.2.2 版本快照与变更审计**：新增配置版本模型和 Alembic 迁移；按单个配置资源保存加密原始快照、脱敏快照、指纹、创建人、原因和时间；写入统一审计日志，禁止把密码、Token、密钥、连接串或原始异常写入响应、日志和文档。
- [ ] **N5.2.3 配置差异与影响提示**：支持当前配置与任意历史快照的字段级差异，敏感字段只显示“已变更/未变更”；根据配置域提示影响范围（例如环境变量影响项目执行、AI 配置影响生成、通知配置影响投递、性能节点影响调度），不把提示当成自动执行。
- [ ] **N5.2.4 单资源可控回滚**：只允许回滚同一资源的历史快照，执行前要求明确确认、再次校验权限和资源存在性；在事务内恢复并写审计，失败自动回滚；启动配置只提供跳转和操作说明，禁止在线接口伪造进程重启或批量删除重建配置。
- [ ] **N5.2.5 配置中心前端工作台**：新增 `/system/config` 统一入口，采用配置域卡片、资源列表、变更时间线、差异面板和回滚确认抽屉；保留现有环境/变量/AI/存储/通知/性能/启动配置入口，加载失败、无权限、无历史和敏感字段状态均有明确反馈，并兼容窄屏与键盘焦点。
- [ ] **N5.2.6 代码审查、回归与文档收口**：补充 API 权限、项目隔离、敏感信息、空历史、并发更新、回滚失败和迁移升级/降级测试；完成后执行后端非集成回归、前端 Vitest、type-check、生产构建、Ruff、格式检查和 `git diff --check`，代码审查发现的问题修复后再标记为 `[E]` 并提交推送。
