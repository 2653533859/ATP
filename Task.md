...
开发计划和单模块交付规则已记录在 [`docs/product-navigation-roadmap-2026-08-24.md`](docs/product-navigation-roadmap-2026-08-24.md)，本节只维护阶段状态；导航已按参考方案固定为五组，N0 导航壳、N1 工作台与任务中心、N2.1/N2.2/N2.3/N2.4/N2.5 本地实现和审查已完成，均待真实环境验收。
...
- [ ] N3 测试能力工作台：接口、APP、UI 和性能统一配置/执行/观察入口。
- [ ] N4 智能中枢：AI 智能测试、Hermes、需求与用例追踪、知识中枢。
- [ ] N5 系统能力：远程工具箱、配置差异、变更审计和可控回滚。
- [ ] N6 质量收口：回归、E2E、迁移、文档、外部环境证据和发布检查。

当前推荐下一步：进入 N3 测试能力工作台，优先实现 API 测试工作台，统一请求编排、环境变量、数据集、断言、变量提取、依赖和登录态复用；N1/N2.1/N2.2/N2.3/N2.4/N2.5 的真实环境验收作为并行环境证据补齐，不阻塞本地模块开发。每个模块完成后先审查和修复，再同步文档并提交推送。
...
- [E] 审查修复了同步状态覆盖详情表单、查看者 403 操作按钮两个问题；后端非集成 `2155 passed`，前端 `55 files / 220 tests passed`，缺陷详情定向 `3 passed`，type-check/build/ruff/diff-check 和迁移 head `20260824_0061` 通过。真实平台凭据、网络和权限仍需环境验收。
