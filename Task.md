...
- [~] C3 迁移、备份恢复、SLO、证据索引和最终 SHA 发布收口：C3.1～C3.3 与 C3.4.1 已完成。C3.4.2 已增加完整 UTC 日 SLO 采集门禁和安全流量 canary；采集比率只统计存在请求/运行的小时，报告按可用性、延迟和运行成功率分别给出结论，抓取缺口仍阻断全部分项。2026-09-13 与 2026-09-14 的 Backend/Worker 均为 288/288；前者无业务样本，后者在 34 次有界合成请求和 3 次运行下取得可用性 100%、P95 95ms、运行成功率 100%，三项达标但告警和发布门禁因不足 14 天继续 deferred。定向采集器 `19 passed`、非集成后端全量 `2617 passed / 1 skipped`；仍须累积有代表性流量的连续 7/14 天。候选异机 `172.31.27.133` 的 VPN/路由与服务仍不可用，未部署 MinIO；之后进入 C3.5 最终 SHA/证据索引。证据见 [`docs/slo-history-2026-09-14-2026-09-14.md`](docs/slo-history-2026-09-14-2026-09-14.md) 与 [`docs/evidence/c3-slo-history-day2-2026-09-15.json`](docs/evidence/c3-slo-history-day2-2026-09-15.json)。
