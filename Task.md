...
- [~] P2：Q10 完成后进入 Q11 下一轮优化路线，优先处理 PR / commit 拆分、发布说明、生产 SLO 校准、前端覆盖率增长和运维 runbook。路线见 `docs/optimization-roadmap-2026-q11.md`；Q11-00 当前大 diff 拆分计划、Q11-01 发布说明 / 风险 / 回滚记录、Q11-02 最终 CI matrix replay 与 GitHub runner 证据归档、Q11-10 API availability / P95 预生产校准、Q11-11 SLO triage runbook 已完成；下一步执行 Q11-12，决定当前是否添加 Grafana SLO 告警模板，或明确暂缓到生产 Prometheus 历史可用后再启用。
