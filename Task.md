...
> 当前有效顺序（2026-08-25 2.0.0）：导航已统一为“工作台、测试能力、测试资产、智能中枢、系统”五组；N2 Karing 单设备的包身份、Worker、低代码、录屏、稳定性/Monkey、性能、流畅度、事件/报告和清理已通过，下一执行游标转为 N4 真实性能环境；该环境因缺少 Kubernetes/Prometheus/独立 MinIO 保持待验收，随后推进 N5-N8 外部依赖复核和 N9 发布收口。详细阶段、验收出口和状态口径以开发计划 2.0.0 为准，下方更早的历史记录不覆盖当前执行版。
