...
- [-] P4 性能测试工作台：目标 Linux 的只读预检已确认没有 `kubectl`；虽有 Prometheus、MinIO 和性能相关 Docker 容器，但仍缺 Kubernetes、发布级 Prometheus 验收和独立 MinIO source/target，不能以单节点 Compose 或跳过项替代。
...
- [x] 计划检查点 2.4.34：对目标 Linux 进行脱敏、只读的 P4 资源预检：`kubectl` 不存在，无法取得 Kubernetes 上下文或 Ready 节点；Docker 中虽有 1 个 Prometheus、2 个 MinIO 和 1 个性能相关运行容器，均不能证明发布级采样或跨主机独立 MinIO。P4 保持 `[-]`，需提供 Kubernetes、发布级 Prometheus 和不同主机或不同 IP 的 MinIO source/target 后再按 U-P4.1～U-P4.5 复验；证据见 [`docs/evidence/p4-environment-preflight-2026-09-04.json`](docs/evidence/p4-environment-preflight-2026-09-04.json)。
