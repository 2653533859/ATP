...
- [~] H9 Hermes 模型辅助只读规划、成本治理和质量评测：H9.1 已完成模型候选计划、固定工具白名单、严格参数 Schema、两步上限、确定性回退和前端可解释展示；模型只在规则未命中的普通问题上调用，取消/挂起补全不消耗调用。定向后端 `31 passed`、前端 `24 passed`，后端非集成全量 `2592 passed / 1 skipped`、前端全量 `76 files / 363 tests passed`，TypeScript、mypy 和生产构建通过。下一步 H9.2 接入 usage、调用次数、延迟、成本估算和回退治理。
