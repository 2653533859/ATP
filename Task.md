...
- [x] P7/N6 验收缺陷修复：空白临时项目无模块时脚本会显式创建模块；真实验收发现需求创建接口在异步提交后序列化过期 ORM 属性导致 `MissingGreenlet`/HTTP 500，已增加 `refresh` 和回归测试。临时项目已清理，失败证据见 `docs/evidence/n7-intelligence-acceptance-2026-08-25.json`；q19 需从最新提交重建后再复验，N7 仍保持 `[E]`。
