...
- [x] 2026-09-24 Hermes K3s 回答与受控回退：一条合成需求的真实模型短问句回答正确引用 `[S1]`，人工核对五次失败、锁定十五分钟及通知缺口；本机桩无引用、模型连接失败时查询退回检索摘要，规划连接失败记录 `model_call_failed`。长中文问句漏检已由 `9f161362` 的有界四字片段兜底修复，后端非集成全量 `2664 passed / 2 skipped`，部署到 Helm revision 48 后原问句命中正确来源、无关问题仍拒答；临时项目与配置均已清理。供应商侧审计、超时/限额和固定十题仍待独立验收。见 [`docs/evidence/hermes-k3s-answer-fallback-cjk-2026-09-24.md`](docs/evidence/hermes-k3s-answer-fallback-cjk-2026-09-24.md)。
