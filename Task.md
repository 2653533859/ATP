...
> 当前有效顺序（截至 2026-09-01，2.4.0）：参考导航按“工作台、测试能力、测试资产、智能中枢、系统”五组落地；P0 工作台、P1 接口、P2 APP、P3 UI 为 `[E]`（本地实现或受控证据完成，仍有对应真实边界复核），P4 性能为 `[-]`（缺 Kubernetes、发布级 Prometheus 和独立 MinIO），P5～P8 为 `[x]`（q19 受控真实验收完成），P9 为 `[~]`（等待 P4）。Hermes H1～H3 已完成本地交付，真实模型、角色、审计和目标部署仍须独立复核。不得把页面可打开、mock 或单节点 Compose 写成真实通过；详细模块边界、依赖和验收出口以开发计划 2.4.0 总表为准。
...
- [x] Hermes 补齐 H2 持久会话、H3 只读工具审计、H4 人工确认后创建禁用计划草稿、H5 prompt/延迟/引用/反馈汇总的最小治理链；真实模型效果与角色/部署证据仍保留环境门禁。
...
- [x] 计划检查点 2.4.25：Hermes H2 已完成项目绑定会话 ID、有限历史传递与脱敏裁剪、Knowledge/Requirement/Case 来源类型筛选、更新时间范围和上下文预算；刷新/切换项目/新会话会清空历史，旧请求响应不会写入新会话。后端 H2 定向 `12 passed`、后端非集成全量 `2400 passed`、前端 Hermes `11 passed`、前端全量 `69 files / 325 tests passed`、TypeScript 检查通过；H3 权限保护只读工具、H4 人工确认草稿、H5 评测治理列入后续计划，真实多副本会话/模型/角色/审计仍待环境复核。
...
- [x] 计划检查点 2.4.27：在 H2 客户端上下文控制和 H3 五类受控只读工具基础上，补齐项目/用户隔离的持久会话、H4 显式确认后创建禁用手工 draft 计划，以及 H5 反馈、延迟、引用覆盖和无结果率最小治理链路；真实模型效果、完整角色矩阵和目标部署证据仍待环境复核。
