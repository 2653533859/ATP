...
- [~] H9 Hermes 模型辅助只读规划、成本治理和质量评测：H9.1～H9.3 已完成安全候选规划、usage/成本治理及 `hermes-core-v2` 十题可复现评测；四项质量指标只统计当前版本固定题目的服务端评分，结果脱离消息保留并按会话/题号保留最新值。定向后端 `36 passed`、前端 `26 passed`，受影响后端文件独立 `13 passed` 与 `23 passed`；后端非集成全量 `2597 passed / 1 skipped`、前端全量 `76 files / 363 tests passed`，TypeScript、mypy 和生产构建通过。下一步 H9.4 从失败任务生成回归范围和套件映射建议，继续保持人工确认和禁用草稿边界。
