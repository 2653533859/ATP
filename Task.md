...
- [~] C3 迁移、备份恢复、SLO、证据索引和最终 SHA 发布收口：C3.1～C3.3 与 C3.4.1 已完成。C3.4.2 已增加可独立运行的 SLO-only 采集、每天 288 个五分钟连续性检查点、`NaN`/无穷值过滤及缺口门禁；采集器会拒绝未结束的当前/未来 UTC 日，并分别判定 Backend 与 Worker 抓取连续性。2026-09-13 首个完整 UTC 日两者均为 288/288，但请求量为 0，API 可用性、延迟和运行成功率无样本，告警与发布门禁保持 deferred。候选异机 `172.31.27.133` 从 Windows 可建立四个候选端口的 TCP，但 SSH 主动关闭且 MinIO 健康检查无有效响应；发布主机到四端口均不可达，未部署 MinIO。定向 `17 passed`、非集成后端全量 `2608 passed / 1 skipped`。继续等待有真实流量的 7/14 天历史与可用的不同故障域 MinIO，之后进入 C3.5 最终 SHA/证据索引。证据见 [`docs/evidence/c3-slo-history-day1-2026-09-15.json`](docs/evidence/c3-slo-history-day1-2026-09-15.json)。
