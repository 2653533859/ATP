...
**最后更新**: 2026-03-03
**当前阶段**: Phase 1 - 基础框架（MVP）→ 1.5 WebSocket 实时推送
...
- [x] 实现执行结果写入数据库
- [ ] 实现 WebSocket 推送执行状态到前端

### 1.6 执行报告（基础版）

- [x] 前端：执行记录列表页面
- [x] 前端：单次执行报告页面（总览 + 用例列表 + 步骤详情）
- [ ] 前端：WebSocket 接收实时执行状态并更新 UI
- [x] 实现执行记录查询接口（`GET /api/v1/runs/{id}`）

### 1.7 环境管理

- [ ] 实现环境 CRUD 接口（开发/测试/预发/生产）
- [ ] 实现环境变量的存储与查询
- [ ] 接口测试执行时注入环境变量（URL 前缀替换、变量占位符解析）
- [ ] 前端：环境管理页面
...
- [ ] 设计步骤数据结构（操作类型 + 参数）
- [ ] 实现低代码用例存储接口
- [ ] 支持操作：跳转 URL / 点击 / 输入 / 断言文本 / 断言元素可见 / 等待 / 截图
- [ ] 前端：步骤配置表单（可拖拽排序）
- [ ] 实现低代码用例 → Playwright API 调用的执行器

### 2.4 报告增强（截图/录像）

- [ ] 执行报告中展示每个步骤的截图
- [ ] 支持失败步骤高亮
- [ ] （可选）支持 Playwright 录像（`.webm`）上传与播放
...
- [ ] 实现 ADB 设备扫描服务（`adb devices` 定期轮询）
- [ ] 实现设备 CRUD 接口（`GET /api/v1/devices`）
- [ ] 实现设备状态监控（在线/离线，实时推送）
- [ ] 前端：设备列表页面（显示设备名、系统版本、状态）

### 3.2 APK 管理

- [ ] 实现 APK 文件上传到 MinIO
- [ ] 实现 APK 版本管理接口（关联项目，支持多版本）
- [ ] 前端：APK 管理页面（上传、列表、删除）

### 3.3 uiautomator2 执行器

- [ ] 验证 Worker 容器通过 ADB over TCP 连接宿主机真机
- [ ] 实现 Android pytest 脚本执行器
  - [ ] 从 MinIO 下载脚本到临时目录
  - [ ] 自动安装 APK 到目标设备
  - [ ] 调用 `pytest --json-report` 执行
  - [ ] 解析结果，收集截图上传 MinIO
  - [ ] 清理临时目录
- [ ] 实现执行配置：设备选择 / APK 版本 / 超时

### 3.4 设备屏幕镜像

- [ ] 实现 MJPEG 截图流（后端定期调用 `uiautomator2.screenshot()` 推流）
- [ ] 前端：嵌入屏幕镜像视频流组件

### 3.5 Android 低代码模式

- [ ] 支持操作：点击坐标 / 点击元素（text/resourceId/xpath）/ 长按 / 滑动 / 输入 / 截图断言
- [ ] 前端：步骤配置表单
- [ ] 实现低代码步骤 → uiautomator2 API 调用的执行器
...
- [ ] **GraphQL**：支持 Query / Mutation，变量配置，断言
- [ ] **WebSocket**：建立连接 / 发送消息 / 接收断言 / 超时配置
- [ ] **gRPC**：上传 `.proto` 文件，方法调用，响应断言

### 4.2 测试套件

- [ ] 实现 `TestSuite` CRUD 接口
- [ ] 支持跨类型用例组合（Web + 接口 + Android）
- [ ] 支持套件内用例顺序配置
- [ ] 支持套件级数据驱动（CSV / JSON 参数化）
- [ ] 实现套件执行触发接口（`POST /api/v1/suites/{id}/run`）
- [ ] 前端：套件管理页面
...
| 里程碑 | 完成条件 | 状态 |
|--------|---------|------|
| **M1** Phase 1 完成 | HTTP 接口测试用例可完整执行并看到报告 | `[ ]` |
| **M2** Phase 2 完成 | Playwright 脚本可上传执行，报告含截图 | `[ ]` |
