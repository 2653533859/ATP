...
- [~] H9 Hermes 模型辅助只读规划、成本治理和质量评测：H9.1～H9.2 已完成模型候选计划、服务端安全校验，以及调用次数、延迟、OpenAI/Claude/Ollama usage、显式配置成本和回退原因治理；缺失 usage 或价格时明确标记不可用，不伪造成本。定向后端 `32 passed`、前端 `26 passed`，受影响后端文件独立 `21 passed` 与 `11 passed`；后端非集成全量 `2593 passed / 1 skipped`、前端全量 `76 files / 363 tests passed`，TypeScript、Ruff、格式、mypy 和生产构建通过。下一步 H9.3 扩展工具选择、引用相关性、答案完整度和拒答正确率评测。
