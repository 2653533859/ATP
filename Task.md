...
- [~] C1 单节点 K3s 性能与可观测性：C1.1 有界采样入口和 C1.2 性能生命周期均已完成。C1.2 在真实 k6 短压中发现并修复上传脚本不应用平台 options 的缺陷，修复后 run 39 完成 974 次迭代、错误率 0、2/2 Threshold 通过、4 项基线指标无回归，JSON/CSV/raw summary 和资源样本齐全；run 40 取消后无进程、临时目录或队列残留，验收运行和对象已精确清理，原测试 options 与节点 allowlist 已恢复。30 秒 4 次采样保持 6/6 Pod Ready、零重启和 0 告警。证据见 [`docs/evidence/c1-performance-lifecycle-2026-09-10.json`](docs/evidence/c1-performance-lifecycle-2026-09-10.json)。下一步 C1.3 验证 PostgreSQL、Redis、MinIO 的连接、权限、备份与恢复边界；发布级 Prometheus/P4 仍为独立门禁，多节点高可用及跨主机 MinIO 灾备保持 `[OUT]`。
