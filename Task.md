...
- [~] 2026-09-27 C3.4 完整日 SLO 与 Flower 检查点：正式采集 9 月 24～25 日 UTC 报告；24 日 Backend 有一次 `up=0`，25 日虽完整但主要为低量 canary，门禁继续 `deferred`。发现首次 Worker 运行结果序列从 1 才出现，9 月 25 日日增量漏算一次；仓库已预初始化 15 个零值序列并加入回归，目标 revision 56 尚未部署该修复。Flower 新抓取约 3 小时、718 个原始样本，RSS 约 154MiB、进程启动变化 0；长期稳定性未关闭。见 [`docs/evidence/c3-slo-flower-checkpoint-2026-09-27.md`](docs/evidence/c3-slo-flower-checkpoint-2026-09-27.md)。
- [~] 2026-09-26 C3.4 发布预检与 Flower 连续观测：新增按仓库逐文件核对 Chart、对照现有 Helm release 渲染资源/镜像/迁移 Hook 的只读预检，未审查变更直接失败；目标机旧 Chart 被拦下，完整 Chart 显示 6 个 Deployment 差异，故业务 Helm 保持 revision 56。发布 Prometheus 已纳入 Flower `:5555/metrics`，5/5 target up、6 条规则健康，Grafana 20 个面板；同时修正运行成功率告警误选 Backend job，实测该指标由 Worker 暴露。9 月 24 日 Backend 287/288（10:10 UTC 一次 down），25 日 Backend/Worker 288/288 但运行量低，7/14 日代表性 SLO 与长期稳定性继续待办。见 [`docs/evidence/c3-flower-preflight-observability-2026-09-26.md`](docs/evidence/c3-flower-preflight-observability-2026-09-26.md)。
- [~] 2026-09-25 C3.4 Flower 稳定性：追查 revision 55 的 512Mi `OOMKilled/137`，确认旧发布 Chart 丢失 9 月 7 日已验证的有界保留参数；从旧 Chart 复制并仅补 Flower 参数/单副本更新策略，渲染 10 个非 Hook 资源仅 Flower Deployment 变化。Helm revision 56 已部署，进程参数 `1000/100/300`、新 Pod 0 重启、6/6 Ready、Backend/Flower 健康 200。9 月 26 日约 22 小时复核仍为 0 重启、123Mi，但同期只有约 20 次 HTTP 请求和 2 次 Worker 运行结果，且缺少 Flower 连续内存指标；长期内存趋势和代表性 7/14 日 SLO 仍待观察。见 [`docs/evidence/k3s-flower-chart-drift-2026-09-25.md`](docs/evidence/k3s-flower-chart-drift-2026-09-25.md)。
- [x] 2026-09-25 Hermes Engineer/Editor 目标环境验收：经正式 API 在两临时项目中测试独立全局 Engineer + A 项目 Editor，冻结十项 **10/10**。A 的真实模型回答正确引用、Editor 确认只生成禁用手工计划；Owner 与跨项目操作 403、跨用户确认 404；降为 Viewer、移除成员、停用账号后的权限即时收敛。初次准备因夹具 `source` 超长停止，项目 106/107 已清理；复跑复用账号 22，项目 108/109、需求和计划 8 清理，账号 22 按正式 API 停用并保留审计身份。Helm revision 55 四次 6/6 Pod Ready/健康 200，但 Flower 在本轮前发生一次 `OOMKilled`，发布稳定性门禁不据此关闭。见 [`docs/evidence/hermes-engineer-editor-2026-09-25.md`](docs/evidence/hermes-engineer-editor-2026-09-25.md)。
- [x] 2026-09-25 Hermes 目标环境项目 Viewer 与审计边界：事前冻结九项，在两临时项目中用现有管理员和仅对 A 授予项目 Viewer 的现有验收账号实测 **9/9**。A 的真实模型回答仅引用 A 的需求；Viewer 只读工具 HTTP 200、`empty` 且审计 0→1，越权写入/跨项目五个入口均 403，跨用户反馈均 404，同值反馈只计一次。成员撤销后再次拒绝并落审计；A/B 项目、需求与会话清理，拒绝/工具审计留存，项目 77 模型绑定未变。正式 Helm revision 55 四次 6/6 Pod Ready、健康 200、零重启。本轮未覆盖全局 Engineer、项目 Editor、供应商账单或完整业务内容。见 [`docs/evidence/hermes-role-audit-2026-09-25.md`](docs/evidence/hermes-role-audit-2026-09-25.md)。
- [x] 2026-09-24 Hermes 真实用例元数据问答样本：项目 77 的五条用例经只读 API 核对字段指纹，仅复制七个元数据字段到绑定现有模型配置的临时项目。首轮人工 3/8 暴露用例类型、优先级、级别未进入来源摘录及集合问题漏检；`a49d0734` 修复后同一冻结八题人工 8/8、缺失执行证据题 2/2、会话审计完整。定向 71 项、两个文件独立 38/33 项通过；正式单节点 Helm revision 55、四次 6/6 Pod Ready/健康 200/零重启，临时项目及用例清理，项目 77 模型绑定仍为空。仅关闭这五条真实用例的元数据问答样本，不代表完整用例行为、业务需求/知识或 P4/P9。见 [`docs/evidence/hermes-real-case-metadata-2026-09-24.md`](docs/evidence/hermes-real-case-metadata-2026-09-24.md)。
- [x] A1 前端 API 领域拆分：Hermes 类型与请求方法已迁移到独立模块，保留 `@/api` 兼容导出；定向 22 项、前端全量 347 项、TypeScript 和生产构建通过，代码审查无可操作问题。
- [x] A2 大型前端页面组件化：完成 Hermes 治理、上下文、消息、证据、诊断、计划草稿、性能节点和设备卡片矩阵拆分；Hermes、性能中心、设备管理父页面分别由 2727/2635/1537 行降至 1100/2482/1033 行，请求序列、筛选、写操作、轮询和镜像租约仍归父页面控制。定向性能 15 项、设备 11 项，前端无并发全量 75 文件/359 项、TypeScript、生产构建和差异检查通过；自审修复 scoped 样式迁移遗漏及离线镜像显式防护后无剩余可操作问题。开发游标进入 A3 统一执行状态机。
...
- [x] B4 Android 单设备持续运行：B4.1 完成 K3s/Windows Worker 配对、队列隔离、遗留双控制面清理、离线零积压与双真机扫描；B4.2 完成 run 103～105 的连续低代码、截图、device-info、logcat、MP4 录像和 HTML/PDF 报告闭环；B4.3 的 run 107 在活动步骤中断开设备后明确失败并成功恢复扫描，Worker 中断 run 108 暴露执行租约回收后设备仍被占用的问题。提交 `ff14383c` 使失联回收同步释放精确 `case-run` 设备租约，已部署到 Helm revision 35；post-fix run 109 收敛为 `error` 且摘要确认租约释放，同设备随即被 run 110 复用并 4/4 通过，50 秒观察内 Worker 存活、6/6 Pod Ready、零重启且三队列为 0。证据见 [`docs/evidence/b4-android-fault-recovery-2026-09-10.json`](docs/evidence/b4-android-fault-recovery-2026-09-10.json)。可选多设备矩阵仍不阻塞单节点发布。
- [x] H9 Hermes 模型辅助只读规划、成本治理和质量评测：H9.1～H9.5 已完成安全候选规划、usage/成本治理、`hermes-core-v2` 十题可复现评测、失败任务到活动套件的可解释回归映射，以及数据库乐观版本保护。会话消息、挂起意图、取消、草稿和治理指标遇到并发更新时会回滚并返回稳定 `409`，前端不会把冲突错误降级为另一条查询或重复确认；治理窗口按更新时间和 ID 稳定排序。H9.5 定向后端 `52 passed`、前端 `2 files / 26 tests passed`；后端非集成全量 `2602 passed / 1 skipped`、前端全量 `76 files / 367 tests passed`，TypeScript、mypy、Ruff 和生产构建通过。目标 K3s revision 46 以两个 Backend 实例完成 40 次并发反馈：8 次提交、32 次稳定 `409`、最终版本 10、丢失更新 0；实测同时发现并修复 Hermes 时间戳默认值缺失导致的 PostgreSQL 500，迁移头为 `0074`。临时副本和会话已清理，30 秒后 6/6 正式 Pod Ready、零重启。该同节点双进程证据不等价于多节点故障域或 P4/P9 发布完成。
- [x] 2026-09-24 Hermes K3s 有界真实模型规划：管理员经正式 API 创建临时项目 84 并绑定已启用配置 1；连接测试 HTTP 200，模型在有证据的第二次挑战中选出 `failed_tasks`、`quality_trend`，两步均 `ok` 且各有 1 条证据，usage 可读。临时 Viewer 只读成功、写入和跨项目读取均 403，工具与拒绝审计持久化；成员和项目/用例/运行已清理，原项目 77 未修改。固定十题、完整回答质量、异常回退和发布门禁不因本次通过而关闭。见 [`docs/evidence/hermes-k3s-real-model-acceptance-2026-09-24.md`](docs/evidence/hermes-k3s-real-model-acceptance-2026-09-24.md)。
- [x] 2026-09-24 Hermes K3s 回答与受控回退：一条合成需求的真实模型短问句回答正确引用 `[S1]`，人工核对五次失败、锁定十五分钟及通知缺口；本机桩无引用、模型连接失败时查询退回检索摘要，规划连接失败记录 `model_call_failed`。长中文问句漏检已由 `9f161362` 的有界四字片段兜底修复，后端非集成全量 `2664 passed / 2 skipped`，部署到 Helm revision 48 后原问句命中正确来源、无关问题仍拒答；临时项目与配置均已清理。本检查点当时尚待的超时/限额和固定十题见下一条续验；供应商侧审计独立保留。见 [`docs/evidence/hermes-k3s-answer-fallback-cjk-2026-09-24.md`](docs/evidence/hermes-k3s-answer-fallback-cjk-2026-09-24.md)。
- [x] 2026-09-24 Hermes 临时 K3s 固定十题与回答范围修复：隔离夹具库的十题初跑 10/10，但人工发现模型把“检索片段没显示”误写成“项目缺少关联用例”。`fcf2c507` 将提示范围限定于检索片段并升至 `hermes-v3`，复跑十题 10/10、模型规划挑战通过，两条真实生成回答经人工复核；后端全量 `2665 passed / 2 skipped`。受控桩的 60 秒超时、查询限额和规划限额回退通过。正式单节点 Helm revision 49 一条合成需求真实模型冒烟通过，四次采样六个 Pod Ready、Backend 200；临时项目、配置、配额键与命名空间已清理。正式业务库未跑固定十题，代表性质量阈值、供应商侧审计、长期 SLO 与 P4/P9 仍待验收。见 [`docs/evidence/hermes-k3s-fixed-ten-prompt-v3-2026-09-24.md`](docs/evidence/hermes-k3s-fixed-ten-prompt-v3-2026-09-24.md)。
- [x] 2026-09-24 Hermes 12 题合成回答质量扩样：运行前冻结事实与 11/12 合格线，`98bd86ca` 修复中文短业务词和追问检索，`47994193` 将无来源流程与数值约束写入 `hermes-v4`。当前 K3s Helm revision 52 使用已启用配置 1（运行后核对模型为 `claude-opus-4-6-thinking`）实测 12/12 有来源，人工 11/12、注入/跨项目隔离 3/3；Q06 仍把未定义流程排成固定顺序。定向 67 项、两个文件独立 30/37 项通过，6/6 Pod 就绪、Backend 200，临时项目 90～95 的资产已清理。仅关闭本组合成题阈值，实际业务代表性质量、供应商审计和 P4/P9 仍待验。见 [`docs/evidence/hermes-quality-sample-2026-09-24.md`](docs/evidence/hermes-quality-sample-2026-09-24.md)。
- [x] 2026-09-24 Hermes Q06 续验：`hermes-v5` 修复单题但整组退化为人工 8/12、安全 2/3；`de0d485c` 的 `hermes-v6` 缩短默认回答并限制为所问、直接证据与必要未知项，相同冻结 12 题最终人工 12/12、安全 3/3。正式 K3s Helm revision 54、30 秒四次 6/6 Pod Ready/Backend 200/零重启；定向 69 项、两个测试文件独立运行、Ruff/格式/mypy/钩子通过，临时项目 96～99 和关联资产清理。项目 77 有 17 条用例但无需求/知识且未绑定模型，实际业务代表性质量仍待独立样本验收。见 [`docs/evidence/hermes-quality-sample-2026-09-24.md`](docs/evidence/hermes-quality-sample-2026-09-24.md)。
- [E] 2026-09-23 Hermes 反馈与评测加固：反馈增加稳定消息 ID，裁剪后失效下标统一 `409`，重复/改评计数幂等；移除仍被页面调用的旧会话工具接口，后端只保留 H3 受控工具入口；`run-detail` 固定题补全 `case` 类型并升级题集版本。定向后端 `57 passed`、后端非集成全量 `2629 passed / 2 skipped`；前端全量 `76 files / 369 tests passed`，TypeScript、生产构建、Ruff、格式和 mypy 通过。发布环境只读核对发现 1 个启用模型配置、0 个项目绑定，固定题的运行/需求/知识编号 1 均不存在；真实十题、角色矩阵和审计验收继续待办，不能把本地回归写成模型质量通过。见 [`docs/hermes-development-plan-2026-09-01.md`](docs/hermes-development-plan-2026-09-01.md) 与 [`docs/evidence/hermes-evaluation-readiness-2026-09-23.json`](docs/evidence/hermes-evaluation-readiness-2026-09-23.json)。
- [E] 2026-09-23 Hermes 评测验收准备：当前十题实际由六道确定性编排、两道无结果拒答和两道可能调用回答模型的查询组成，不能把固定集工具选择率解释为模型规划准确率。正向题缺少相关来源或工具有效证据时不计分，规则检索回退不再取得模型引用相关性通过；题集版本升至 `2026-09-23.1`。新增默认只读、显式 `--execute` 才运行的脱敏验收执行器，逐题区分通过、缺夹具和失败，并增设独立模型规划挑战题。后端定向独立 `11 + 23 + 36` 项、非集成全量 `2656 passed / 2 skipped`，前端全量 `76 files / 369 tests passed`，Ruff、mypy、TypeScript 和生产构建通过。目标 K3s 六个 Pod Ready，但 Backend 仍是 `bdb84286`；独立数据库/项目及资产、模型绑定和新版本部署尚未准备，未运行真实模型验收。操作边界见 [`docs/hermes-evaluation-acceptance.md`](docs/hermes-evaluation-acceptance.md)。
- [x] C1 单节点 K3s 性能、可观测性与数据服务边界：C1.1～C1.3 已完成。自动 PostgreSQL 备份已在 Helm revision 41 启用，真实 `maintenance` 任务生成的 65,254 字节备份恢复到隔离数据库后，65 张表、项目/用户计数和迁移头均与源库一致；Redis RDB 在隔离容器中成功恢复探针，MinIO 同端点对象恢复哈希一致，临时数据库、key、容器、快照及对象均已清理。收尾 6/6 Pod Ready、零重启、四队列归零、30 秒 4 次采样 0 告警。权限审计同时确认 PostgreSQL 运行账号为 superuser、Redis ACL 为全命令/全 key/channel、MinIO 使用 root 命名凭据且无 bucket policy/versioning/独立备份端，因此 C1 验证完成但 C3/P4 继续阻塞。证据见 [`docs/evidence/c1-data-services-recovery-2026-09-10.json`](docs/evidence/c1-data-services-recovery-2026-09-10.json)。
- [~] C3 迁移、备份恢复、SLO、证据索引和最终 SHA 发布收口：C3.1～C3.3 与 C3.4.1 已完成。首个 7 日窗口因 6 日无业务样本未通过；9 月 20 日因主机重启只有 276/288 抓取点而被拒绝。9 月 21～23 日完整 UTC 日的 Backend/Worker 均为 288/288，可用性 100%、P95 95ms、运行成功率 100%，但每天只有 21/20/20 次请求和各 2 次运行，均为有界合成 canary；9 月 21 日首次定时任务失败后人工重试，9 月 22 日起无人值守成功。发布 Grafana 已通过同源 Prometheus 数据查询与 17 面板核对；7/14 日代表性流量校准与独立 MinIO 仍待完成，告警和发布门禁保持 deferred；自动采集不再仅凭 14 天无超标启用门禁，Q12 验收摘要须经人工审查。证据见 [9 月 21 日](docs/slo-history-2026-09-21-2026-09-21.md)、[9 月 22 日](docs/slo-history-2026-09-22-2026-09-22.md)、[9 月 23 日](docs/slo-history-2026-09-23-2026-09-23.md)及 [Grafana 验证](docs/evidence/c3-slo-grafana-2026-09-23.json)。

> 历史口径（截至 2026-09-04，2.4.0，已由 2026-09-08 计划接续）：参考导航按“工作台、测试能力、测试资产、智能中枢、系统”五组落地；P0 工作台、P1 接口、P2 APP、P3 UI 为 `[E]`（本地实现或受控证据完成，仍有对应真实边界复核），P4 性能原为 `[-]`，P5～P8 为 `[x]`（q19 受控真实验收完成），P9 为 `[~]`。Hermes H1～H8 已完成本地交付；真实模型工具选择效果、角色、成本、审计和目标部署仍须独立复核。新范围以顶部链接的 2026-09-08 计划为准。

> 本次发布范围已于 2026-09-01 明确：iOS/Appium、SMTP/企业微信/钉钉、Jira/禅道/GitHub/GitLab 不纳入正式支持范围，现有代码保留为技术预览；这些能力不阻塞本次发布，也不代表真实环境通过。统一矩阵见 [`docs/release-scope-2026-09-01.md`](docs/release-scope-2026-09-01.md)。

## 2026-09-01 产品能力缺口闭环

- [x] GraphQL 增加 `graphql-transport-ws` Subscription、有界重连和公共端点 Schema introspection 根字段补全；普通 WebSocket 增加仅握手阶段的有界重试。
- [x] Web/Android 脚本增加精确锁定 `requirements.txt` 资产，Worker 在运行临时目录隔离安装，拒绝 URL、Git、本地路径与 pip 索引参数。
- [x] 新增可管理 Android 设备组并接入设备矩阵；新增 Suite 共享变量与 setup/teardown Fixtures，复用受控 Hook 动作且记录生命周期结果。
- [x] 新增 iOS 设备/IPA 前端资产入口并固定标记技术预览；不改变本次 `[OUT]` 范围，也不声称真实 macOS/iPhone 通过。
- [x] Hermes 补齐 H2 持久会话、H3 只读工具审计、H4 人工确认后创建禁用计划草稿、H5 版本化 5 题评测集与 prompt/延迟/引用/拒答/反馈汇总治理面板、H6 受控自然语言只读编排、H7 会话内受控多轮参数补全，以及 H8 挂起意图取消与安全恢复；真实模型工具选择效果、成本 usage 与角色/部署证据仍保留环境门禁。
- [x] 新增迁移 `20260901_0067`、`20260901_0068`；运行与边界说明见 [`docs/product-capability-closure-2026-09-01.md`](docs/product-capability-closure-2026-09-01.md)。

## 2026-08-25 当前开发计划 2.4.0 跟踪摘要（截至 2026-09-04 统一口径）

- [x] 导航计划登记：五组分工、入口职责、旧 URL 兼容、项目上下文、阶段顺序和状态口径已同步到开发计划 2.4.0。
- [E] P0 工作台与任务中心：本地实现和回归已完成，保留真实账号、角色、任务数据和越权边界复核。
...
- [-] P4 性能测试工作台：目标 Linux 已完成单节点 K3s `v1.36.3+k3s1`，root 私有 kubeconfig 的 `atp-single-node` context、节点、系统 Pod 与 API 均就绪；本轮用户明确不要求多节点，因此该集群用于本机 Kubernetes 开发/联调。发布级 Prometheus、独立 MinIO source/target，以及原 P4 多节点短压/恢复验证仍未完成，不能以单节点集群、Compose 或跳过项替代。
...
- [x] 计划检查点 2.4.26：Hermes H3 已完成固定只读工具目录和执行 API，覆盖失败任务、运行详情、质量趋势、需求—用例关联和知识详情；所有工具要求项目 viewer 权限，专属参数白名单、最长 5 秒、脱敏结果、稳定 `HERMES-*` 证据和 `hermes_read_tool` 审计均已实现，未知参数被拒绝，工具不触发写入或外部操作。H3 API/服务定向 `11 passed`、H1～H3 Hermes 组合回归 `27 passed`、后端非集成全量 `2411 passed`、前端全量 `69 files / 325 tests passed`；自然语言自动编排、真实模型/角色/审计/目标部署仍待环境复核，下一步为 H4 人工确认结构化草稿。
- [x] 计划检查点 2.4.27：在 H2 客户端上下文控制和 H3 五类受控只读工具基础上，补齐项目/用户隔离的持久会话、H4 显式确认后创建禁用手工 draft 计划，以及 H5 反馈、延迟、引用覆盖和无结果率最小治理链；同时补齐 H4 测试计划结构化草稿审阅，支持名称/目标/测试点编辑、模块/用例/失败任务回归范围选择、基线/当前 diff、来源和影响数量。确认前不能打开保存页；确认后可通过有界路由状态预填现有计划页，或在显式二次确认后创建禁用手工 draft，首次保存草稿时按需创建空会话，仍不会自动执行。Hermes/计划页定向 `20 passed`，后端非集成全量 `2429 passed`，前端全量 `69 files / 327 tests passed`，TypeScript、生产构建和 `git diff --check` 通过；真实模型效果、完整角色矩阵、审计和目标部署证据仍待环境复核，下一步为 H5 评测治理收口。
- [x] 计划检查点 2.4.28：H5 增加版本化 `hermes-core-v1` 五题只读评测集与元数据接口，治理汇总严格按有效 `[S#]` 引用统计覆盖率，补齐拒答/无结果率、平均/P95 延迟、prompt 版本、helpful/not-helpful 反馈和成本不可用状态；Hermes 页面增加紧凑治理状态卡片并保护项目切换时的异步响应。H5 定向后端 `18 passed`、前端 Hermes `12 passed`，后端非集成全量 `2432 passed`、前端全量 `69 files / 327 tests passed`，TypeScript、mypy、生产构建、Python 编译、Ruff、格式/差异检查、密钥扫描及提交钩子均通过；真实模型效果阈值、token usage/成本、角色矩阵、审计和目标部署证据仍待环境复核。
- [x] 计划检查点 2.4.29：H6 新增 `/hermes/orchestrate` 受控自然语言只读编排，从五个固定 H3 工具中最多选择两步；缺少运行/需求/用例/知识显式目标时返回 `needs_input`，未命中时由前端回退 H1 检索；实际执行复用 viewer、参数白名单、5 秒超时、脱敏、稳定证据和审计，成功状态写入会话并展示自动读取链路。H6 定向后端 `22 passed`、前端 Hermes `14 passed`，后端非集成全量 `2436 passed`、前端全量 `69 files / 329 tests passed`，TypeScript、mypy、生产构建、Python 编译、Ruff、格式/差异检查、密钥扫描及提交钩子均通过；真实模型工具选择效果、角色矩阵、成本 usage、完整审计和目标部署证据仍待环境复核。
...
- [x] 计划检查点 2.4.34：对目标 Linux 进行脱敏、只读的 P4 资源预检后，已按官方二进制路径安装并校验 `kubectl v1.37.0`；该次初始快照中客户端未配置 Kubernetes 上下文，常用 kubeconfig 路径、`kubelet`/`k3s`/`microk8s` 服务和本地控制面容器均不存在，无法取得 Ready 节点。Docker 中虽有 1 个 Prometheus、2 个 MinIO 和 1 个性能相关运行容器，均不能证明发布级采样或跨主机独立 MinIO。后续单节点 K3s 上下文见 2.4.35；P4 保持 `[-]`，仍须提供发布级 Prometheus 和不同主机或不同 IP 的 MinIO source/target，并按 U-P4.1～U-P4.5 复验；预检与安装验证证据见 [`docs/evidence/p4-environment-preflight-2026-09-04.json`](docs/evidence/p4-environment-preflight-2026-09-04.json) 和 [`docs/evidence/p4-kubectl-install-2026-09-04.json`](docs/evidence/p4-kubectl-install-2026-09-04.json)。
- [x] 计划检查点 2.4.35：按单节点范围完成目标 Linux K3s `v1.36.3+k3s1` 自建集群，禁用未使用的 Traefik；节点及 CoreDNS、local-path-provisioner、Metrics Server 全部 Ready，API `/readyz` 和节点读取授权通过。K3s 重启会重写自身 kubeconfig，因此 root 改用 `0600` 私有副本并将 context 固定为 `atp-single-node`。公共镜像仓库初始超时后，仅复用宿主 Docker 既有、无凭据且可达的镜像源恢复系统 Pod，未记录镜像源地址或凭据。单节点环境可用于 Kubernetes 开发/联调；P4 发布级 Prometheus、独立 MinIO 与原多节点验收仍独立保留。证据见 [`docs/evidence/k3s-single-node-bootstrap-2026-09-04.json`](docs/evidence/k3s-single-node-bootstrap-2026-09-04.json)。
- [x] 计划检查点 2.4.36：新增单节点 K3s 性能联调 Helm overlay，固定 1 个 `atp-single-node` 性能 Worker、专用队列、已导入镜像的 `Never` 拉取策略，并关闭跨节点反亲和、HPA、Ingress 与 ServiceMonitor；仓库预检、24 项部署契约回归、Helm lint/template 均通过。目标 Linux 已校验安装 Helm `v4.2.4`，以临时 Chart 副本完成同样的远端 lint/template；未创建 namespace、Helm release 或应用 Pod。当前没有 Prometheus Operator/ServiceMonitor CRD，K3s containerd 也没有 ATP 镜像，因此仍须先提供同一 SHA 的已导入镜像和外部 Secret，再决定是否执行受控联调安装；不关闭 P4。证据见 [`docs/evidence/k3s-single-node-helm-preflight-2026-09-04.json`](docs/evidence/k3s-single-node-helm-preflight-2026-09-04.json)。
- [x] 计划检查点 2.4.37：复用目标 Linux 当前 ATP 运行容器的 PostgreSQL/Redis/MinIO 配置键创建 `atp-single-node` namespace 内的 `atp-single-node-secrets`，仅将三个 Host 改为 K3s 节点内部地址，保留凭据值在远端 Secret 中且未输出；Secret 完整性核验通过，包含 18 个键。临时 Pod 到 PostgreSQL `5432`、Redis `6379`、MinIO `9000` 的 TCP 连通均通过，探针已清理；未创建应用 release/Pod，P4 不变。证据见 [`docs/evidence/k3s-single-node-secret-preflight-2026-09-04.json`](docs/evidence/k3s-single-node-secret-preflight-2026-09-04.json)。
- [x] 计划检查点 2.4.38：以当前 Git `HEAD` `1bccfef4` 构建 backend、worker、frontend 镜像并导入单节点 K3s containerd；当前提交 Chart 使用单节点 overlay 执行 Helm server-side dry-run 通过，渲染 5 个 Deployment、3 个 Service、6 处当前 SHA 镜像引用，未生成 ServiceMonitor/HPA/Ingress/Secret；dry-run 后没有 Helm release、应用 Pod 或迁移 Job。Backend/Worker 使用目标机上与当前 `requirements.txt` SHA-256 一致的依赖/浏览器层做源码覆盖，Frontend 按当前 Dockerfile 构建；仓库 Dockerfile 未修改，该证据仅用于单节点开发/联调，不关闭 P4/P9。证据见 [`docs/evidence/k3s-single-node-image-dry-run-2026-09-04.json`](docs/evidence/k3s-single-node-image-dry-run-2026-09-04.json)。
...
- [x] 计划检查点 2.4.44：修复 `hostNetwork` 下两个 Worker 的 Celery 节点名冲突和更新指标端口竞态；部署回归 `27 passed`，Helm revision 6 已部署，4 个不同 Celery 身份可回应、普通 Worker 定向查询唯一。5 个核心 Pod Ready/0 重启、Backend 健康；短任务须显式指定单节点专用队列，真实执行/报告/取消恢复仍待验收。详见开发计划 2.4.44 及脱敏证据。

- [x] 计划检查点 2.4.45：真实单节点 k6 短任务、运行中取消、取消后恢复及三次 JSON 报告核验完成；10 秒/10 次迭代和 5 秒/5 次迭代均错误率 0。清理 2 个临时项目、5 条运行及 6 个 MinIO 对象，无残留 k6 进程。修复试跑发现的自动项目编码碰撞，相关回归 `37 passed`、独立 `28 passed`；该代码修复尚未部署 Linux。其余执行器及 P4/P9 发布门禁独立保留，详见开发计划 2.4.45。

- [E] 计划检查点 2.4.46：JMeter 单节点短任务、取消/恢复及 JSON 报告在 revision 7 通过；Linux 取消路径改为独立 session/进程组终止，93 项性能服务回归通过、1 项 Linux 专属用例在 Windows 跳过，另有 Linux 真实子进程验证通过。补齐 Windows mypy 兼容后，最终单文件 Worker 补丁同步至 Helm revision 8，源码哈希及健康核验通过。revision 7 复验无存活 Java 负载进程，但留下 1 个僵尸记录，下一项优先补齐 PID 1 回收，不能用更换 Pod 代替修复。2 个临时项目、6 条运行和 6 个 MinIO 对象已清理；不算完整进程清理或 P4/P9 通过，详见开发计划 2.4.46。

- [x] 计划检查点 2.4.47：通过镜像 Tini 默认入口和 Helm 显式 Tini 启动关闭 2.4.46 的孤儿回收缺口；revision 9 已部署，两个 Worker 的 PID 1 均为 tini。隔离进程树连续取消 3 轮无僵尸，信号/退出码传递通过；真实 JMeter 短任务、连续取消 3 次、恢复和五份 JSON 报告全部通过，各次 Java/僵尸数均为 0。32 项相关回归、独立 28 项通过；项目 74、5 条运行及 3 个 MinIO 对象已清理。下一步 Locust/gRPC 单节点验收，P4/P9 不变。
- [x] 计划检查点 2.4.48：revision 9 专用性能队列的 Locust 短任务、取消和恢复通过（Run 18/19/20）；短任务 9 次请求、恢复 4 次请求，错误率均为 0，三份 JSON 报告核对通过，各段执行器残留/僵尸均为 0。回归 8 passed/1 Linux-only skipped，Locust 独立 2 passed；审查无新增代码问题。临时项目 75、3 条运行与 3 个 MinIO 对象已清理，5 Pod Ready/0 重启。下一步 gRPC 单节点验收；非最新 main 全量部署，P4/P9 不变。
- [x] 计划检查点 2.4.49：gRPC TLS 四模式、取消及恢复通过（Run 21–26），成功任务各 5 次 RPC、错误率 0，六份报告核对通过，各段结束后目标连接为 0。使用独立临时目标绕开旧证书过期前置问题，保留证书/主机名校验；项目 76、6 条运行、6 个 MinIO 对象、临时目标和证书已清理，Worker 域名映射原样恢复。相关回归 20 passed/1 Linux-only skipped，独立 14 passed。5 Pod Ready，但 Flower 有 1 次 OOMKilled/137（512Mi）；下一步排查其内存增长，P4/P9 仍不关闭。
- [x] 计划检查点 2.4.50（配置修复）：Flower 任务缓存收敛至 1000、Worker 缓存 100、离线清理 300 秒，512Mi 限额不变；合成 11 万事件验证峰值 RSS 由 280352 降至 52728 KiB。revision 10 仅修改 Flower args，健康检查 200、5 Pod Ready；独立回归 29 passed，Helm lint/渲染范围/非法值校验及 Ruff 通过。
- [ ] 2.4.50 后续：真实事件积累下 Flower 长期内存与重启趋势复核。历史 OOM 无堆快照，本轮只验证缓存内存贡献及短时部署健康，不能宣称已排除全部内存增长来源；P4/P9 不变。

> 下方逐项明细保留各检查点的原始描述；其中“仍待验收”属于当时快照，不覆盖以上截至 2026-09-06 的统一状态。
...
- [x] N6 viewer 角色矩阵验收工具补强：提供普通 viewer 账号或 `ATP_VIEWER_TOKEN` 时创建独立隔离项目，验证主项目的用例/评审/套件/计划/运行/缺陷读取，以及跨项目项目、成员、用例、套件、计划、缺陷读取拒绝和用例/模块写入拒绝；分别清理成员、主项目和隔离项目。定向 API/脚本回归 `93 passed`、后端非集成全量 `2385 passed`、Ruff/格式/diff-check 通过；真实 viewer/admin 凭据和真实项目数据仍待外部门禁。
- [x] P0 任务中心终止安全：单任务和批量终止均增加二次确认，中英文提示接入 `vue-i18n`；确认回调再次校验 `can_stop/can_retry`，避免状态刷新后的过期确认绕过前端保护。前端全量 `69 files / 303 tests passed`，`vue-tsc`、生产 build 和差异检查通过；真实角色/终止权限仍待环境复核。
- [x] P0 任务中心失败诊断入口：失败、异常、取消和停止任务可直接打开诊断弹窗；case 复用既有用例诊断链，suite/plan/android/performance 复用工作台诊断接口，展示来源、摘要、证据和修复建议，并丢弃关闭后的过期响应。定向测试 `4 passed`，前端全量 `69 files / 304 tests passed`，`vue-tsc`、生产 build 和差异检查通过；真实角色、诊断数据和执行器证据仍待环境复核。
- [x] 计划检查点 2.4.12：已完成任务中心失败诊断的实现、回归、独立审查、修复和文档/记忆同步；外部顺序仍为受控 viewer/admin 角色矩阵 → 真实 AI/来源审计 → P8 目标治理 → P9 发布收口。
- [x] P0 任务中心失败证据展示：诊断弹窗展示失败步骤数、截图数、错误样本和安全截图入口；前端对错误样本明确建模，并拒绝 `javascript:`/`data:` 等非 HTTP(S) 地址。定向测试 `4 passed`，前端全量 `69 files / 304 tests passed`，`vue-tsc`、生产 build 和差异检查通过；真实角色、诊断数据和执行器证据仍待环境复核。
- [x] 计划检查点 2.4.13：已完成任务中心失败证据的实现、回归、独立审查、问题修复和文档/记忆同步；外部顺序仍为受控 viewer/admin 角色矩阵 → 真实 AI/来源审计 → P8 目标治理 → P9 发布收口。
- [x] P1 API 工作台运行记录请求竞态：用例加载序列同时保护最近运行记录的成功、失败和空结果分支，旧模块响应不会覆盖最新模块结果；定向测试 `3 passed`，前端全量 `69 files / 305 tests passed`，`vue-tsc`、生产 build 和差异检查通过；真实角色和执行数据仍待环境复核。
- [x] 计划检查点 2.4.14：已完成 API 工作台竞态修复、回归、独立审查、问题修复和文档/记忆同步；外部顺序仍为受控 viewer/admin 角色矩阵 → 真实 AI/来源审计 → P8 目标治理 → P9 发布收口。
- [x] P1 API 工作台用例详情请求竞态：详情/编辑请求增加序列保护，关闭编辑、新建和保存会失效旧请求，避免迟到响应覆盖当前用例或重新打开表单；定向测试 `5 passed`，前端全量 `69 files / 307 tests passed`，`vue-tsc`、生产 build 和差异检查通过；真实角色和执行数据仍待环境复核。
- [x] 计划检查点 2.4.15：已完成 API 工作台详情竞态修复、回归、独立审查、问题修复和文档/记忆同步；外部顺序仍为受控 viewer/admin 角色矩阵 → 真实 AI/来源审计 → P8 目标治理 → P9 发布收口。
- [x] P1 API 工作台项目环境列表竞态：项目切换请求增加序列保护，旧项目环境的成功、失败和 loading 收尾不会覆盖最新项目，清空项目会失效旧请求；定向测试 `6 passed`，前端全量 `69 files / 308 tests passed`，`vue-tsc`、生产 build 和差异检查通过；真实角色和执行数据仍待环境复核。
- [x] 计划检查点 2.4.16：已完成 API 工作台环境列表竞态修复、回归、独立审查、问题修复和文档/记忆同步；外部顺序仍为受控 viewer/admin 角色矩阵 → 真实 AI/来源审计 → P8 目标治理 → P9 发布收口。
- [x] P1 API 工作台项目列表刷新竞态：刷新请求增加序列保护，旧项目列表响应/错误不会覆盖最新结果或重置项目；清空项目时用例请求也推进序列并停止 loading；定向测试 `8 passed`，前端全量 `69 files / 310 tests passed`，`vue-tsc`、生产 build 和差异检查通过；真实角色和执行数据仍待环境复核。
- [x] 计划检查点 2.4.17：已完成 API 工作台项目刷新竞态修复、回归、独立审查、问题修复和文档/记忆同步；外部顺序仍为受控 viewer/admin 角色矩阵 → 真实 AI/来源审计 → P8 目标治理 → P9 发布收口。
- [x] P1 UI 自动化工作台运行记录竞态：最近运行记录绑定用例加载序列，旧模块运行记录和旧详情不会覆盖最新模块；定向测试 `5 passed`，前端全量 `69 files / 311 tests passed`，`vue-tsc`、生产 build 和差异检查通过；真实角色和浏览器执行数据仍待环境复核。
- [x] 计划检查点 2.4.18：已完成 UI 工作台运行记录竞态修复、回归、独立审查、问题修复和文档/记忆同步；外部顺序仍为受控 viewer/admin 角色矩阵 → 真实 AI/来源审计 → P8 目标治理 → P9 发布收口。
- [x] P2 APP 工作台项目数据竞态：项目列表刷新和项目数据整组加载增加代际保护，设备/Worker、APK、Android 用例、专项任务/运行及 Android 运行记录的旧响应不会覆盖当前项目；新增项目切换延迟响应回归，定向 `4 passed`、前端全量 `69 files / 312 tests passed`，`vue-tsc`、生产 build 和差异检查通过；真实角色和 Android/Worker 执行数据仍待环境复核。
- [x] 计划检查点 2.4.19：已完成 APP 工作台项目数据竞态修复、回归、独立审查、问题修复和文档/记忆同步；外部顺序仍为受控 viewer/admin 角色矩阵 → 真实 AI/来源审计 → P8 目标治理 → P9 发布收口。
- [x] P8 N8 治理角色矩阵凭据契约：普通 viewer 支持短期 `ATP_VIEWER_TOKEN`，令牌优先并兼容 viewer 用户名/密码，认证值不写入证据；新增令牌路径回归，N8 定向 `7 passed`、后端非集成全量 `2387 passed`，Ruff/格式/发布契约 `19 passed` 和差异检查通过；目标部署真实角色证据仍待验收。
- [x] 计划检查点 2.4.20：已完成 N8 角色矩阵 token 支持、回归、独立审查、问题修复和文档/记忆同步；外部顺序仍为受控 viewer/admin 角色矩阵 → 真实 AI/来源审计 → P8 目标治理 → P9 发布收口。
- [x] P7 N7 智能中枢角色矩阵凭据契约：普通 viewer 支持短期 `ATP_VIEWER_TOKEN`，令牌优先并兼容 viewer 用户名/密码，认证值不写入证据；新增令牌路径回归，N7 定向 `12 passed`、后端非集成全量 `2389 passed`，Ruff/格式和差异检查通过；真实 viewer/admin、模型和目标部署证据仍待验收。
- [x] 计划检查点 2.4.21：已完成 N7 角色矩阵 token 支持、回归、独立审查、问题修复和文档/记忆同步；外部顺序仍为受控 viewer/admin 角色矩阵 → 真实 AI/来源审计 → P8 目标治理 → P9 发布收口。
