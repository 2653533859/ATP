...
- [~] C3 迁移、备份恢复、SLO、证据索引和最终 SHA 发布收口：C3.1～C3.3 已完成。目标 Linux 已切换 PostgreSQL 运行/迁移、Redis ACL 和 MinIO bucket 级身份；空库到 head、`0071 → 0072 → 0071 → 0072` 及最新 daily 备份隔离恢复均通过。演练发现 `--no-acl` 恢复后运行角色无权限，提交 `6755ed9f` 已让 Alembic 重建当前与默认授权并部署到 Helm revision 44；恢复库 65 表、4 项目、4 用户一致，运行角色可读/DML/使用序列但无 schema CREATE，6/6 Pod 在 40 秒 5 次采样内零重启、0 告警。Redis RDB-only、独立 MinIO/versioning/lifecycle、发布级 Prometheus/SLO 及最终 SHA/证据索引仍待 C3 收口。证据见 [`docs/evidence/c3-migration-restore-2026-09-12.json`](docs/evidence/c3-migration-restore-2026-09-12.json)。
