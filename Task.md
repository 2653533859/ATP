...
- [~] N2 测试资产闭环：按 N2.1 内部缺陷 → N2.2 失败证据 → N2.3 报告中心 → N2.4 用例评审 → N2.5 外部缺陷兼容推进，N2.1/N2.2/N2.3 已完成本地实现，当前开发入口为 N2.4。
...
当前推荐下一步：实现 N2.4 用例评审工作台，复用现有评审、评论、版本差异和记录能力，补齐待评审队列、批量评审和独立入口；N1/N2.1/N2.2/N2.3 真实环境验收作为并行环境证据补齐，不阻塞本地模块开发。每个模块完成后先审查和修复，再同步文档并提交推送。
