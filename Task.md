...
- [E] B1 工作台与角色矩阵：已完成权限感知的任务操作标记、三角色脱敏验收探针，以及任务中心深链/Viewer 按钮前端契约；提交 `5c0f6908` 的 Backend/Worker 已以不可变标签部署到 Linux K3s revision 12，Windows Vite 到 Linux Backend 传输已复核。目标环境当前缺少工程师账号、项目级 Viewer 成员关系和全部五域运行数据，待创建受控验收数据后完成 B1.3/B1.4 实测。
