...
- [~] C3 迁移、备份恢复、SLO、证据索引和最终 SHA 发布收口：C3.1/C3.2 已完成。目标 Linux 的 PostgreSQL 已分离未挂载的 bootstrap operator、DDL owner `atp_migrator` 和无 DDL 的 `atp_runtime`；Redis 使用受限 ACL 用户并覆盖实际 Celery queue/reply/pidbox/event 模式；MinIO 使用 bucket 级应用身份。Helm revision 43 的运行、迁移、operator Secret 互相分离，6/6 Pod Ready、零重启，业务读写、双 Worker、对象读写、真实定时备份及 40 秒稳定性均通过。Redis RDB-only、独立 MinIO/versioning/lifecycle、空库迁移/回滚演练及发布级 Prometheus/SLO 仍待 C3 收口。证据见 [`docs/evidence/c3-least-privilege-2026-09-12.json`](docs/evidence/c3-least-privilege-2026-09-12.json)。
