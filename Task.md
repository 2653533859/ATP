...
> 当前有效顺序：先完成 N1 的完整报告真实证据；Android Karing 因设备未确认真实包名保持阻塞，但不阻塞 N1；随后推进性能真实环境、通知、外部缺陷平台和发布收口。下方更早的历史记录不覆盖当前执行版。
...
- `[~]` N1 接口测试：HTTP、会话复用、gRPC TLS Unary、OpenAPI/Postman 导入预览/落库以及 GraphQL、WebSocket、三种流式 gRPC 受控目标已通过；完整报告导出/详情闭环仍待验收。
...
当前执行游标为 **N1 完整报告闭环 → N4～N9 外部收口**；N2 Karing 真机门禁并行保持阻塞，N0/N3 Windows 复核已由本轮完整 smoke 关闭。每个游标项都要完成实现、测试、代码审查、修复、文档/记忆同步和提交推送后再移动。

## 2026-08-25 当前开发计划登记

- [~] **N1 接口测试**：GraphQL、WebSocket、Server/Client/Bidi Streaming 已在 q19 受控目标完成真实创建、审批、执行、断言/提取和清理；下一步只剩完整报告导出/详情闭环。
