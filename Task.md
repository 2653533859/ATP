...
- [~] 计划检查点 2.4.39：单节点 K3s 首次 Helm 安装先暴露迁移 Hook 在普通 ConfigMap 创建前读取配置的问题，修复为内联 `.Values.config` 并仅从既有 Secret 读取敏感配置；重试时又确认 q19 依赖端口仅绑定宿主机回环，已增加单节点 overlay 的 `hostNetwork` 开关并将联调 Secret 端点切至 q19 回环端口。两项修复均已补回归测试，待推送后重试实际安装，复核迁移、核心 Pod 健康和短时稳定性；不关闭 P4/P9。
