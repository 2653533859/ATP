...
> 当前有效顺序（2026-08-25 1.0）：N4 本地性能验收入口已完成，下一步验证真实 Kubernetes/Prometheus/独立 MinIO；Android Karing 因设备未确认真实包名保持阻塞但不阻塞 N4；随后推进 N5-N8 外部依赖复核和 N9 发布收口。下方更早的历史记录不覆盖当前执行版。
...
当前执行游标为 **N4 真实环境验收 → N2 Karing 解阻 → N5-N8 外部依赖复核 → N9 发布收口**；N2 Karing 真机门禁并行保持阻塞，N0/N1/N3 Windows 复核已由本轮证据关闭。每个游标项都要完成实现、测试、代码审查、修复、文档/记忆同步和提交推送后再移动。
