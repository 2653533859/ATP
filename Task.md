...
主计划已更新至 [`docs/product-navigation-roadmap-2026-08-24.md`](docs/product-navigation-roadmap-2026-08-24.md) 的“0.4 参考导航下一阶段开发计划（当前跟踪版）”。目标导航与参考方案保持一致：工作台（首页、我的待办、项目中心、任务中心）、测试能力（接口/APP/UI/性能/AI）、测试资产（用例/计划/缺陷/报告/评审）、智能中枢（Hermes/需求与用例/知识）和系统（远程工具箱/配置中心）。旧资产和治理页面不从系统管理重复铺开，而是从对应工作台或配置中心进入并保持 URL 兼容。
...
本轮按照参考导航的五组结构继续推进：工作台、测试能力、测试资产、智能中枢、系统。导航入口和本地工作台已完成，不把入口存在误记为真实环境闭环；详细交付矩阵见 [`docs/product-navigation-roadmap-2026-08-24.md`](docs/product-navigation-roadmap-2026-08-24.md) 的“0.4 参考导航下一阶段开发计划（当前跟踪版）”。
...
- [~] A3 APP 自动化：ADB 基础检查通过，Agent/Backend 的 Redis 实例、DB、认证、队列和注册前缀已配对，Worker registry 和扫描回调已通过；下一步验证租约、控件属性、截图/录屏、APK 包名、低代码和专项任务。
...
1. 完成单设备 Android 租约、控件属性、截图/录屏、APK 包名、低代码、专项任务和结果回传，每个阶段记录执行事件。
...
- [ ] 解除条件：继续租约、控件属性、截图/录屏、APK 包名、Android 低代码、专项任务和结果回传；未完成前不把 Android 单设备闭环标记为完成。
