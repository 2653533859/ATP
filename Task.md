...
> 当前有效顺序（2026-08-25 2.0.0）：导航已统一为“工作台、测试能力、测试资产、智能中枢、系统”五组；N2 的 Karing 包名、启动入口、Windows Worker、设备扫描、低代码和录屏回传已通过，当前先完成异常回放、专项任务及事件/日志/报告闭环；N4 真实性能环境因缺少 Kubernetes/Prometheus/独立 MinIO 保持待环境验收，随后推进 N5-N8 外部依赖复核和 N9 发布收口。详细阶段、验收出口和状态口径以开发计划 2.0.0 为准，下方更早的历史记录不覆盖当前执行版。
...
- [~] 当前模块：N2 Karing 单设备已完成包身份、Worker 前置、低代码、截图和录屏回传；继续完成异常回放、性能/稳定性/流畅度专项任务、事件/日志/报告详情、下载和清理。
