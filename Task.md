...
- [x] 实现统计数据聚合接口（总览、通过率趋势、执行时长趋势、失败 Top 10）
...
- [ ] 套件/计划维度的通过率趋势（当前仅统计单用例 TestRun）
- [ ] 按触发方式（手动 / 定时 / Webhook）分类统计
- [ ] 按执行人维度统计执行次数排行
- [ ] 支持 case_type 筛选联动到前端 UI（当前后端已支持，前端未暴露）
...
- [ ] Redis 缓存加速：规则变更时同步到 Redis，Mock 请求优先查缓存
- [ ] 条件响应：根据请求参数（query/body）匹配不同响应（如 `id=1` 返回成功，`id=999` 返回 404）
- [ ] 响应模板：支持在响应体中引用请求参数（如 `{{request.body.name}}`）
- [ ] 批量导入/导出：支持 JSON 格式批量导入导出 Mock 规则
...
- [ ] 套件执行报告导出：`GET /suite-runs/{id}/export/html` 和 `/export/pdf`，聚合多用例结果
- [ ] 计划执行报告导出：`GET /plan-runs/{id}/export/html` 和 `/export/pdf`，多层嵌套套件+用例
...
- [ ] 截图附件上传：从 MinIO 读取步骤截图，作为附件上传到 Jira/禅道 Issue
- [ ] 自定义字段映射：支持配置 Jira 自定义字段（优先级、标签、组件等）
- [ ] 禅道多产品支持：配置中支持多产品切换
- [ ] 重复缺陷检测：创建前搜索同名 Issue，避免重复创建
- [ ] 连接测试：BugTracker 配置页添加"测试连接"按钮，验证凭据是否有效

### P2 - 高级功能

- [ ] 自动创建缺陷：测试计划执行完成后自动为所有失败用例创建缺陷（可配置开关）
- [ ] 缺陷状态同步：定期查询 Jira/禅道 Issue 状态，在平台内显示缺陷是否已修复
- [ ] GitHub Issues / GitLab Issues 集成：扩展第三方平台支持
