...
> 当前有效顺序（截至 2026-09-01，2.4.0）：参考导航按“工作台、测试能力、测试资产、智能中枢、系统”五组落地；P0 工作台、P1 接口、P2 APP、P3 UI 为 `[E]`（本地实现或受控证据完成，仍有对应真实边界复核），P4 性能为 `[-]`（缺 Kubernetes、发布级 Prometheus 和独立 MinIO），P5～P8 为 `[x]`（q19 受控真实验收完成），P9 为 `[~]`（等待 P4）。Hermes H1～H5 已完成本地交付；真实模型效果、角色、成本、审计和目标部署仍须独立复核。不得把页面可打开、mock 或单节点 Compose 写成真实通过；详细模块边界、依赖和验收出口以开发计划 2.4.0 总表为准。
...
- [x] Hermes 补齐 H2 持久会话、H3 只读工具审计、H4 人工确认后创建禁用计划草稿、H5 版本化 5 题评测集与 prompt/延迟/引用/拒答/反馈汇总治理面板；真实模型效果、成本 usage 与角色/部署证据仍保留环境门禁。
...
## 2026-08-25 当前开发计划 2.4.0 跟踪摘要（截至 2026-09-01 统一口径）
