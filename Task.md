...
- [~] B1 工作台与角色矩阵：B1.1～B1.3 已完成。提交 `5c0f6908` 的 Backend/Worker 运行于 Linux K3s revision 12；受控 Admin/Engineer/Viewer、项目 77/78 和五域各 2 条运行已建立，API 七项门禁与 Windows 本地三角色浏览器矩阵通过。当前 K3s Backend 入口为 `192.168.3.196:8000`，旧 q19 Docker `29080` 不作为本轮验收入口。B1.4 的五类重试、停止、批量操作、过期确认和状态收敛仍待实测，临时验收数据暂时保留。
- [E] B2～B4 真实能力闭环：UI 回放失败链路、接口对象生命周期和 Android 单设备持续运行。
