...
- [E] N3.4 性能测试工作台：性能场景、环境/节点发起、实时运行队列、吞吐/延迟/错误率、资源采样时间线、门禁、基线、趋势和报告导出已完成；q19 已验证专用性能 Worker、四类执行器、Prometheus 采样和一次真实 k6 运行，节点容量、多节点和生产报告治理仍待补充。
...
当前推荐下一步：恢复并完成 Android Worker/真机单设备证据，再验收真实通知供应商和外部缺陷平台；性能趋势、基线门禁和运行记录/报告清理本地闭环已完成，真实多节点/容量和生产存储治理仍待环境验收。N5.2 已统一聚合启动配置、环境、全局变量、AI、存储、通知和性能配置，N6.1～N6.13 已完成本地回归、代码审查、文档和 Windows/q19 API/Web/性能/通知/Android 配置证据；每个子模块完成后先审查和修复，再同步文档并提交推送。
...
- [ ] 后续：真实性能多节点、容量限制、生产 MinIO 生命周期和跨主机恢复环境验收。
...
主计划见 [`docs/product-navigation-roadmap-2026-08-24.md`](docs/product-navigation-roadmap-2026-08-24.md) 的“导航对齐执行版计划”。后续按“工作台 → 测试能力 → 测试资产 → 智能中枢 → 系统 → 质量发布”跟踪，不因为菜单已显示就把真实能力标记为完成：
...
- [~] **质量发布**：Windows API/Web 可用性和 q19 Web/性能证据已形成；Android 当前 ADB 为 `offline`，真实通知供应商和外部缺陷平台尚未验收，多节点性能/容量/长期趋势仍需增强。
...
- [ ] **P1-F 发布收口**：汇总上述证据，更新能力矩阵、操作手册、Q12/Q18 验收总结，记录 GitHub 分支保护限制。
