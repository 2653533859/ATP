...
> 当前有效顺序（2026-08-25 1.5）：N4 真实性能环境仍等待 Kubernetes/Prometheus/独立 MinIO；N2 已补齐 Windows Android 包名/启动入口探针、控件属性/坐标回退、长按/输入定位以及滑动分辨率适配，但仍需 Karing APK/真实包名和在线 Worker 做单设备闭环；随后推进 N5-N8 外部依赖复核和 N9 发布收口。下方更早的历史记录不覆盖当前执行版。
