...
- [~] 计划检查点 2.4.39：单节点 K3s 首次 Helm 安装暴露迁移 Hook 在普通 ConfigMap 创建前读取配置的问题；已将迁移 Job 改为内联 `.Values.config`、仅通过既有 Secret 读取敏感配置，并新增回归测试。待提交推送后重试实际安装，复核迁移、核心 Pod 健康和短时稳定性，再将本检查点收口；不关闭 P4/P9。
