...
- [~] C3 迁移、备份恢复、SLO、证据索引和最终 SHA 发布收口：C3.1～C3.3 已完成，C3.4 的持续采集与同机数据保护部分已落地。目标 Linux 新增独立持久化 Prometheus，15 天保留、Backend/双 Worker/自身 4/4 target 正常且 4 条规则执行健康；Redis 已从 RDB-only 在线切换为 AOF everysec + RDB，并通过隔离预演、Compose 重建和二次重启；MinIO bucket 已启用 versioning，生命周期因对象引用风险保持未配置。40 秒 5 次采样保持 6/6 Pod Ready、零重启、4/4 target、0 firing alert。C3.4 仍等待 7/14 天 SLO 历史和不同故障域 MinIO 备份，之后才进入 C3.5 最终 SHA/证据索引。证据见 [`docs/evidence/c3-release-observability-data-governance-2026-09-13.json`](docs/evidence/c3-release-observability-data-governance-2026-09-13.json)。
