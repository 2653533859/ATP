...
export interface WorkbenchBatchActionResult {
  ...
  failures: Array<{ task_type: WorkbenchTaskType; run_id: number; command_id?: string | null; detail: string }>
}
...
export const workbenchApi = {
  ...
  retry: (taskType: WorkbenchTaskType, runId: number, commandId = `workbench:retry:${taskType}:${runId}`) =>
    http.post<unknown, WorkbenchTaskActionResult>(`/workbench/tasks/${taskType}/${runId}/retry`, undefined, {
      headers: { 'Idempotency-Key': commandId },
    }),
  stop: (taskType: WorkbenchTaskType, runId: number, commandId = `workbench:stop:${taskType}:${runId}`) =>
    http.post<unknown, WorkbenchTaskActionResult>(`/workbench/tasks/${taskType}/${runId}/stop`, undefined, {
      headers: { 'Idempotency-Key': commandId },
    }),
  batchAction: (
    action: WorkbenchAction,
    tasks: Array<{ task_type: WorkbenchTaskType; run_id: number; command_id?: string }>,
  ) =>
