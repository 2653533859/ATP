...
LINT_SCRIPTS = scripts/scaffold-q12-evidence.py scripts/validate-q12-evidence.py scripts/collect-q12-evidence.py scripts/slo-traffic-canary.py scripts/pytest-standalone-sweep.py scripts/validate-deployment-readiness.py scripts/validate-android-worker-config.py scripts/performance-gate.py scripts/performance-environment-smoke.py scripts/performance_acceptance_target.py scripts/notification-channel-smoke.py scripts/notification-channel-acceptance.py scripts/web-recording-worker-smoke.py scripts/minio-dataset-acceptance.py scripts/minio-dr-acceptance.py scripts/b1-workbench-role-matrix.py scripts/n6-project-asset-acceptance.py scripts/n7-intelligence-acceptance.py scripts/n8-system-governance-acceptance.py scripts/validate-release-evidence.py
...
slo-traffic-canary:
	@if [ -z "$(API_BASE_URL)" ]; then \
		echo "Usage: export ATP_TOKEN=... (or ATP_USERNAME/ATP_PASSWORD); make slo-traffic-canary API_BASE_URL=http://... [ARGS='--project-id ... --case-id ... --confirm-case-run']"; \
		exit 2; \
	fi
	$(PYTHON) scripts/slo-traffic-canary.py --api-base-url "$(API_BASE_URL)" $(ARGS)
...
minio-dr-acceptance:
	@if [ -z "$(ARGS)" ]; then \
		echo "Usage: make minio-dr-acceptance ARGS='--report docs/evidence/minio-dr-YYYY-MM-DD.json [--require-lifecycle-rule tmp/=7]'"; \
		exit 2; \
	fi
	$(PYTHON) scripts/minio-dr-acceptance.py $(ARGS)
...
n6-project-asset-acceptance:
	@if [ -z "$(ARGS)" ]; then \
		echo "Usage: make n6-project-asset-acceptance ARGS='--base-url ... --allow-mutations --execute --target-url ... --require-role-matrix'"; \
		exit 2; \
	fi
	$(PYTHON) scripts/n6-project-asset-acceptance.py $(ARGS)

b1-workbench-role-matrix:
	@if [ -z "$(ARGS)" ]; then \
		echo "Usage: make b1-workbench-role-matrix ARGS='--base-url ... --project-id ... [--foreign-project-id ... --require-five-domains --verify-denials]'"; \
		exit 2; \
	fi
	$(PYTHON) scripts/b1-workbench-role-matrix.py $(ARGS)
...
n8-system-governance-acceptance:
	@if [ -z "$(ARGS)" ]; then \
		echo "Usage: make n8-system-governance-acceptance ARGS='--base-url ... [--require-role-matrix] [--allow-mutations --rollback --require-rollback]'"; \
		exit 2; \
	fi
	$(PYTHON) scripts/n8-system-governance-acceptance.py $(ARGS)

validate-release-evidence:
	$(PYTHON) scripts/validate-release-evidence.py $(ARGS)
