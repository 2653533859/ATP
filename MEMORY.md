...
- 2026-08-24 已将后续开发拆为 N2.1 内部缺陷、N2.2 失败证据关联、N2.3 报告中心、N2.4 用例评审工作台、N2.5 外部缺陷兼容；N2.1/N2.2 已完成本地实现、审查和回归，当前开发入口切换为 N2.3，真实数据库/失败执行/对象存储证据仍待环境验收。完整计划与验收口径见 `docs/product-navigation-roadmap-2026-08-24.md`。
...
- 2026-08-24 当前状态：N0 基础导航已实现为五层菜单（工作台、测试能力、测试资产、智能中枢、系统）；N1 工作台/任务中心与 N2.1/N2.2 内部缺陷/失败证据闭环已完成本地实现、代码审查和回归，均标记为“已实现，待真实环境验收”。Hermes、知识中枢、统一报告中心、用例评审和外部缺陷映射仍未完整实现。
- 2026-08-24 N0/N1/N2.1/N2.2 回归：前端 Vitest `53 files / 214 tests passed`，后端非集成 `2142 passed`；type-check、生产 build、ruff、迁移回归和 `git diff --check` 通过。N2.1/N2.2 新增项目级内部缺陷、失败运行证据脱敏与运行/缺陷双向关联；审查修复了失败状态限制、跨项目用例校验、嵌套敏感字段脱敏和 `/bugs` 导航契约问题。
- 2026-08-24 文档跟踪规则：代码完成但未联调必须标记“已实现，待环境验收”；每阶段先审查和回归，再同步 `Task.md`、`MEMORY.md` 与路线文档；旧 URL 和已有 API 保持兼容。
- 2026-08-24 导航路线跟踪：主计划为 `docs/product-navigation-roadmap-2026-08-24.md`；N1 已进入环境验收，N2.1/N2.2 已完成本地闭环，当前按 N2.3 测试报告中心推进。单模块必须完成实现、测试、代码审查、问题修复、文档/记忆同步后才可提交推送；不得把工作区中的半成品标记为完成。
