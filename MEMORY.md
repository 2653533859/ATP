...
- 2026-08-24 N2.5 外部缺陷兼容已完成本地实现、代码审查和回归：新增 `defect_external_links` 与迁移 `20260824_0061`，内部缺陷支持 Jira/禅道/GitHub/GitLab 的手工映射、外部 Issue 创建、重复查询、状态同步和可选内部状态回写；创建内容脱敏、外部 URL 安全校验、错误信息不泄露凭据。缺陷详情支持关联卡片和操作弹窗，查看者仅可读。后端非集成 `2155 passed`，前端 `55 files / 220 tests passed`，缺陷详情定向 `3 passed`，type-check/build/ruff/diff-check 通过；真实外部平台凭据、网络、项目权限和状态差异仍待环境验收。下一开发入口为 N3 API 测试工作台。
...
- 2026-08-24 已将后续开发拆为 N2.1 内部缺陷、N2.2 失败证据关联、N2.3 报告中心、N2.4 用例评审工作台、N2.5 外部缺陷兼容；N2.1/N2.2/N2.3/N2.4/N2.5 已完成本地实现、审查和回归，当前开发入口切换为 N3 API/APP/UI/性能工作台，真实数据库/权限/套件计划联动/审计、外部平台和生产数据仍待环境验收。完整计划与验收口径见 `docs/product-navigation-roadmap-2026-08-24.md`。
...
- 2026-08-24 当前状态：N0 基础导航已实现为五层菜单（工作台、测试能力、测试资产、智能中枢、系统）；N1 工作台/任务中心与 N2.1/N2.2/N2.3/N2.4/N2.5 测试资产闭环已完成本地实现、代码审查和回归，均标记为“已实现，待真实环境验收”。Hermes、知识中枢和 N3 测试能力工作台仍未完整实现。
...
- 2026-08-24 导航路线跟踪：主计划为 `docs/product-navigation-roadmap-2026-08-24.md`；N1/N2.1/N2.2/N2.3/N2.4/N2.5 已进入环境验收，当前下一入口为 N3 API 测试工作台。单模块必须完成实现、测试、代码审查、问题修复、文档/记忆同步后才可提交推送；不得把工作区中的半成品标记为完成。
