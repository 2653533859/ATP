...
- 2026-08-25 计划检查点 2.4.20 已同步：N8 角色矩阵 token 契约完成本地实现、测试、独立审查、问题修复和文档同步；下一项仍是提供受控 viewer/admin 真实角色矩阵，再进入真实 AI 与 P8 目标治理复核。P4 仍因缺少 Kubernetes、发布级 Prometheus 和独立 MinIO 阻塞。
- 2026-08-25 P2 APP 自动化工作台项目数据竞态修复本地交付已完成：项目刷新和项目切换使用代际序列，设备/Worker、APK、Android 用例、专项任务/运行和 Android 运行记录的迟到成功、失败、空结果与 loading 收尾不会覆盖当前项目；清空项目会失效旧请求，新增项目切换延迟响应回归。定向 `4 passed`、前端全量 `69 files / 312 tests passed`，类型检查、生产 build 和 diff-check 通过；真实角色和 Android/Worker 执行数据仍待环境复核。
- 2026-08-25 计划检查点 2.4.19 已同步：APP 工作台项目数据竞态完成本地实现、测试、独立审查、问题修复和文档同步；下一项仍是受控 viewer/admin 真实角色矩阵，再进入真实 AI 与 P8 目标治理复核。P4 仍因缺少 Kubernetes、发布级 Prometheus 和独立 MinIO 阻塞。
- 2026-08-25 P1 UI 自动化工作台运行记录竞态修复本地交付已完成：最近运行记录绑定用例加载序列，旧模块运行记录和旧详情不会覆盖最新模块；详情成功/失败/loading 收尾也校验当前序列，新增延迟响应回归。定向 `5 passed`、前端全量 `69 files / 311 tests passed`，类型检查、生产 build 和 diff-check 通过；真实角色和浏览器执行数据仍待环境复核。
- 2026-08-25 计划检查点 2.4.18 已同步：UI 工作台运行记录/详情竞态完成本地实现、测试、独立审查、问题修复和文档同步；下一项仍是受控 viewer/admin 真实角色矩阵，再进入真实 AI 与 P8 目标治理复核。P4 仍因缺少 Kubernetes、发布级 Prometheus 和独立 MinIO 阻塞。
- 2026-08-25 P1 API 工作台项目刷新竞态修复本地交付已完成：项目列表刷新增加序列保护，旧响应/错误不会覆盖最新项目；清空项目时用例列表推进序列并停止 loading，旧项目用例响应不会回写；新增两组回归。定向 `8 passed`、前端全量 `69 files / 310 tests passed`，类型检查、生产 build 和 diff-check 通过；真实角色和执行数据仍待环境复核。
- 2026-08-25 计划检查点 2.4.17 已同步：API 工作台项目刷新竞态完成本地实现、测试、独立审查、问题修复和文档同步；下一项仍是受控 viewer/admin 真实角色矩阵，再进入真实 AI 与 P8 目标治理复核。P4 仍因缺少 Kubernetes、发布级 Prometheus 和独立 MinIO 阻塞。
- 2026-08-25 P1 API 工作台项目环境列表竞态修复本地交付已完成：项目序列保护环境列表请求的成功、失败和 loading 收尾，快速切换或清空项目时旧响应不会覆盖当前状态；新增连续切换延迟响应回归。定向 `6 passed`、前端全量 `69 files / 308 tests passed`，类型检查、生产 build 和 diff-check 通过；真实角色和执行数据仍待环境复核。
- 2026-08-25 计划检查点 2.4.16 已同步：API 工作台项目环境列表竞态完成本地实现、测试、独立审查、问题修复和文档同步；下一项仍是受控 viewer/admin 真实角色矩阵，再进入真实 AI 与 P8 目标治理复核。P4 仍因缺少 Kubernetes、发布级 Prometheus 和独立 MinIO 阻塞。
- 2026-08-25 P1 API 工作台用例详情竞态修复本地交付已完成：详情和编辑详情请求增加序列保护，快速切换、关闭编辑、新建和保存完成都会失效旧请求，迟到成功/失败/loading 收尾不会覆盖当前 UI 或重新打开表单；新增两组回归。定向 `5 passed`、前端全量 `69 files / 307 tests passed`，类型检查、生产 build 和 diff-check 通过；真实角色和执行数据仍待环境复核。
- 2026-08-25 计划检查点 2.4.15 已同步：API 工作台详情竞态完成本地实现、测试、独立审查、问题修复和文档同步；下一项仍是受控 viewer/admin 真实角色矩阵，再进入真实 AI 与 P8 目标治理复核。P4 仍因缺少 Kubernetes、发布级 Prometheus 和独立 MinIO 阻塞。
- 2026-08-25 P1 API 工作台最近运行记录竞态修复本地交付已完成：用例加载序列传递到运行记录请求，旧的成功响应、错误和空结果不会覆盖最新项目、模块或筛选结果；新增延迟响应回归。定向 `3 passed`、前端全量 `69 files / 305 tests passed`，类型检查、生产 build 和 diff-check 通过；真实角色和执行数据仍待环境复核。
- 2026-08-25 计划检查点 2.4.14 已同步：API 工作台竞态保护完成本地实现、测试、独立审查、问题修复和文档同步；下一项仍是受控 viewer/admin 真实角色矩阵，再进入真实 AI 与 P8 目标治理复核。P4 仍因缺少 Kubernetes、发布级 Prometheus 和独立 MinIO 阻塞。
...
- 2026-08-25 P0 任务中心失败证据展示本地交付已完成：诊断弹窗展示失败步骤数、截图数、结构化错误样本和截图入口；前端错误样本类型明确，截图链接只允许 HTTP(S) 协议并带 `noopener noreferrer`，不安全地址不渲染。定向 `4 passed`、前端全量 `69 files / 304 tests passed`，类型检查、生产 build 和 diff-check 通过；真实角色、失败运行数据和执行器证据仍待环境复核。
- 2026-08-25 计划检查点 2.4.13 已同步：任务中心失败证据展示完成本地实现、测试、审查、问题修复和文档同步；下一项仍是受控 viewer/admin 真实角色矩阵，再进入真实 AI 与 P8 目标治理复核。P4 仍因缺少 Kubernetes、发布级 Prometheus 和独立 MinIO 阻塞。
...
- 2026-08-25 q19 N8 目标账号只读核对：仅查询用户 ID、用户名、角色和启用状态，确认启用管理员用户名为 `parado`；未读取密码字段。由于受控登录仍返回 HTTP 401，未继续猜测、未执行配置/回滚/审计变更，也未记录凭据或响应正文；完整 N8 仍需注入可验证的管理员和普通 viewer 凭据。
- 2026-08-25 N1 任务中心分页本地交付：统一任务 API 增加有界 `offset`，先合并五类任务域再按创建时间切页；前端增加服务端分页，项目/状态/类型/页码进入 URL，筛选自动回到第一页。工作台定向 `15 passed`、后端非集成 `2380 passed`、前端全量 `69 files / 302 tests passed`，type-check/build/Ruff/diff-check 通过；审查修复了路由筛选初始化顺序、前后端字段不一致和分页提示文案。真实角色、任务数据和执行器环境仍待验收。
...
- 2026-08-25 P7/N6 真实验收缺陷已定位并修复：blank 临时项目不会自动生成模块，N6/N7 脚本现在显式创建并测试临时模块；q19 实际验收继续后发现 `POST /requirements` 在 async commit 后读取过期 ORM 属性，触发 `MissingGreenlet`/HTTP 500，接口已在响应前 `refresh`，需求路由回归通过。该阶段失败后的临时项目已清理；后续 q19 重建、清理级联修复和基础链路复验见本文件首条记录，未记录凭据或响应正文。

- 2026-08-25 导航第三轮开发计划已同步为 2.4.0，当前检查点为 2.4.5：参考导航固定为工作台、测试能力、测试资产、智能中枢、系统五组，阶段统一为 P0 工作台、P1 接口、P2 APP、P3 UI、P4 性能、P5 AI、P6 测试资产、P7 智能中枢、P8 系统和 P9 发布收口。P7 q19 基础需求/知识数据链路已复验，下一项先做普通 viewer 角色矩阵，再做受控真实 AI 草稿和 P8 目标部署复核；P4 因缺少 Kubernetes、发布级 Prometheus 和独立 MinIO 保持阻塞。计划、任务、路线图和发布状态已同步，未记录凭据；所有模块仍遵循实现→测试→独立审查→修复→文档/记忆同步→提交推送闭环，真实环境未验证不得从 `[E]` 写成通过。

- 2026-08-25 N6 项目资产与角色矩阵验收工具已完成本地交付：新增 `scripts/n6-project-asset-acceptance.py`，按临时项目→模块→API 用例→评审→套件→计划→执行/报告→缺陷→清理验证链路，支持可选 viewer 角色隔离；`--allow-mutations` 与 `--execute` 显式授权，凭据只读环境变量，错误响应正文不入证据，清理在 finally 中执行并验证删除后 404。脚本定向 `6 passed`、质量门禁一致性 `10 passed`，Makefile、CI、pre-commit、运行手册和 N6 发布证据索引已同步。真实角色、执行报告、外部缺陷和可清理数据仍待受控环境验收，未记录凭据。

- 2026-08-25 N6 工作台任务详情上下文已完成本地交付：工作台聚合的 case、suite、plan、Android、performance 详情地址统一保留所属 `project_id`；suite/plan/performance 额外携带 `run_id`，从任务中心可直接定位具体执行记录，避免默认项目或列表级跳转丢失证据。工作台 API 定向 `14 passed`，后端非集成全量 `2352 passed`；独立审查修复了数据集影响范围静态契约对旧无 `query` 路由的断言，相关回归 `17 passed`，Ruff/差异检查通过。真实项目角色、跨项目可见性、可清理运行和报告环境仍待 N6 验收，未记录凭据。

- 2026-08-25 N6 用例评审打开详情项目上下文已完成本地交付：评审工作台打开案例详情时携带记录自身 `project_id`，案例详情可加载项目环境并保留返回筛选；评审工作台定向 `3 passed`、前端全量 `69 files / 300 tests passed`，`vue-tsc`、生产构建和独立审查通过。真实项目角色、跨项目可见性和可清理评审数据仍待 N6 验收，未记录凭据。

- 2026-08-25 N6 数据集影响范围项目上下文已完成本地交付：数据集影响范围中的案例/套件/计划跳转保留当前 `project_id`，案例详情可加载项目环境并返回项目筛选，避免回到默认项目或混入其他项目；数据集定向 `9 passed`、前端全量 `69 files / 299 tests passed`，`vue-tsc`、生产构建通过，独立审查已补齐案例入口上下文。真实项目角色、跨项目可见性和可清理数据仍待 N6 验收，未记录凭据。

- 2026-08-25 N6 计划报告套件明细导航已完成本地交付：计划运行报告的套件明细新增查看详情入口，跳转时保留 `project_id` 和 `suite_run_id`，套件页按项目过滤并展开对应运行；缺失或非法运行 ID 不触发导航。计划列表定向 `7 passed`、前端全量 `69 files / 298 tests passed`，`vue-tsc`、生产构建和独立审查通过；真实项目角色、可清理失败运行和报告环境仍待 N6 验收，未记录凭据。

- 2026-08-25 N6 缺陷证据跨类型报告导航已完成本地交付：case 缺陷进入运行详情，Android 进入专项报告，性能进入性能工作台并选中指定运行，suite/plan 通过 `run_id` 解析所属资源后打开执行记录抽屉；suite/plan 深链同时携带项目上下文，避免默认项目导致证据丢失。前端定向 `23 passed`、全量 `69 files / 297 tests passed`，`vue-tsc`、生产构建和独立审查通过；真实项目角色、可清理失败运行和报告环境仍待 N6 验收，未记录凭据。

- 2026-08-25 N6 缺陷状态刷新项目隔离已完成本地交付：刷新 case 执行记录关联缺陷状态前，先解析用例/模块所属项目并校验 viewer 权限；已保存 tracker 必须属于同一项目，跨项目配置在调用外部平台前拒绝，避免仅凭 run_id 越权读取外部 Issue 状态。新增无权限和跨项目 tracker 回归，缺陷跟踪 API 定向 `11 passed`，独立审查和差异检查通过。真实项目角色矩阵、外部缺陷平台和可清理失败运行仍待 N6 环境验收；未记录凭据或外部响应正文。
...
- 2026-08-25 N9 发布门禁证据索引与一致性校验子模块已完成本地交付：新增脱敏 `docs/release-evidence-index-2026-08-25.json`、`scripts/validate-release-evidence.py` 及 Make/CI/预提交入口；未关闭门禁必须记录原因、依赖、负责人、复验命令和仓库内证据路径，敏感字段、重复门禁、状态不一致和候选 SHA/HEAD 不一致都会失败。定向校验与发布契约 `22 passed`、后端非集成全量 `2333 passed`、Ruff/格式/差异检查通过；N4-N8 真实门禁和 N9 整体发布收口仍待环境复核，未写入任何凭据。

- 2026-08-25 N9.2 发布文档同步一致性契约已完成本地交付：发布索引同时校验开发计划、`Task.md`、`MEMORY.md`、路线图、发布状态和 `docs/q9-release-checklist.md` 的路径及关键标记；只要文件缺失、不是 UTF-8 文本或标记漂移，发布校验就失败。定向校验与发布契约 `23 passed`、后端非集成全量 `2334 passed`，真实 N4-N8 门禁仍待环境复核，未写入任何凭据。

- 2026-08-25 N6 失败运行转内部缺陷入口已完成本地交付：Android 专项报告详情和性能压测详情在失败、异常、取消或停止状态显示一键创建内部缺陷；创建后展示关联缺陷，支持跳转缺陷管理并按 `run_type/run_id` 过滤。重复指纹继续由后端追加运行证据，不在前端保存运行日志、凭据或预签名地址；性能详情切换运行会先清空旧缺陷，避免关联串行。性能定向 `13 passed`，缺陷 API/静态契约 `14 passed`，前端全量 `69 files / 292 tests passed`，后端非集成全量 `2326 passed`，类型检查、生产构建、Ruff、格式、差异检查和独立审查通过。真实项目角色和真实失败运行仍待 N6 环境复核，状态保持 `[E]`。

- 2026-08-25 N5/N6 AI 用例来源追踪摘要已完成本地交付：AI 生成响应返回配置 ID、供应商、模型名、接口数量、数据集/版本、去重 Mock 规则 ID 和 UTC 生成时间；保存草稿时写入用例 `config._ai_source`，详情页展示来源，审计同步写入模型摘要。未保存 Endpoint、提示词、API Key 或原始响应；历史仅含数据集/Mock 的 AI 用例保持兼容。后端变更测试独立 `2 passed`、非集成全量 `2323 passed`，前端全量 `69 files / 289 tests passed`，类型检查、生产构建、Ruff、格式、差异检查和独立审查通过。真实模型、真实项目权限和 N6 全链路仍待受控环境复核，N5/N6 保持 `[E]`。

- 2026-08-25 N5 AI 生成失败与原始响应安全边界已完成本地交付：AI 用例、测试数据集、Mock 规则和模型列表的 HTTP/超时/网络错误不再返回供应商响应正文或异常字符串；用例接口只返回 HTTP 状态、固定网络/超时提示，并继续记录脱敏失败审计。`raw_response` 最多 12,000 字符，经过 JSON 敏感字段、键值、URL 查询密钥和 URL 用户信息脱敏。新增回归覆盖错误正文、网络错误、原始响应和长度限制；AI 用例/治理定向 `21 passed`，数据集/Mock/LLM 相关回归 `78 passed`，后端非集成全量 `2322 passed`，Ruff、差异检查和独立审查通过。真实供应商错误格式、模型返回内容和项目级生成仍需受控环境复核，N5 保持 `[E]`，未记录任何密钥。

- 2026-08-25 N5 模型能力提示与思考参数快捷配置已完成本地交付：AI 配置页默认关闭思考，可选择 `thinking=true`、`enable_thinking=true` 或 `reasoning_effort` low/medium/high，并同步高级 JSON；手工 JSON 未使用快捷选择时保持兼容。拉取模型后提示思考和多模态能力为已识别、未识别或未知，不自动替用户开启思考；切换供应商会清除快捷选择但不静默改写高级参数。前端定向 `5 passed`、全量 `69 files / 289 tests passed`，类型检查、生产构建、差异检查和独立审查通过。能力提示仍依赖供应商返回/模型名推断，真实模型参数接受情况需受控环境复核。

- 2026-08-25 N5 AI 模型连接健康检查已完成本地交付：新增管理员接口和配置页“测试连接”按钮，编辑配置时复用 Fernet 密钥，新建 Ollama 允许空 API Key，跨供应商不复用旧密钥。检查请求使用固定短文本、15 秒超时和 4 token 上限，保留受支持的 thinking/reasoning 参数但不允许配置覆盖健康检查的温度/token；Endpoint 校验后使用规范化值，错误不返回供应商原始响应/密钥，成功只返回模型、耗时并写安全审计摘要。后端配置定向 `15 passed`、非集成全量 `2317 passed`，前端配置页 `3 passed`、全量 `69 files / 287 tests passed`，类型检查、生产构建、Ruff、格式和差异检查通过。N5 仍为 `[E]`：真实模型连接、多模态/思考参数和项目生成需要受控环境复核，未记录任何密钥。
...
- 2026-08-25 参考导航后续开发计划已登记并同步：侧栏按“工作台、测试能力、测试资产、智能中枢、系统”五组组织，N0-N9 分别跟踪导航壳、API、APP、UI、性能、AI、测试资产、智能中枢、系统和发布收口。N2 Karing 单设备闭环已完成，当前执行游标转为 N4 真实性能环境；N4 因缺少 Kubernetes、Prometheus 和独立 MinIO 保持待环境验收，N5-N8 外部依赖复核和 N9 发布收口后置。每个模块固定执行“实现/调整 → 定向测试 → 受影响全量门禁 → 独立代码审查 → 修复 → 文档与记忆同步 → 提交推送”；详细台账以 `docs/development-plan-2026-08-25.md` 2.1.0 为准，任务勾选见 `Task.md`，路线图和发布边界同步更新。此次仅同步计划文档，不写入凭据或新增业务代码。

- 2026-08-25 N2 Karing 低代码与录屏回传已通过真实单设备验收：目标设备 `172.16.102.91:5555`、包 `com.nebula.karing`，临时已审批用例 `run 27` 完成启动/等待/截图 3/3，返回 3 张步骤截图和 2 个 Android 产物；临时项目已清理。代码审查发现原 `-RequireAndroidEvidence` 只接受任意截图/附件，可能掩盖录屏缺失，已在 `scripts/windows-local-smoke.ps1` 增加 `-RequireAndroidRecording`，要求 `result_summary.android_artifacts.screen_recording` 存在并在报告输出 `recording=True/False`；脚本契约 `12 passed`、PowerShell 解析通过。真实 `run 29` 开启录屏后 3/3 步骤、3 张截图、3 个产物且 `recording=True`，临时项目已清理，脱敏证据为 `docs/evidence/android-karing-lowcode-2026-08-25.json` 和 `docs/evidence/android-karing-recording-gate-2026-08-25.json`。异常回放、专项任务、事件/日志/报告和最终清理闭环仍待继续；未写入凭据。
- 2026-08-25 N2 Karing 单设备前置门禁历史快照：在明确目标设备 `172.16.102.91:5555` 上以设备包管理确认 `com.nebula.karing` 已安装，入口为 `com.nebula.karing/.MainActivity`；Windows Android Worker doctor、Backend 登录、PostgreSQL/Redis/MinIO readiness、Worker registry 和设备扫描均通过。脱敏证据为 `docs/evidence/android-karing-acceptance-2026-08-25.json` 与 `docs/evidence/windows-android-karing-worker-2026-08-25.json`。该条只记录包身份、启动入口和 Worker 前置，不代表后续低代码、录屏/异常回放、专项任务、事件/日志/报告或清理通过；当前进度以本文件顶部的最新 N2 记录为准。凭据未写入代码、文档或证据。
- 2026-08-25 已建立参考导航对齐开发计划 2.0：产品入口统一为工作台（首页/待办/项目中心/任务中心）、测试能力（接口/APP/UI/性能/AI）、测试资产（用例/计划/缺陷/报告/评审）、智能中枢（Hermes/需求与用例生成/知识中枢）和系统（远程工具箱/配置中心）。旧设备、APK、Mock、数据集、Web/API 资产 URL 保持兼容，但不再全部放在系统管理下。当前台账为 N0-N1/N3/N5-N8 本地实现待真实边界复核，N2 Karing 的包身份/Worker 前置已通过但单设备业务闭环仍待执行，N4 Kubernetes/Prometheus/独立 MinIO 仍待外部条件，N9 依赖其收口；详细范围、验收出口和跟踪规则见 `docs/development-plan-2026-08-25.md` 2.0，已同步 `Task.md`、路线图和发布状态。本轮同步了最新脱敏环境证据，未新增业务代码。

- 2026-08-25 N2 Android 控件属性获取诊断已完成本地交付：UIAutomator 目标查询在 API 进程和 Windows Android Worker 两条路径统一返回 `found`/`not_found`/`unavailable` 与脱敏诊断码；录制界面对 Worker/设备/UIAutomator 不可用显示提示，但仍保存坐标步骤，未命中语义控件不打扰坐标回退。代码审查发现并修复旧 Worker 缺少诊断字段时不应返回 `diagnostic: null` 的兼容问题。后端定向 `22 passed`、前端相关定向 `23 passed`、后端非集成全量 `2305 passed`、前端全量 `69 files / 284 tests passed`，类型检查、生产构建、Ruff、格式检查和差异检查通过。真实 Karing 页面、UIAutomator 权限和 Windows Worker 真机回传仍待 N2 环境门禁；计划已同步到 `docs/development-plan-2026-08-25.md` 2.1、`Task.md`、路线图和发布状态。
...
- 2026-08-25 Windows Android 包名与启动入口验收探针已完成：`scripts/windows-android-acceptance.ps1` 新增 `-LaunchActivity`，指定 `-AppPackage` 后先检查 `pm path`，再通过 `cmd package resolve-activity --brief` 解析显式组件或 `MAIN/LAUNCHER` 默认入口；报告只保存包名、请求/解析组件和状态，不启动/修改应用，不保存 APK 内容或日志正文。脚本契约 `2 passed`，脚本目录 `93 passed`，质量/发布文档回归 `15 passed`，PowerShell 语法检查通过；当前两台在线设备使用 `com.android.settings` 完成自动 Launcher 与显式 `.Settings` 只读探针。该设备探针不替代 Karing：当前仍未发现 Karing，真实 APK/Manifest 包名、Worker 调度、低代码、录屏/异常回放、专项任务和报告仍待验收。计划已同步到 `docs/development-plan-2026-08-25.md` 1.2、`Task.md`、路线图、发布状态和 `docs/windows-local-run.md`。

- 2026-08-25 Android 专项应用启动兼容本地交付已完成：新增 `build_android_launch_command`/`launch_android_app` 统一入口，性能、稳定性和流畅度执行器在明确配置 Activity 时使用 `am start -n`，未配置时使用 Launcher Intent 自动发现，不再假定 APK 入口为 `.MainActivity`；流畅度执行器尊重前置启动设置的 `auto_start=false`，避免重复启动；专项任务表单空 Activity 不再保存 `.MainActivity`，旧任务显式 Activity 保持兼容。代码审查补充了 Launcher、显式 Activity、启动失败和前置启动不重复回归；后端非集成 `2295 passed`，四个受影响测试文件独立 `3/25/19/15 passed`，前端全量 `67 files / 275 tests passed`，`vue-tsc`、生产构建、Ruff、`git diff --check` 通过。真实 Karing APK/包名、Windows Android Worker/ADB、专项媒体和报告仍待环境验收，不能用其他应用替代；下一入口为 Karing 单设备闭环，N4 Kubernetes/Prometheus/独立 MinIO 仍是独立外部环境门禁。计划同步到 `docs/development-plan-2026-08-25.md` 1.1、`Task.md`、路线图和发布状态。
...
- 2026-08-25 N1 完整报告闭环已完成：提交 `86f3bf7` 在运行详情页补齐 JUnit XML 导出入口，q19 临时项目 `42` / 用例 `27` / 运行 `26` 通过详情查询与 HTML `200/2561 bytes`、JUnit XML `200/220 bytes`（可解析）、PDF `200/167056 bytes` 导出，删除项目 `204` 后清理匹配数为 `0`。前端定向 `11 passed`、后端报告/导出 `24 passed`、前端全量 `67 files / 275 tests passed`，类型检查、生产构建、diff-check 和独立审查通过；证据为 `docs/evidence/report-closure-2026-08-25.json`。N1 受控协议/报告游标关闭，下一步切换 N4 性能真实环境；生产协议服务、生产对象存储和外部发布环境仍保持独立门禁。
...
- 2026-08-25 发现 q19 Backend 启动缺陷：管理员被改名后仍占用默认邮箱，`backend/app/main.py::_init_admin` 只按 `FIRST_ADMIN_USERNAME` 查询，随后尝试插入重复邮箱并触发 `users_email_key`，容器进入重启循环。下一代码动作是按用户名或邮箱幂等查询并补回归测试；不在仓库或证据中记录账号密码。
...
- 2026-08-25 Android 单设备真实执行复核：在 `172.16.102.214:5555` 使用包管理确认存在的 `com.microsoft.emmx` 完成临时低代码运行，3/3 步骤、3 个步骤截图、设备信息、logcat 和录屏均成功，临时项目/用例/运行已清理；Karing 仍未出现在两台设备包列表中。上传约 262 MB APK 时 q19 MinIO 5 秒读超时导致 HTTP 500，根因已定位并修复为独立 `MINIO_READ_TIMEOUT_SECONDS=60` 与 `PoolManager(maxsize=10, block=True)`；待最新 q19 重建后复验 APK 上传/包名解析和对象清理。
...
- 2026-08-25 工作台任务状态枚举隔离修复已完成：q19 日志确认 `/workbench/overview` 将 Android `stopped`、性能 `cancelled` 与普通 `TestRun`/套件/计划状态混用，触发 PostgreSQL `runstatus` 枚举错误。`backend/app/api/v1/workbench.py` 现在按五个任务域过滤状态，空交集使用无匹配条件，重试状态也恢复按域判断（含 case `skipped`）。工作台定向 `8 passed`，完整后端非集成 `2229 passed`，Ruff/diff-check 通过；未改执行器、权限或迁移。q19 已按 `36cacb9` 受控重建，迁移 `20260824_0065`、Backend `200`、Prometheus 4 targets `up`、Celery 2 节点在线，重启后未出现新的 enum 错误；脱敏证据见 `docs/evidence/q19-workbench-status-filter-2026-08-25.json`。鉴权工作台请求仍因缺少当前有效账号未执行，不能把 `401` 或无凭据状态记为完整聚合通过。
...
- 2026-08-25 P0-B.3.5 事件、日志与报告回传本地交付已完成：新增 `backend/app/services/mobile_special_artifacts.py`，Android 专项 Worker 在执行成功、失败或前置异常后的公共收尾阶段按配置采集最多 10000 行 logcat（5 MB）与结束 PNG 截图（10 MB），上传成功写入 `MobileRunArtifact`，并在 `summary_json.android_artifacts` 记录 `requested/saved/file_name/file_size/error`；ADB/截图/上传失败只记录告警，不覆盖原始运行终态。专项任务配置页新增设备日志/结束截图开关，报告详情展示产物状态，既有性能 CSV、录屏、异常日志和异常回放兼容。`MobileRunEventRecorder` 对参数、结果、消息和 JSON 报告的运行摘要/配置快照统一脱敏 Authorization、Cookie、密码、Token、Secret、API Key、URL 查询凭据；审查修复事件达到 5000 条上限时产物摘要未提交的边界，并补充上传错误摘要脱敏。定向 84 项、后端非集成 `2260 passed`，前端 `66 files / 269 tests passed`，type-check/build/Ruff/diff-check 通过；脱敏证据为 `docs/evidence/android-event-artifact-reporting-2026-08-25.json`。真实 APK、在线 `device`、MinIO 上传/下载/清理、三类专项结束日志/截图和性能录屏/异常回放仍待真实环境验收，不能把本地证据写成 Android 发布通过。
- 2026-08-25 Android APK 上传/包名验收：q19 按 `e1dc113` 重建后，262,615,229 字节的已安装 `com.microsoft.emmx` APK 上传成功，标准 `ResXMLTree` Manifest 解析出 `package_name=com.microsoft.emmx`、`version_name=150.0.4078.81`、`version_code=407808105`；项目对象绑定、列表回读和临时 APK/项目/MinIO 对象清理通过。修复 `backend/app/services/apk_metadata.py` 对 XML 外层包装块和标准 `RES_STRING_POOL_TYPE=0x0001` 的兼容，APK 元数据/API 定向 `17 passed`。脱敏证据为 `docs/evidence/android-apk-upload-2026-08-25.json`。当前两台设备包列表仍没有 Karing，Karing 专项动作、APK 下载端点和完整报告下载仍待真实包验证。
- 2026-08-25 Windows API/Web 完整 smoke：当前有效账号下 `scripts/windows-local-smoke.ps1 -SeedWebDownloadCase -RequireWebLowcode -RequireWebDownload` 首次暴露空历史数据库的报告导出顺序缺陷；修复为先创建/执行临时 Web run，再导出 HTML/JUnit。修复后 Backend/Frontend 200、Cookie 登录、PostgreSQL/Redis/MinIO readiness、Web Worker `registered=1 available=1`、Playwright `12 passed`、浏览器矩阵零失败请求/错误响应、47 bytes 文件上传/清理、Web 下载低代码 run 9、HTML/JUnit `18715/317` bytes、临时项目/5 个对象清理均通过。脚本契约 `12 passed`，证据见 `docs/evidence/windows-full-readiness-2026-08-25.json`。P0-A 仍保留环境/认证复用与真实协议目标的后续复核；不替代 Karing Android、真实通知、外部缺陷平台或生产性能验收。
- 2026-08-25 当前开发计划与导航跟踪：新增 `docs/development-plan-2026-08-25.md` 作为参考导航对应的当前执行版计划，统一工作台、测试能力、测试资产、智能中枢、系统五组边界，以及 `[x]` 本地完成、`[E]` 已实现待环境验收、`[~]` 进行中、`[-]` 阻塞的状态口径。q19 受控 API 真实目标最小链路已通过，当前优先顺序为 Karing APK/包名驱动的 Android 单设备闭环 → Windows API/Web 复核 → API 认证复用/导入/多协议与真实通知 → 外部缺陷平台 → 生产性能 → 发布收口；每个模块固定执行实现、测试、代码审查、修复、文档/记忆同步、Conventional Commit 推送。Karing APK/包名、通知供应商、外部缺陷平台和生产性能环境仍是明确外部依赖，不能用 mock、跳过项或 Worker 心跳替代。
- 2026-08-25 API 真实目标最小闭环：q19 受控 Compose 目标上使用当前账号创建临时 API 项目/模块/用例，完成评审提交、审批、Worker 执行和运行查询；`GET http://acceptance-target:8080/api/hello` 返回 200，`status_code eq 200` 断言和 `$.service` JSONPath 提取均成功，临时数据清理成功。API 执行器相关回归 `77 passed`，脱敏证据为 `docs/evidence/api-real-target-2026-08-25.json`。该证据只关闭受控 HTTP 最小链路，认证复用、OpenAPI/Postman、GraphQL/WebSocket/gRPC 真实目标和完整报告仍保持待环境验收。
- 2026-08-25 API 会话复用：q19 受控 Backend 内部认证目标的两步场景显式设置 `session_lifecycle=reuse`，登录步骤和后续 `/auth/me` 均通过，后续步骤提取当前用户成功；登录请求体密码在持久化执行证据中脱敏，临时项目清理成功。证据为 `docs/evidence/api-session-reuse-2026-08-25.json`；OpenAPI/Postman、GraphQL/WebSocket/gRPC 真实目标和完整报告仍待环境验收。
- 2026-08-25 导航分组计划同步：参考导航已固化为 N0～N9 执行台账，覆盖导航壳/工作台、接口测试、APP、UI、性能、AI、测试资产、智能中枢、系统和发布收口；`docs/development-plan-2026-08-25.md`、`Task.md`、`docs/product-navigation-roadmap-2026-08-24.md` 已统一当前游标为 N2 Karing 单设备闭环 → N0/N3 Windows 复核 → N1 其他协议/完整报告 → N4～N9 外部收口。状态仍严格区分 `[x]` 本地完成、`[E]` 已实现待环境验收、`[~]` 进行中和 `[-]` 阻塞；真实包、设备、协议服务、模型和生产基础设施缺失时不写成通过。
- 2026-08-25 N1 协议执行防护：派发层新增 GraphQL/WebSocket/gRPC 空步骤 fail-fast，缺失、空数组或 `null` 的 `config.steps` 终止为 `error`，避免零步骤运行误报通过；API 旧配置默认主请求保持兼容。定向 `94 passed`、后端非集成全量 `2279 passed`，前端类型检查/生产构建/Ruff/diff-check 通过，代码审查未发现问题。真实 GraphQL/WebSocket/流式 gRPC 和完整报告目标仍待环境验收。
- 2026-08-25 N1 协议保存校验：创建/更新 GraphQL、WebSocket、gRPC 用例时校验步骤及协议必填字段，失败返回 `422`，且在快照创建前阻断，不产生无效用例；派发层运行期兜底保持。用例管理定向 `35 passed`、后端非集成全量 `2282 passed`，Ruff/diff-check 和代码审查通过；真实协议目标和完整报告仍待环境验收。
- 2026-08-25 N1 报告中心按用例类型统计：新增 `ReportCaseTypeItem` 和 `ReportOverviewOut.case_type_stats`，后端在项目可见范围内按 `TestCase.case_type` 聚合已完成运行，返回总运行、通过、失败、异常和通过率；前端报告中心新增类型分布卡片、进度条、运行/失败摘要、空态及中英文文案。报告定向 `5 passed`、前端报告页 `3 passed`、后端非集成全量 `2282 passed`，`vue-tsc`/生产构建/Ruff/diff-check 通过，代码审查无可操作问题；真实 GraphQL/WebSocket/流式 gRPC 和完整报告环境证据仍待验收。
- 2026-08-25 Windows smoke 最新复核：Windows doctor、Backend/Frontend HTTP、Playwright `12 passed` 和浏览器矩阵通过，但 `.env` 中用于 bootstrap 的账号返回 HTTP `401`，认证读接口、文件传输、Web 低代码和报告导出未执行，不把本轮写成完整通过；后续使用当前有效账号重跑，凭据不写入文档或证据。
- 2026-08-25 Windows smoke 凭据边界修复：`scripts/windows-local-smoke.ps1` 默认只读取 `ATP_USERNAME/ATP_PASSWORD`，不再自动回退或混用 `FIRST_ADMIN_*`；仅显式 `-UseBootstrapCredentials` 才读取全新数据库的初始化账号。缺少当前账号时失败并提示，不输出凭据；脚本契约与 `docs/windows-local-run.md` 已同步。真实 API/Web smoke 仍需使用当前有效账号复验。
- 2026-08-25 报告对比边界修复：`ReportCenterView` 默认从最近运行中选择同一 `case_id` 的基线/当前组合；切换任一选择器时自动对齐另一侧的同用例记录，`canCompare`/请求前再次校验同一用例，避免后端必然返回 400。代码审查发现并修复了初版禁用选项造成的选择死锁，混合用例顺序/切换回归保留；报告页 `4 passed`，前端全量 `66 files / 270 tests passed`，type-check/build/diff-check 通过；真实报告环境验收仍待完成。
- 2026-08-25 N2 Karing 单设备稳定性专项闭环：Windows Android Worker 已配对当前 q19 受控依赖，设备 `153 / 172.16.102.91:5555` 在线，包名 `com.nebula.karing`。真实稳定性运行 `6` 与固定种子 `20260825` 的回放运行 `7` 均 `completed`；回放保存 78 条事件、序号 1～78 无重复，包含 Monkey 日志/动作、Crash、logcat 和 screenshot，JSON 报告与产物 URL 均返回 200，临时项目 `50` 已以 204 删除并确认 404。真实运行先暴露同一 `AsyncSession` 的并发 commit 缺陷，后续审查又发现调度层/执行器各自记录器导致序号重复；现已用事件循环级写锁串行化写入，并将一个 `MobileRunEventRecorder` 贯穿调度、三类执行器和产物收尾。定向回归 `82 passed`，后端非集成全量 `2306 passed`，Ruff、格式和 `git diff --check` 通过；脱敏证据见 `docs/evidence/android-karing-special-task-2026-08-25.json`。N2 仍未总体验收，下一步是同一 Worker/设备的性能专项样本与流畅度 FPS/jank 专项。
- 2026-08-26 P5/P6/P7/P8 q19 受控真实验收关闭四个外部门禁：q19 acceptance 栈就在本机 WSL2（`172.31.27.133`，部署目录 `/opt/atp-q19-external-tracker-20260825`，backend 端口 `127.0.0.1:29080`，Windows 侧经 localhost 转发可直连），宿主重启后 `postgres`/`redis`/`minio`/`acceptance-target` 均为 `Exited (255)` 且 backend 陷入重启循环；用 `wsl -u root`（免密，无需改 docker 组或 sudo 配置）恢复依赖容器后 backend 立即 healthy。q19 此前登录 401 的根因已查清：容器 `FIRST_ADMIN_USERNAME=admin`，但 bootstrap 只在首次启动生效，数据库从旧卷恢复且已有 `parado`，因此 `admin` 从未被创建。部署从 `716d1b3` 前进到 `4087974`，`716d1b3..4087974` 无迁移或模型变更，迁移头仍为 `20260825_0066`，重建 backend 镜像即可。受控模型为第三方 `openai_compatible` 端点，其 `/models` 只返回 `id`/`object`/`owned_by`、不含任何 `capabilities`/`modalities` 字段，因此走 `model-name-hint` 兼容路径；同端点上 `owned_by=xai` 的 grok 模型全部返回 HTTP 403，只有 `owned_by=antigravity` 可用。验收暴露并修复两处能力识别缺陷：`_REASONING_MARKERS` 的 `reason` 子串把 `grok-4.20-0309-non-reasoning` 误判为支持推理（改为先剥离否定片段再匹配），以及带日期的 `claude-sonnet-4-6` 既不含 `claude-3` 也不含 `claude-4` 导致整个 Claude 4 系列漏判多模态（补齐系列名与 `-image`/`_image`）。N7 `passed`（`config_id=1`、`model=claude-opus-4-6-thinking`、`discovered=29`、`vision=True`、`thinking_keys=thinking`，真实模型返回 3 份可编辑草稿，viewer 需求写入 403，临时项目已删除）、N6 `passed`（资产链到计划运行终态 `passed` 加缺陷关联，跨项目读写 403，双项目清理）、N8 `passed`（7 项脱敏诊断、无密钥配置聚合、`performance_node` 版本创建/差异、单资源精确确认回滚、有界审计 CSV、viewer 拒绝）。重要操作知识：N7 脚本默认 `--timeout 20` 低于后端 LLM 调用的 60 秒上限，思考类模型生成用例时会先超时并只报 `request failed`，启用 `--require-ai` 必须传大于 60 秒的值（本轮用 150）。按验收决定保留 q19 的 AI 配置 `config_id=1`（Key 以 Fernet 加密存储，未写入仓库、文档或证据）与临时 viewer `n7-acceptance-viewer-tmp`（`id=18`）；N8 的 `revision_id=1` 按设计留在审计历史。定向 `22 passed`、后端非集成全量 `2391 passed`、Ruff/格式/差异检查和发布索引校验（gates=7, blocking=2）通过。P4 性能仍缺 Kubernetes、发布级 Prometheus 和独立 MinIO source/target，保持 `[-]`，P9 继续 `[~]`，发布结论仍为存在未关闭门禁。
