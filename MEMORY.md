...
- 2026-08-25 已建立参考导航对齐开发计划 2.0：产品入口统一为工作台（首页/待办/项目中心/任务中心）、测试能力（接口/APP/UI/性能/AI）、测试资产（用例/计划/缺陷/报告/评审）、智能中枢（Hermes/需求与用例生成/知识中枢）和系统（远程工具箱/配置中心）。旧设备、APK、Mock、数据集、Web/API 资产 URL 保持兼容，但不再全部放在系统管理下。当前台账为 N0-N1/N3/N5-N8 本地实现待真实边界复核，N2 Karing 单设备和 N4 Kubernetes/Prometheus/独立 MinIO 仍待外部条件，N9 依赖其收口；详细范围、验收出口和跟踪规则见 `docs/development-plan-2026-08-25.md` 2.0，已同步 `Task.md`、路线图和发布状态。本轮仅更新文档，没有新增代码或环境验收结论。
