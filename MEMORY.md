...
- 2026-08-25 当前开发计划与导航跟踪：新增 `docs/development-plan-2026-08-25.md` 作为参考导航对应的当前执行版计划，统一工作台、测试能力、测试资产、智能中枢、系统五组边界，以及 `[x]` 本地完成、`[E]` 已实现待环境验收、`[~]` 进行中、`[-]` 阻塞的状态口径。当前优先顺序为 q19 受控 API 真实目标 → Karing APK/包名驱动的 Android 单设备闭环 → Windows API/Web 复核 → 真实通知 → 外部缺陷平台 → 生产性能 → 发布收口；每个模块固定执行实现、测试、代码审查、修复、文档/记忆同步、Conventional Commit 推送。Karing APK/包名、通知供应商、外部缺陷平台和生产性能环境仍是明确外部依赖，不能用 mock、跳过项或 Worker 心跳替代。
